...
- Entrypoint: `crates/evohime-core/src/stateful_tool_workbench_sessions.rs` + handler в `crates/evohime-core/src/lib.rs`; сервис `StatefulToolWorkbenchSessionsService` должен выполнять `validate → policy → bounded operation → typed result/event`.
...
- Тесты: `crates/evohime-core/tests/stateful_tool_workbench_sessions_recovery.rs` — timeout/cancel, duplicate, stale version/lease, crash до/после dispatch, restart и optional-unavailable.

### Acceptance-to-runtime matrix

- `C02` — Stateful tool collections имеют explicit lifecycle. → журналировать переходы и восстановление через replay/reconciliation.
- `C04` — Есть reset/restart/close semantics. → журналировать переходы и восстановление через replay/reconciliation.
- `C05` — Optional snapshot/restore безопасно ограничены adapter capability. → журналировать переходы и восстановление через replay/reconciliation.
- `C06` — MCP/stateful backends могут переиспользовать один lifecycle contract. → журналировать переходы и восстановление через replay/reconciliation.
- `C07` — Backend private state не становится model authority. → журналировать переходы и восстановление через replay/reconciliation.
