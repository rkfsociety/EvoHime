# План 128.0 — Local Inference Scheduler

Статус: предложено по [issue #109](https://github.com/rkfsociety/EvoHime/issues/109). Это implementation contract; функционал этим документом не считается реализованным.

## Цель

Реализовать Core-owned контур **Local Inference Scheduler** как отдельный versioned/revision-aware слой поверх существующей архитектуры EvoHime. Он должен иметь bounded контракт, явного владельца состояния, проверяемые переходы, recovery и metadata-only Electron projection. Renderer не получает authority над runtime, workspace, SQLite, secrets или policy.
...
- [Этап 1 — Core-контракт, schema и storage](./128-1-local-inference-scheduler.md)
- [Этап 2 — runtime-интеграция и recovery](./128-2-local-inference-scheduler.md)
- [Этап 3 — IPC, projection и UI](./128-3-local-inference-scheduler.md)
- [Этап 4 — verification, release evidence и закрытие](./128-4-local-inference-scheduler.md)
...
- [#109 Local Inference Scheduler](https://github.com/rkfsociety/EvoHime/issues/109)
