# План 85.3 — Customization Inventory: единый каталог Skills, Integrations, Profiles, Workflows и UI Extensions: IPC, client projection и UI

Статус: этап 3 для [плана 85.0](./85-0-customization-inventory.md); после [плана 85.2](./85-2-customization-inventory.md).
...
- План 85.2 — runtime vertical slice, recovery и стабильные commands/events.
...
- Proto: добавить additive `CustomizationInventoryRequest`, `CustomizationInventoryResponse`, `CustomizationInventoryEvent` и command/event oneof в `crates/desktop-ipc/proto/evohime.desktop.proto` после проверки свободных tags; сохранить major, replay/resync и bounded frame limits.
...
- UI: создать `desktop/evohime-electron/src/renderer/src/CustomizationInventoryPanel.tsx` только как projection/action surface; тесты — `desktop/evohime-electron/tests/customization_inventory.test.tsx` и protocol/typecheck gates.

### Acceptance-to-projection matrix

- `C02` — Skills/Integrations/MCP/Profiles/Workflows/UI Extensions можно увидеть в одном inventory. → дать bounded projection и явные Core-checked actions.
- `C03` — Runtime semantics/actions остаются у owner subsystems. → дать bounded projection и явные Core-checked actions.
- `C07` — UI имеет единый Customize catalog с filters/search/details. → дать bounded projection и явные Core-checked actions.
