...
- Recency и trust работают только как тай-брейк внутри одного уровня иерархии:
  ...
  пользователя. Если уровень, trust и время совпадают, финальный тай-брейк
  детерминированный: сравнение `content_hash`, затем `id` в лексикографическом
  порядке. Сортировка обязана быть стабильной, чтобы одинаковый вход давал
  одинаковый порядок item и одинаковый `context_ledger_hash`.
