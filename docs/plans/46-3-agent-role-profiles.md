# План 46.3 — Agent Role Profiles: versioned специализация, ограничения и strategy contracts: IPC, client projection и UI

Статус: этап 3 для [плана 46.0](./46-0-agent-role-profiles.md); после [плана 46.2](./46-2-agent-role-profiles.md).
...
- План 46.2 — runtime vertical slice, recovery и стабильные commands/events.
...
- Proto: добавить additive `AgentRoleProfilesRequest`, `AgentRoleProfilesResponse`, `AgentRoleProfilesEvent` и command/event oneof в `crates/desktop-ipc/proto/evohime.desktop.proto` после проверки свободных tags; сохранить major, replay/resync и bounded frame limits.
...
- UI: создать `desktop/evohime-electron/src/renderer/src/AgentRoleProfilesPanel.tsx` только как projection/action surface; тесты — `desktop/evohime-electron/tests/agent_role_profiles.test.tsx` и protocol/typecheck gates.

### Acceptance-to-projection matrix

- `C06` — Поддержан human/AI execution mode. → дать bounded projection и явные Core-checked actions.
