# План 86.3 — Semantic Repository Map: symbol graph и token-budgeted контекст большого репозитория: IPC, client projection и UI

Статус: этап 3 для [плана 86.0](./86-0-semantic-repository-map.md); после [плана 86.2](./86-2-semantic-repository-map.md).
...
- План 86.2 — runtime vertical slice, recovery и стабильные commands/events.
...
- Proto: добавить additive `SemanticRepositoryMapRequest`, `SemanticRepositoryMapResponse`, `SemanticRepositoryMapEvent` и command/event oneof в `crates/desktop-ipc/proto/evohime.desktop.proto` после проверки свободных tags; сохранить major, replay/resync и bounded frame limits.
...
- UI: создать `desktop/evohime-electron/src/renderer/src/SemanticRepositoryMapPanel.tsx` только как projection/action surface; тесты — `desktop/evohime-electron/tests/semantic_repository_map.test.tsx` и protocol/typecheck gates.

### Acceptance-to-projection matrix

- `C05` — Projection соблюдает explicit token budget. → показывать состояние только из Core event/evidence, без локального вывода renderer.
