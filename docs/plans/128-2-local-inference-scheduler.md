# План 128.2 — Local Inference Scheduler: runtime-интеграция и recovery

Статус: этап 2 для [плана 128.0](./128-0-local-inference-scheduler.md); issue: [#109](https://github.com/rkfsociety/EvoHime/issues/109).
...
Подключить Local Inference Scheduler к Core runtime через явные commands/state transitions. Реализовать cancellation, timeout, approval/policy checks, optimistic concurrency, crash recovery и last-known-safe behavior. Unknown, stale, denied, conflict и partial failure не превращаются в успешный результат; активный run pin-ит immutable revision.
