...
- [ ] Runtime/UI wiring registry: UI показывает выбранные role/skill, version, причины, risk, tools и acceptance criteria; пользователь может закрепить или заменить выбор.
  - Частично: `evohime-core::capability_selection` (select_for_task/reconcile_with_pin/pin/replace, 5 тестов) и `evohime-local-storage::capability_selection_store` (upsert/get/delete по task_id, переживает reconnect, 3 теста) дают полный офлайн-контракт выбора, pin и replace с персистентностью. НЕ хватает: protobuf IPC-сообщений (`GetCapabilitySelection`/`PinCapabilitySelection`/`ReplaceCapabilitySelection` по образцу `GetBuildPolicy`/`SaveBuildPolicy`), `CoreCommand`-диспетчера в `ipc_bridge.rs`/`lib.rs` и WinUI-панели в `desktop/EvoHime.Desktop`. Это отдельный по объёму proto+C# кусок; оставлено как next step, чекбокс не закрыт.
