...
- [x] Snapshot, связанный с run: `snapshot { id, run_id, workspace_hash, diff[], created_at }`; snapshot+diff атомарны. Он не включает SQLite и не откатывает external effects, и UI обязан показывать эту границу очень заметно — иначе пользователь решит, что Rollback вернул всё состояние. Реализовано как `WorkspaceSnapshot { id, run_id, baseline_workspace_hash, files, diff, created_at_ms, rollback_scope }`, персистентно через одиночный атомарный `INSERT` в `snapshots`; `rollback_scope: RollbackScope::WorkspaceFilesOnly` — не bool — прокинут в IPC и явно отрисован в WinUI rollback-диалоге.
- [x] Workspace как manifest + content hashes; snapshot ограничен размером и текстовыми файлами. Запись требует `expected_content_hash`; mismatch даёт workspace conflict, а не overwrite. Отдельно описать untracked files, renames, deletes, binaries и symlinks/reparse points. Git diff/rollback — отдельный ограниченный Core tool класса `dangerous`; auto-commit/push не входят. Untracked/binary файлы исключены из манифеста по умолчанию (allowlist расширений); symlinks/reparse points никогда не резолвятся — обнаруживаются через `symlink_metadata` и пропускаются при построении манифеста и при разрешении путей записи; rename — явная операция (`BuildChange::rename_from`), гейтится `allow_rename`, restore воссоздаёт исходный файл.
- [x] Machine-readable scope: `allowed_paths`, `allowed_operations`, `expected_outputs`, `protected_paths`, `max_files_changed`, `max_bytes_changed`, `allow_create`, `allow_delete`, `allow_rename`, `allowed_file_types`, `baseline_snapshot_id`, `acceptance_criteria`. Единый `BuildScope`, `allow_rename` теперь реально enforced против classified `"rename"` operation.
- [x] Approval UI: diff, files, path/risk/budget/timeout и immutable `intent_hash`. `PrepareBuild`/`ApplyApprovedBuild` IPC отдают `intent_hash`/`effective_permissions_hash` в стиле `canonical_call_hash`; WinUI approval-диалог (`PrepareBuildDialogAsync`) и rollback-диалог (`RestoreLatestSnapshotAsync`) читают и явно показывают их вместе с scope/budget/rollback_scope.
...
- [x] Runtime/UI wiring registry: UI показывает выбранные role/skill, version, причины, risk, tools и acceptance criteria; пользователь может закрепить или заменить выбор.
  - `evohime-core::capability_selection` (select_for_task/reconcile_with_pin/pin/replace, 5 тестов) и `evohime-local-storage::capability_selection_store` (upsert/get/delete по task_id, переживает reconnect, 3 теста) дают офлайн-контракт. Поверх него: protobuf `GetCapabilitySelection`/`PinCapabilitySelection`/`ReplaceCapabilitySelection` (`crates/desktop-ipc/proto/evohime.desktop.proto`, oneof id 58-60, по образцу `GetBuildPolicy`/`SaveBuildPolicy`), новая миграция `capability_selections` (schema v13), `CoreCommand`-варианты и диспетчер в `ipc_bridge.rs`/`lib.rs`, реконсиляция и персистентность через `EventJournal::save_capability_selection`/`get_capability_selection`, сквозной тест `capability_selection_get_pin_replace_round_trip_against_real_storage`. WinUI: `CoreIpcClient`/`ProtocolEnvelope` методы и панель "Capability Selection" (`MainWindow.xaml.cs`) с полями task_id/intent/risk, кнопками "Получить выбор"/"Закрепить"/"Заменить" (picker по `ListCapabilities`), рендером reasons/risk/tools/acceptance criteria/origin.
- [x] Lifecycle snapshot: активная definition immutable в рамках run; skill не расширяет permissions и не меняет порядок context.
  - Проверено на коде, не на веру: (a) immutable-в-рамках-run — `LocalDatabase::create_run_if_absent` использует `INSERT OR IGNORE`, поэтому `role_snapshot`/`skill_snapshot`/`policy_snapshot`/`model_route_snapshot` для существующего `run_id` никогда не перезаписываются (`crates/evohime-local-storage/src/lib.rs`, тест `persists_immutable_run_snapshots`). (b) skill не расширяет permissions — `CapabilityManifest::effective_permissions` в `crates/evohime-core/src/capability_registry.rs` берёт `intersection()` для `allowed_tools`/`allowed_domains` и отклоняет `RegistryError::RiskEscalation`, если `requested.risk_class > self.risk_class`; расширение невозможно, а не "тихо urезается" — это тот же паттерн отказа, что и у остального registry-кода. (c) порядок context — был реальный пробел: `assemble_context` в `crates/evohime-core/src/workspace.rs` не имел secции для skill-контента вовсе. Добавлено поле `ContextInput::skill_context: &[(String, String)]`, которое всегда сортируется по имени skill перед рендером и рендерится в фиксированной позиции ("## Skill context" после Non-goals, перед Workspace references) — независимо от порядка, в котором вызывающий код их передал. Тесты: `skill_context_is_ordered_deterministically_regardless_of_input_order`, `context_assembly_is_deterministic_across_repeated_runs_with_same_inputs`.
