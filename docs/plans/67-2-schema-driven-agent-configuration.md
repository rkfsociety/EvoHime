...
- Entrypoint: `crates/evohime-core/src/schema_driven_agent_configuration.rs` + handler в `crates/evohime-core/src/lib.rs`; сервис `SchemaDrivenAgentConfigurationService` должен выполнять `validate → policy → bounded operation → typed result/event`.
...
- Тесты: `crates/evohime-core/tests/schema_driven_agent_configuration_recovery.rs` — timeout/cancel, duplicate, stale version/lease, crash до/после dispatch, restart и optional-unavailable.

### Acceptance-to-runtime matrix

- `C03` — Есть layered defaults/overrides и effective snapshot. → журналировать переходы и восстановление через replay/reconciliation.
- `C06` — Apply/restart semantics формализованы. → журналировать переходы и восстановление через replay/reconciliation.
- `C07` — Active run фиксирует immutable effective config snapshot/hash. → журналировать переходы и восстановление через replay/reconciliation.
