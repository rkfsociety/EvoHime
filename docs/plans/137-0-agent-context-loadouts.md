# План 137.0 — Agent Context Loadouts

Статус: предложено по [issue #118](https://github.com/rkfsociety/EvoHime/issues/118). Это implementation contract; функционал этим документом не считается реализованным.

## Цель

Реализовать Core-owned контур **Agent Context Loadouts** как отдельный versioned/revision-aware слой поверх существующей архитектуры EvoHime. Он должен иметь bounded контракт, явного владельца состояния, проверяемые переходы, recovery и metadata-only Electron projection. Renderer не получает authority над runtime, workspace, SQLite, secrets или policy.
...
- [Этап 1 — Core-контракт, schema и storage](./137-1-agent-context-loadouts.md)
- [Этап 2 — runtime-интеграция и recovery](./137-2-agent-context-loadouts.md)
- [Этап 3 — IPC, projection и UI](./137-3-agent-context-loadouts.md)
- [Этап 4 — verification, release evidence и закрытие](./137-4-agent-context-loadouts.md)
...
- [#118 Agent Context Loadouts](https://github.com/rkfsociety/EvoHime/issues/118)
