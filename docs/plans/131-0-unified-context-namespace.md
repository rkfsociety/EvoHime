# План 131.0 — Unified Context Namespace

Статус: предложено по [issue #112](https://github.com/rkfsociety/EvoHime/issues/112). Это implementation contract; функционал этим документом не считается реализованным.

## Цель

Реализовать Core-owned контур **Unified Context Namespace** как отдельный versioned/revision-aware слой поверх существующей архитектуры EvoHime. Он должен иметь bounded контракт, явного владельца состояния, проверяемые переходы, recovery и metadata-only Electron projection. Renderer не получает authority над runtime, workspace, SQLite, secrets или policy.
...
- [Этап 1 — Core-контракт, schema и storage](./131-1-unified-context-namespace.md)
- [Этап 2 — runtime-интеграция и recovery](./131-2-unified-context-namespace.md)
- [Этап 3 — IPC, projection и UI](./131-3-unified-context-namespace.md)
- [Этап 4 — verification, release evidence и закрытие](./131-4-unified-context-namespace.md)
...
- [#112 Unified Context Namespace](https://github.com/rkfsociety/EvoHime/issues/112)
