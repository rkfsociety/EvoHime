# План 160.0 — IDE Companion Bridge

Статус: предложено по [issue #140](https://github.com/rkfsociety/EvoHime/issues/140). Это implementation contract; функционал этим документом не считается реализованным.

## Цель

Реализовать Core-owned контур **IDE Companion Bridge** как versioned/revision-aware capability поверх существующей архитектуры EvoHime. Контур должен иметь bounded contract, одного владельца состояния, безопасные transitions, recovery и metadata-only Electron projection. Renderer не получает authority над runtime, workspace, SQLite, secrets или policy.
...
- [Этап 1 — Core-контракт, schema и storage](./160-1-ide-companion-bridge.md)
- [Этап 2 — runtime-интеграция и recovery](./160-2-ide-companion-bridge.md)
- [Этап 3 — IPC, projection и UI](./160-3-ide-companion-bridge.md)
- [Этап 4 — verification, release evidence и закрытие](./160-4-ide-companion-bridge.md)
...
- [#140 IDE Companion Bridge](https://github.com/rkfsociety/EvoHime/issues/140)
