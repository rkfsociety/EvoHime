# План 58.3 — Workspace State Checkpoints: безопасный rollback файлов отдельно от task history: IPC, client projection и UI

Статус: этап 3 для [плана 58.0](./58-0-workspace-state-checkpoints.md); после [плана 58.2](./58-2-workspace-state-checkpoints.md).
...
- План 58.2 — runtime vertical slice, recovery и стабильные commands/events.
...
- Proto: добавить additive `WorkspaceStateCheckpointsRequest`, `WorkspaceStateCheckpointsResponse`, `WorkspaceStateCheckpointsEvent` и command/event oneof в `crates/desktop-ipc/proto/evohime.desktop.proto` после проверки свободных tags; сохранить major, replay/resync и bounded frame limits.
...
- UI: создать `desktop/evohime-electron/src/renderer/src/WorkspaceStateCheckpointsPanel.tsx` только как projection/action surface; тесты — `desktop/evohime-electron/tests/workspace_state_checkpoints.test.tsx` и protocol/typecheck gates.

### Acceptance-to-projection matrix

- `C05` — Preflight защищает user/external changes от тихого overwrite. → дать bounded projection и явные Core-checked actions.
