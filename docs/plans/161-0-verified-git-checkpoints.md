# План 161.0 — Verified Git Checkpoints

Статус: предложено по [issue #141](https://github.com/rkfsociety/EvoHime/issues/141). Это implementation contract; функционал этим документом не считается реализованным.

## Цель

Реализовать Core-owned контур **Verified Git Checkpoints** как versioned/revision-aware capability поверх существующей архитектуры EvoHime. Контур должен иметь bounded contract, одного владельца состояния, безопасные transitions, recovery и metadata-only Electron projection. Renderer не получает authority над runtime, workspace, SQLite, secrets или policy.
...
- [Этап 1 — Core-контракт, schema и storage](./161-1-verified-git-checkpoints.md)
- [Этап 2 — runtime-интеграция и recovery](./161-2-verified-git-checkpoints.md)
- [Этап 3 — IPC, projection и UI](./161-3-verified-git-checkpoints.md)
- [Этап 4 — verification, release evidence и закрытие](./161-4-verified-git-checkpoints.md)
...
- [#141 Verified Git Checkpoints](https://github.com/rkfsociety/EvoHime/issues/141)
