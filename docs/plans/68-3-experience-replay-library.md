# План 68.3 — Experience Replay Library: episodic trajectories, success/failure retrieval и context injection: IPC, client projection и UI

Статус: этап 3 для [плана 68.0](./68-0-experience-replay-library.md); после [плана 68.2](./68-2-experience-replay-library.md).
...
- План 68.2 — runtime vertical slice, recovery и стабильные commands/events.
...
- Proto: добавить additive `ExperienceReplayLibraryRequest`, `ExperienceReplayLibraryResponse`, `ExperienceReplayLibraryEvent` и command/event oneof в `crates/desktop-ipc/proto/evohime.desktop.proto` после проверки свободных tags; сохранить major, replay/resync и bounded frame limits.
...
- UI: создать `desktop/evohime-electron/src/renderer/src/ExperienceReplayLibraryPanel.tsx` только как projection/action surface; тесты — `desktop/evohime-electron/tests/experience_replay_library.test.tsx` и protocol/typecheck gates.

### Acceptance-to-projection matrix

- `C07` — Есть scope/sensitivity/retention policy. → показывать состояние только из Core event/evidence, без локального вывода renderer.
