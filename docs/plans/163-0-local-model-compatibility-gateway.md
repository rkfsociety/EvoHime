# План 163.0 — Local Model Compatibility Gateway

Статус: предложено по [issue #143](https://github.com/rkfsociety/EvoHime/issues/143). Это implementation contract; функционал этим документом не считается реализованным.

## Цель

Реализовать Core-owned контур **Local Model Compatibility Gateway** как versioned/revision-aware capability поверх существующей архитектуры EvoHime. Контур должен иметь bounded contract, одного владельца состояния, безопасные transitions, recovery и metadata-only Electron projection. Renderer не получает authority над runtime, workspace, SQLite, secrets или policy.
...
- [Этап 1 — Core-контракт, schema и storage](./163-1-local-model-compatibility-gateway.md)
- [Этап 2 — runtime-интеграция и recovery](./163-2-local-model-compatibility-gateway.md)
- [Этап 3 — IPC, projection и UI](./163-3-local-model-compatibility-gateway.md)
- [Этап 4 — verification, release evidence и закрытие](./163-4-local-model-compatibility-gateway.md)
...
- [#143 Local Model Compatibility Gateway](https://github.com/rkfsociety/EvoHime/issues/143)
