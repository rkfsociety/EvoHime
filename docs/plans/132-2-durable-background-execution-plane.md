# План 132.2 — Durable Background Execution Plane: runtime-интеграция и recovery

Статус: этап 2 для [плана 132.0](./132-0-durable-background-execution-plane.md); issue: [#113](https://github.com/rkfsociety/EvoHime/issues/113).
...
Подключить Durable Background Execution Plane к Core runtime через явные commands/state transitions. Реализовать cancellation, timeout, approval/policy checks, optimistic concurrency, crash recovery и last-known-safe behavior. Unknown, stale, denied, conflict и partial failure не превращаются в успешный результат; активный run pin-ит immutable revision.
