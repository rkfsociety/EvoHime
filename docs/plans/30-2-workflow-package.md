...
Провести «Workflow Package: переносимый import/export без секретов и с rebinding зависимостей» через Core runtime: validation -> authorization -> bounded operation -> typed result/event -> recovery.
...
- Entrypoint: `crates/evohime-core/src/workflow_package.rs` + handler в `crates/evohime-core/src/lib.rs`; сервис `WorkflowPackageService` должен выполнять `validate → policy → bounded operation → typed result/event`.
...
- Тесты: `crates/evohime-core/tests/workflow_package_recovery.rs` — timeout/cancel, duplicate, stale version/lease, crash до/после dispatch, restart и optional-unavailable.

### Acceptance-to-runtime matrix

- `C02` — Export удаляет credentials/secrets/runtime-specific state. → журналировать переходы и восстановление через replay/reconciliation.
