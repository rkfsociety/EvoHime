...
- Entrypoint: `crates/evohime-core/src/human_work_items.rs` + handler в `crates/evohime-core/src/lib.rs`; сервис `HumanWorkItemsService` должен выполнять `validate → policy → bounded operation → typed result/event`.
...
- `C04` — Есть accept/revise/replan semantics. → провести через typed outcome, timeout, cancellation и idempotency.
