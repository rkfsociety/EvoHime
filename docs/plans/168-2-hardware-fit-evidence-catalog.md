# План 168.2 — Hardware Fit Evidence Catalog

Статус: этап 2 для [плана 168.0](./168-0-hardware-fit-evidence-catalog.md); issue: [#148](https://github.com/rkfsociety/EvoHime/issues/148).
...
Подключить Hardware Fit Evidence Catalog к Core через typed commands и state machine. Реализовать supervised lifecycle, policy/capability/approval checks, timeout/cancellation, revision fencing, crash recovery и безопасную деградацию. Interrupted/unknown/stale/denied/conflicting operations не объявляются успешными; active runs pin-ят exact revision/identity.
