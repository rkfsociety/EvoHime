...
- Renderer: `ViewId` получает `'listening'`, новый `ListeningPanel.tsx` —
  ...
  срок хранения), установка движка распознавания, список доступных микрофонов
  и переключение устройства без перезапуска приложения. Состояния
  `DeviceConflict`, сна и деградации модели показывают понятную причину.
...
- Системный индикатор активности микрофона использует Windows API; приложение
  ...
  отключения и локализованными текстами.
...
- Скачивание рантайма движка в Electron main: контролируемый pinned URL,
  проверка Authenticode/SHA-256/ABI, атомарный staging, approval, прогресс,
  повторные попытки и сохранение предыдущей рабочей версии при ошибке.
