...
- Поддержать graceful absence: если local model не установлена, сообщать
  ...
  повод для свободного текста. Наружу, в доходящую до renderer часть trace,
  эти значения не проходят как есть: 02.3 сворачивает их в `reject_reason =
  health_unavailable` и в категорию `reason_code` из 02.1, а исходная причина
  остаётся в diagnostics. Собственного словаря для UI этап не заводит.
