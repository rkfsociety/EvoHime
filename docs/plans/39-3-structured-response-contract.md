# План 39.3 — Structured Response Contract: schema-first ответы модели с provider/tool fallback: IPC, client projection и UI

Статус: этап 3 для [плана 39.0](./39-0-structured-response-contract.md); после [плана 39.2](./39-2-structured-response-contract.md).
...
- План 39.2 — runtime vertical slice, recovery и стабильные commands/events.
...
- Proto: добавить additive `StructuredResponseContractRequest`, `StructuredResponseContractResponse`, `StructuredResponseContractEvent` и command/event oneof в `crates/desktop-ipc/proto/evohime.desktop.proto` после проверки свободных tags; сохранить major, replay/resync и bounded frame limits.
...
- UI: создать `desktop/evohime-electron/src/renderer/src/StructuredResponseContractPanel.tsx` только как projection/action surface; тесты — `desktop/evohime-electron/tests/structured_response_contract.test.tsx` и protocol/typecheck gates.

### Acceptance-to-projection matrix

- `C08` — Provider fallback может менять transport strategy без изменения caller contract. → показывать состояние только из Core event/evidence, без локального вывода renderer.
