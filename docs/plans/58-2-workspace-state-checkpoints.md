...
- Entrypoint: `crates/evohime-core/src/workspace_state_checkpoints.rs` + handler в `crates/evohime-core/src/lib.rs`; сервис `WorkspaceStateCheckpointsService` должен выполнять `validate → policy → bounded operation → typed result/event`.
...
- Тесты: `crates/evohime-core/tests/workspace_state_checkpoints_recovery.rs` — timeout/cancel, duplicate, stale version/lease, crash до/после dispatch, restart и optional-unavailable.

### Acceptance-to-runtime matrix

- `C02` — Snapshot backend не загрязняет пользовательский Git history. → журналировать переходы и восстановление через replay/reconciliation.
- `C03` — Можно сравнить checkpoint с текущим workspace. → проверить exact revision/hash перед mutation и сохранить observed evidence.
