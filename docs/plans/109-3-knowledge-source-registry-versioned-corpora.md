# План 109.3 — Knowledge Source Registry: versioned RAG corpora, ingestion lineage и role-scoped retrieval: IPC, client projection и UI

Статус: этап 3 для [плана 109.0](./109-0-knowledge-source-registry-versioned-corpora.md); после [плана 109.2](./109-2-knowledge-source-registry-versioned-corpora.md).
...
- План 109.2 — runtime vertical slice, recovery и стабильные commands/events.
...
- Proto: добавить additive `KnowledgeSourceRegistryVersionedCorporaRequest`, `KnowledgeSourceRegistryVersionedCorporaResponse`, `KnowledgeSourceRegistryVersionedCorporaEvent` и command/event oneof в `crates/desktop-ipc/proto/evohime.desktop.proto` после проверки свободных tags; сохранить major, replay/resync и bounded frame limits.
...
- UI: создать `desktop/evohime-electron/src/renderer/src/KnowledgeSourceRegistryVersionedCorporaPanel.tsx` только как projection/action surface; тесты — `desktop/evohime-electron/tests/knowledge_source_registry_versioned_corpora.test.tsx` и protocol/typecheck gates.

### Acceptance-to-projection matrix

- `C08` — Knowledge и Memory остаются разными слоями с explicit evidence links. → показывать состояние только из Core event/evidence, без локального вывода renderer.
