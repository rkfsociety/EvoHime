...
- План 106 — закрытая зависимость из обзора; conformance kit проверяет совместимость, не создавая второй runtime contract.
...
- Proto: добавить additive `ExtensionConformanceKitRequest`, `ExtensionConformanceKitResponse`, `ExtensionConformanceKitEvent` и command/event oneof в `crates/desktop-ipc/proto/evohime.desktop.proto` после проверки свободных tags; сохранить major, replay/resync и bounded frame limits.
...
- UI: создать `desktop/evohime-electron/src/renderer/src/ExtensionConformanceKitPanel.tsx` только как projection/action surface; тесты — `desktop/evohime-electron/tests/extension_conformance_kit.test.tsx` и protocol/typecheck gates.

### Acceptance-to-projection matrix

- `C02` — Registration transactional и rollback тестируется. → дать bounded projection и явные Core-checked actions.
- `C07` — Security assertions входят в обязательный suite. → дать bounded projection и явные Core-checked actions.
- `C08` — Минимум IntegrationProvider, ExternalAgentAdapter и Workbench имеют specialized suites. → дать bounded projection и явные Core-checked actions.
