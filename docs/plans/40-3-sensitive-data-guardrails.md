# План 40.3 — Sensitive Data Guardrails: PII/secret detection и streaming redaction на model/tool boundaries: IPC, client projection и UI

Статус: этап 3 для [плана 40.0](./40-0-sensitive-data-guardrails.md); после [плана 40.2](./40-2-sensitive-data-guardrails.md).
...
- План 40.2 — runtime vertical slice, recovery и стабильные commands/events.
...
- Proto: добавить additive `SensitiveDataGuardrailsRequest`, `SensitiveDataGuardrailsResponse`, `SensitiveDataGuardrailsEvent` и command/event oneof в `crates/desktop-ipc/proto/evohime.desktop.proto` после проверки свободных tags; сохранить major, replay/resync и bounded frame limits.
...
- UI: создать `desktop/evohime-electron/src/renderer/src/SensitiveDataGuardrailsPanel.tsx` только как projection/action surface; тесты — `desktop/evohime-electron/tests/sensitive_data_guardrails.test.tsx` и protocol/typecheck gates.

### Acceptance-to-projection matrix

- `C08` — Raw local authoritative data и redacted projection явно разделены. → показывать только redacted projection и provenance без raw payload.
