# План 78.3 — Capability Workbenches: lifecycle-scoped tool groups with shared state and resources: IPC, client projection и UI

Статус: этап 3 для [плана 78.0](./78-0-capability-workbenches.md); после [плана 78.2](./78-2-capability-workbenches.md).
...
- План 78.2 — runtime vertical slice, recovery и стабильные commands/events.
...
- Proto: добавить additive `CapabilityWorkbenchesRequest`, `CapabilityWorkbenchesResponse`, `CapabilityWorkbenchesEvent` и command/event oneof в `crates/desktop-ipc/proto/evohime.desktop.proto` после проверки свободных tags; сохранить major, replay/resync и bounded frame limits.
...
- UI: создать `desktop/evohime-electron/src/renderer/src/CapabilityWorkbenchesPanel.tsx` только как projection/action surface; тесты — `desktop/evohime-electron/tests/capability_workbenches.test.tsx` и protocol/typecheck gates.

### Acceptance-to-projection matrix

- `C01` — Workbench имеет versioned contract и lifecycle → дать bounded projection и явные Core-checked actions.
- `C05` — Concurrency semantics заданы каждым workbench → дать bounded projection и явные Core-checked actions.
- `C10` — UI показывает lifecycle и shared resources → дать bounded projection и явные Core-checked actions.
