# План 137.2 — Agent Context Loadouts: runtime-интеграция и recovery

Статус: этап 2 для [плана 137.0](./137-0-agent-context-loadouts.md); issue: [#118](https://github.com/rkfsociety/EvoHime/issues/118).

## Зависимости

### Блокирующие

- План 137.0 и предыдущий этап этого направления.
...
Подключить Agent Context Loadouts к Core runtime через явные commands/state transitions. Реализовать cancellation, timeout, approval/policy checks, optimistic concurrency, crash recovery и last-known-safe behavior. Unknown, stale, denied, conflict и partial failure не превращаются в успешный результат; активный run pin-ит immutable revision.
