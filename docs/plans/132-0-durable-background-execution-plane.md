# План 132.0 — Durable Background Execution Plane

Статус: предложено по [issue #113](https://github.com/rkfsociety/EvoHime/issues/113). Это implementation contract; функционал этим документом не считается реализованным.

## Цель

Реализовать Core-owned контур **Durable Background Execution Plane** как отдельный versioned/revision-aware слой поверх существующей архитектуры EvoHime. Он должен иметь bounded контракт, явного владельца состояния, проверяемые переходы, recovery и metadata-only Electron projection. Renderer не получает authority над runtime, workspace, SQLite, secrets или policy.
...
- [Этап 1 — Core-контракт, schema и storage](./132-1-durable-background-execution-plane.md)
- [Этап 2 — runtime-интеграция и recovery](./132-2-durable-background-execution-plane.md)
- [Этап 3 — IPC, projection и UI](./132-3-durable-background-execution-plane.md)
- [Этап 4 — verification, release evidence и закрытие](./132-4-durable-background-execution-plane.md)
...
- [#113 Durable Background Execution Plane](https://github.com/rkfsociety/EvoHime/issues/113)
