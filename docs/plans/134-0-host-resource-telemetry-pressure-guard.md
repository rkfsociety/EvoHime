# План 134.0 — Host Resource Telemetry & Pressure Guard

Статус: предложено по [issue #115](https://github.com/rkfsociety/EvoHime/issues/115). Это implementation contract; функционал этим документом не считается реализованным.

## Цель

Реализовать Core-owned контур **Host Resource Telemetry & Pressure Guard** как отдельный versioned/revision-aware слой поверх существующей архитектуры EvoHime. Он должен иметь bounded контракт, явного владельца состояния, проверяемые переходы, recovery и metadata-only Electron projection. Renderer не получает authority над runtime, workspace, SQLite, secrets или policy.
...
- [Этап 1 — Core-контракт, schema и storage](./134-1-host-resource-telemetry-pressure-guard.md)
- [Этап 2 — runtime-интеграция и recovery](./134-2-host-resource-telemetry-pressure-guard.md)
- [Этап 3 — IPC, projection и UI](./134-3-host-resource-telemetry-pressure-guard.md)
- [Этап 4 — verification, release evidence и закрытие](./134-4-host-resource-telemetry-pressure-guard.md)
...
- [#115 Host Resource Telemetry & Pressure Guard](https://github.com/rkfsociety/EvoHime/issues/115)
