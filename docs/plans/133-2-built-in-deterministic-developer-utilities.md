# План 133.2 — Built-in Deterministic Developer Utilities: runtime-интеграция и recovery

Статус: этап 2 для [плана 133.0](./133-0-built-in-deterministic-developer-utilities.md); issue: [#114](https://github.com/rkfsociety/EvoHime/issues/114).
...
Подключить Built-in Deterministic Developer Utilities к Core runtime через явные commands/state transitions. Реализовать cancellation, timeout, approval/policy checks, optimistic concurrency, crash recovery и last-known-safe behavior. Unknown, stale, denied, conflict и partial failure не превращаются в успешный результат; активный run pin-ит immutable revision.
