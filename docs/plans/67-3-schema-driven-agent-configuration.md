# План 67.3 — Schema-Driven Agent Configuration: Core-owned schemas для agent/conversation settings: IPC, client projection и UI

Статус: этап 3 для [плана 67.0](./67-0-schema-driven-agent-configuration.md); после [плана 67.2](./67-2-schema-driven-agent-configuration.md).
...
- План 67.2 — runtime vertical slice, recovery и стабильные commands/events.
...
- Proto: добавить additive `SchemaDrivenAgentConfigurationRequest`, `SchemaDrivenAgentConfigurationResponse`, `SchemaDrivenAgentConfigurationEvent` и command/event oneof в `crates/desktop-ipc/proto/evohime.desktop.proto` после проверки свободных tags; сохранить major, replay/resync и bounded frame limits.
...
- UI: создать `desktop/evohime-electron/src/renderer/src/SchemaDrivenAgentConfigurationPanel.tsx` только как projection/action surface; тесты — `desktop/evohime-electron/tests/schema_driven_agent_configuration.test.tsx` и protocol/typecheck gates.

### Acceptance-to-projection matrix

- `C02` — UI не является source of truth для допустимых runtime settings. → дать bounded projection и явные Core-checked actions.
- `C05` — Secret settings не возвращаются renderer как raw values. → показывать только redacted projection и provenance без raw payload.
