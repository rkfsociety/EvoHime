# План 100.3 — Workspace Sets: multi-root и cross-repository задачи с независимыми grants, VCS и checkpoints: IPC, client projection и UI

Статус: этап 3 для [плана 100.0](./100-0-workspace-sets.md); после [плана 100.2](./100-2-workspace-sets.md).
...
- План 100.2 — runtime vertical slice, recovery и стабильные commands/events.
...
- Proto: добавить additive `WorkspaceSetsRequest`, `WorkspaceSetsResponse`, `WorkspaceSetsEvent` и command/event oneof в `crates/desktop-ipc/proto/evohime.desktop.proto` после проверки свободных tags; сохранить major, replay/resync и bounded frame limits.
...
- UI: создать `desktop/evohime-electron/src/renderer/src/WorkspaceSetsPanel.tsx` только как projection/action surface; тесты — `desktop/evohime-electron/tests/workspace_sets.test.tsx` и protocol/typecheck gates.

### Acceptance-to-projection matrix

- `C08` — Active run pinned к exact set/root binding snapshot. → показывать состояние только из Core event/evidence, без локального вывода renderer.
