# План 51.3 — Causal Collaboration Bus: typed pub/sub для team agents поверх child mailbox: IPC, client projection и UI

Статус: этап 3 для [плана 51.0](./51-0-causal-collaboration-bus.md); после [плана 51.2](./51-2-causal-collaboration-bus.md).
...
- План 51.2 — runtime vertical slice, recovery и стабильные commands/events.
...
- Proto: добавить additive `CausalCollaborationBusRequest`, `CausalCollaborationBusResponse`, `CausalCollaborationBusEvent` и command/event oneof в `crates/desktop-ipc/proto/evohime.desktop.proto` после проверки свободных tags; сохранить major, replay/resync и bounded frame limits.
...
- UI: создать `desktop/evohime-electron/src/renderer/src/CausalCollaborationBusPanel.tsx` только как projection/action surface; тесты — `desktop/evohime-electron/tests/causal_collaboration_bus.test.tsx` и protocol/typecheck gates.

### Acceptance-to-projection matrix

- `C01` — Есть typed CollaborationMessage envelope. → дать bounded projection и явные Core-checked actions.
- `C06` — Durable significant messages recoverable/deduplicated. → дать bounded projection и явные Core-checked actions.
