# План 126.0 — Design Intent Review Lane

Статус: предложено по [issue #107](https://github.com/rkfsociety/EvoHime/issues/107). Это implementation contract; функционал этим документом не считается реализованным.

## Цель

Реализовать Core-owned контур **Design Intent Review Lane** как отдельный versioned/revision-aware слой поверх существующей архитектуры EvoHime. Он должен иметь bounded контракт, явного владельца состояния, проверяемые переходы, recovery и metadata-only Electron projection. Renderer не получает authority над runtime, workspace, SQLite, secrets или policy.
...
- [Этап 1 — Core-контракт, schema и storage](./126-1-design-intent-review-lane.md)
- [Этап 2 — runtime-интеграция и recovery](./126-2-design-intent-review-lane.md)
- [Этап 3 — IPC, projection и UI](./126-3-design-intent-review-lane.md)
- [Этап 4 — verification, release evidence и закрытие](./126-4-design-intent-review-lane.md)
...
- [#107 Design Intent Review Lane](https://github.com/rkfsociety/EvoHime/issues/107)
