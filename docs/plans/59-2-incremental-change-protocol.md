# План 59.2 — Incremental Change Protocol: safe requirement-delta pipeline для существующих репозиториев: runtime-интеграция и recovery

Статус: этап 2 для [плана 59.0](./59-0-incremental-change-protocol.md); после [плана 59.1](./59-1-incremental-change-protocol.md).

## Цель

Провести «Incremental Change Protocol: safe requirement-delta pipeline для существующих репозиториев» через Core runtime: validation -> authorization -> bounded operation -> typed result/event -> recovery.
...
- Entrypoint: `crates/evohime-core/src/incremental_change_protocol.rs` + handler в `crates/evohime-core/src/lib.rs`; сервис `IncrementalChangeProtocolService` должен выполнять `validate → policy → bounded operation → typed result/event`.
...
- Тесты: `crates/evohime-core/tests/incremental_change_protocol_recovery.rs` — timeout/cancel, duplicate, stale version/lease, crash до/после dispatch, restart и optional-unavailable.
