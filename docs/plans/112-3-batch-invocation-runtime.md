# План 112.3 — Batch Invocation Runtime: bounded map execution по наборам inputs с per-item isolation и resume: IPC, client projection и UI

Статус: этап 3 для [плана 112.0](./112-0-batch-invocation-runtime.md); после [плана 112.2](./112-2-batch-invocation-runtime.md).
...
- План 112.2 — runtime vertical slice, recovery и стабильные commands/events.
...
- Proto: добавить additive `BatchInvocationRuntimeRequest`, `BatchInvocationRuntimeResponse`, `BatchInvocationRuntimeEvent` и command/event oneof в `crates/desktop-ipc/proto/evohime.desktop.proto` после проверки свободных tags; сохранить major, replay/resync и bounded frame limits.
...
- UI: создать `desktop/evohime-electron/src/renderer/src/BatchInvocationRuntimePanel.tsx` только как projection/action surface; тесты — `desktop/evohime-electron/tests/batch_invocation_runtime.test.tsx` и protocol/typecheck gates.

### Acceptance-to-projection matrix

- `C08` — Есть aggregate result/export и per-item drill-down. → показывать состояние только из Core event/evidence, без локального вывода renderer.
