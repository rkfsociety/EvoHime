# План 42.3 — Model Resilience Policy: retry, fallback и provider-safe request adaptation: IPC, client projection и UI

Статус: этап 3 для [плана 42.0](./42-0-model-resilience-policy.md); после [плана 42.2](./42-2-model-resilience-policy.md).
...
- План 42.2 — runtime vertical slice, recovery и стабильные commands/events.
...
- Proto: добавить additive `ModelResiliencePolicyRequest`, `ModelResiliencePolicyResponse`, `ModelResiliencePolicyEvent` и command/event oneof в `crates/desktop-ipc/proto/evohime.desktop.proto` после проверки свободных tags; сохранить major, replay/resync и bounded frame limits.
...
- UI: создать `desktop/evohime-electron/src/renderer/src/ModelResiliencePolicyPanel.tsx` только как projection/action surface; тесты — `desktop/evohime-electron/tests/model_resilience_policy.test.tsx` и protocol/typecheck gates.

### Acceptance-to-projection matrix

- `C08` — Attempts видны в diagnostics/provenance. → показывать состояние только из Core event/evidence, без локального вывода renderer.
