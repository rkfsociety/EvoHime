# План 48.3 — Team SOP Protocols: versioned multi-agent playbooks и формальные handoff правила: IPC, client projection и UI

Статус: этап 3 для [плана 48.0](./48-0-team-sop-protocols.md); после [плана 48.2](./48-2-team-sop-protocols.md).
...
- План 48.2 — runtime vertical slice, recovery и стабильные commands/events.
...
- Proto: добавить additive `TeamSopProtocolsRequest`, `TeamSopProtocolsResponse`, `TeamSopProtocolsEvent` и command/event oneof в `crates/desktop-ipc/proto/evohime.desktop.proto` после проверки свободных tags; сохранить major, replay/resync и bounded frame limits.
...
- UI: создать `desktop/evohime-electron/src/renderer/src/TeamSopProtocolsPanel.tsx` только как projection/action surface; тесты — `desktop/evohime-electron/tests/team_sop_protocols.test.tsx` и protocol/typecheck gates.

### Acceptance-to-projection matrix

- `C07` — UI показывает roster/phases/handoffs/progress. → дать bounded projection и явные Core-checked actions.
