# План 60.3 — Revision-Safe Workspace Files: uploads/workspace/outputs namespaces и stale-write protection: IPC, client projection и UI

Статус: этап 3 для [плана 60.0](./60-0-revision-safe-workspace-files.md); после [плана 60.2](./60-2-revision-safe-workspace-files.md).
...
- План 60.2 — runtime vertical slice, recovery и стабильные commands/events.
...
- Proto: добавить additive `RevisionSafeWorkspaceFilesRequest`, `RevisionSafeWorkspaceFilesResponse`, `RevisionSafeWorkspaceFilesEvent` и command/event oneof в `crates/desktop-ipc/proto/evohime.desktop.proto` после проверки свободных tags; сохранить major, replay/resync и bounded frame limits.
...
- UI: создать `desktop/evohime-electron/src/renderer/src/RevisionSafeWorkspaceFilesPanel.tsx` только как projection/action surface; тесты — `desktop/evohime-electron/tests/revision_safe_workspace_files.test.tsx` и protocol/typecheck gates.
