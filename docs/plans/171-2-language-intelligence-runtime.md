# План 171.2 — Language Intelligence Runtime

Статус: этап 2 для [плана 171.0](./171-0-language-intelligence-runtime.md); issue: [#151](https://github.com/rkfsociety/EvoHime/issues/151).
...
Подключить Language Intelligence Runtime к Core через typed commands и state machine. Реализовать supervised lifecycle, policy/capability/approval checks, timeout/cancellation, revision fencing, crash recovery и безопасную деградацию. Interrupted/unknown/stale/denied/conflicting operations не объявляются успешными; active runs pin-ят exact revision/identity.
