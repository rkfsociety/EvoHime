# План 105.3 — Prompt Cache Planner: stable context segments, provider-aware cache hints и reuse metrics: IPC, client projection и UI

Статус: этап 3 для [плана 105.0](./105-0-prompt-cache-planner.md); после [плана 105.2](./105-2-prompt-cache-planner.md).
...
- План 105.2 — runtime vertical slice, recovery и стабильные commands/events.
...
- Proto: добавить additive `PromptCachePlannerRequest`, `PromptCachePlannerResponse`, `PromptCachePlannerEvent` и command/event oneof в `crates/desktop-ipc/proto/evohime.desktop.proto` после проверки свободных tags; сохранить major, replay/resync и bounded frame limits.
...
- UI: создать `desktop/evohime-electron/src/renderer/src/PromptCachePlannerPanel.tsx` только как projection/action surface; тесты — `desktop/evohime-electron/tests/prompt_cache_planner.test.tsx` и protocol/typecheck gates.

### Acceptance-to-projection matrix

- `C08` — Benchmark/evals покрывают cache reuse/invalidation. → показывать состояние только из Core event/evidence, без локального вывода renderer.
