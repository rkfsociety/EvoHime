# План 70.3 — Code Diagnostics Feedback Loop: LSP/compiler evidence и regression delta после agent edits: IPC, client projection и UI

Статус: этап 3 для [плана 70.0](./70-0-code-diagnostics-feedback-loop.md); после [плана 70.2](./70-2-code-diagnostics-feedback-loop.md).
...
- План 70.2 — runtime vertical slice, recovery и стабильные commands/events.
...
- Proto: добавить additive `CodeDiagnosticsFeedbackLoopRequest`, `CodeDiagnosticsFeedbackLoopResponse`, `CodeDiagnosticsFeedbackLoopEvent` и command/event oneof в `crates/desktop-ipc/proto/evohime.desktop.proto` после проверки свободных tags; сохранить major, replay/resync и bounded frame limits.
...
- UI: создать `desktop/evohime-electron/src/renderer/src/CodeDiagnosticsFeedbackLoopPanel.tsx` только как projection/action surface; тесты — `desktop/evohime-electron/tests/code_diagnostics_feedback_loop.test.tsx` и protocol/typecheck gates.

### Acceptance-to-projection matrix

- `C08` — Workbench показывает Problems/Diagnostics с regression distinction. → дать bounded projection и явные Core-checked actions.
