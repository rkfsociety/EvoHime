# План 89.3 — Checkpoint Forking & Replay: branch-and-compare запусков из сохранённого состояния: IPC, client projection и UI

Статус: этап 3 для [плана 89.0](./89-0-checkpoint-forking-and-replay.md); после [плана 89.2](./89-2-checkpoint-forking-and-replay.md).
...
- План 89.2 — runtime vertical slice, recovery и стабильные commands/events.
...
- Proto: добавить additive `CheckpointForkingAndReplayRequest`, `CheckpointForkingAndReplayResponse`, `CheckpointForkingAndReplayEvent` и command/event oneof в `crates/desktop-ipc/proto/evohime.desktop.proto` после проверки свободных tags; сохранить major, replay/resync и bounded frame limits.
...
- UI: создать `desktop/evohime-electron/src/renderer/src/CheckpointForkingAndReplayPanel.tsx` только как projection/action surface; тесты — `desktop/evohime-electron/tests/checkpoint_forking_and_replay.test.tsx` и protocol/typecheck gates.

### Acceptance-to-projection matrix

- `C07` — Есть branch lineage/comparison projection. → показывать состояние только из Core event/evidence, без локального вывода renderer.
