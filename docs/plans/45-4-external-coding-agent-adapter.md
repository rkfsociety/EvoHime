# План 45.4 — External Coding Agent Adapter: подключение Codex/Claude/Gemini-подобных executors через typed protocol: verification, release-evidence и закрытие

Статус: этап 4 для [плана 45.0](./45-0-external-coding-agent-adapter.md); после [плана 45.3](./45-3-external-coding-agent-adapter.md).
...
- План 45.3 — Core, IPC и client projection.
...
- После закрытия всего направления и переноса подтверждённого контракта удалить комплект `45-0` … `45-4`; отдельные stage-файлы до этого не удалять. Незавершённое направление оставить blocked с evidence.
...
- [issue](https://github.com/rkfsociety/EvoHime/issues/25)
