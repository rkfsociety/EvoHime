...
- Proto: добавить additive `ModelPurposeRoutingRequest`, `ModelPurposeRoutingResponse`, `ModelPurposeRoutingEvent` и command/event oneof в `crates/desktop-ipc/proto/evohime.desktop.proto` после проверки свободных tags; сохранить major, replay/resync и bounded frame limits.
...
- UI: создать `desktop/evohime-electron/src/renderer/src/ModelPurposeRoutingPanel.tsx` только как projection/action surface; тесты — `desktop/evohime-electron/tests/model_purpose_routing.test.tsx` и protocol/typecheck gates.

### Acceptance-to-projection matrix

- `C04` — Purpose задаёт requirements, tool ceiling и context policy. → дать bounded projection и явные Core-checked actions.
- `C08` — UI позволяет настраивать основные purpose routes через ModelProfile refs. → дать bounded projection и явные Core-checked actions.
