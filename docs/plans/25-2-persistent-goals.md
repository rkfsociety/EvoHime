# План 25.2 — Persistent Goals для длительных задач: runtime-интеграция и recovery

Статус: самостоятельный этап 2 для [плана 25.0](./25-0-persistent-goals.md); начинается после [плана 25.1](./25-1-persistent-goals.md).

## Цель

Подключить контракт «Persistent Goals для длительных задач» к Core runtime одним вертикальным срезом: validated request -> policy/capability/approval -> bounded execution -> typed result/event -> recovery.

## Зависимости

### Блокирующие

- План 25.1 — contract, validators, error codes и storage policy.
