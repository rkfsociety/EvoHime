# План 107.3 — Guided Calibration Sessions: iterative human feedback и versioned role guidance: IPC, client projection и UI

Статус: этап 3 для [плана 107.0](./107-0-guided-calibration-sessions.md); после [плана 107.2](./107-2-guided-calibration-sessions.md).
...
- План 107.2 — runtime vertical slice, recovery и стабильные commands/events.
...
- Proto: добавить additive `GuidedCalibrationSessionsRequest`, `GuidedCalibrationSessionsResponse`, `GuidedCalibrationSessionsEvent` и command/event oneof в `crates/desktop-ipc/proto/evohime.desktop.proto` после проверки свободных tags; сохранить major, replay/resync и bounded frame limits.
...
- UI: создать `desktop/evohime-electron/src/renderer/src/GuidedCalibrationSessionsPanel.tsx` только как projection/action surface; тесты — `desktop/evohime-electron/tests/guided_calibration_sessions.test.tsx` и protocol/typecheck gates.

### Acceptance-to-projection matrix

- `C03` — Human feedback структурирован и имеет provenance. → дать bounded projection и явные Core-checked actions.
- `C04` — Temporary guidance не выходит за session. → дать bounded projection и явные Core-checked actions.
- `C05` — Consolidation создаёт atomic versioned guidance candidates. → дать bounded projection и явные Core-checked actions.
- `C08` — Automated evals учитываются вместе с human feedback. → дать bounded projection и явные Core-checked actions.
