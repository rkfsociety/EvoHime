# План 126.2 — Design Intent Review Lane: runtime-интеграция и recovery

Статус: этап 2 для [плана 126.0](./126-0-design-intent-review-lane.md); issue: [#107](https://github.com/rkfsociety/EvoHime/issues/107).
...
Подключить Design Intent Review Lane к Core runtime через явные commands/state transitions. Реализовать cancellation, timeout, approval/policy checks, optimistic concurrency, crash recovery и last-known-safe behavior. Unknown, stale, denied, conflict и partial failure не превращаются в успешный результат; активный run pin-ит immutable revision.
