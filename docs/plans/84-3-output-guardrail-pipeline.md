# План 84.3 — Output Guardrail Pipeline: semantic validators, transforms и bounded correction loops: IPC, client projection и UI

Статус: этап 3 для [плана 84.0](./84-0-output-guardrail-pipeline.md); после [плана 84.2](./84-2-output-guardrail-pipeline.md).
...
- План 84.2 — runtime vertical slice, recovery и стабильные commands/events.
...
- Proto: добавить additive `OutputGuardrailPipelineRequest`, `OutputGuardrailPipelineResponse`, `OutputGuardrailPipelineEvent` и command/event oneof в `crates/desktop-ipc/proto/evohime.desktop.proto` после проверки свободных tags; сохранить major, replay/resync и bounded frame limits.
...
- UI: создать `desktop/evohime-electron/src/renderer/src/OutputGuardrailPipelinePanel.tsx` только как projection/action surface; тесты — `desktop/evohime-electron/tests/output_guardrail_pipeline.test.tsx` и protocol/typecheck gates.

### Acceptance-to-projection matrix

- `C08` — UI показывает validation state без смешения классов доказательств. → дать bounded projection и явные Core-checked actions.
