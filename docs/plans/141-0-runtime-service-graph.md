# План 141.0 — Runtime Service Graph

Статус: предложено по [issue #122](https://github.com/rkfsociety/EvoHime/issues/122). Это implementation contract; функционал этим документом не считается реализованным.

## Цель

Реализовать Core-owned контур **Runtime Service Graph** как отдельный versioned/revision-aware слой поверх существующей архитектуры EvoHime. Он должен иметь bounded контракт, явного владельца состояния, проверяемые переходы, recovery и metadata-only Electron projection. Renderer не получает authority над runtime, workspace, SQLite, secrets или policy.
...
- [Этап 1 — Core-контракт, schema и storage](./141-1-runtime-service-graph.md)
- [Этап 2 — runtime-интеграция и recovery](./141-2-runtime-service-graph.md)
- [Этап 3 — IPC, projection и UI](./141-3-runtime-service-graph.md)
- [Этап 4 — verification, release evidence и закрытие](./141-4-runtime-service-graph.md)
...
- [#122 Runtime Service Graph](https://github.com/rkfsociety/EvoHime/issues/122)
