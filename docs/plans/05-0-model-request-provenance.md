...
1. **05.2 ждёт 05.8 по хранению текста.** До появления правил удаления хранилище работает в hash-only режиме: метаданные и хеши есть, captured model-visible текст не пишется. Включать хранение текста раньше, чем существует его удаление, запрещено.
