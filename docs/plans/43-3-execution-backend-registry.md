# План 43.3 — Execution Backend Registry: несколько agent backends, health и capability handshake: IPC, client projection и UI

Статус: этап 3 для [плана 43.0](./43-0-execution-backend-registry.md); после [плана 43.2](./43-2-execution-backend-registry.md).
...
- План 43.2 — runtime vertical slice, recovery и стабильные commands/events.
...
- Proto: добавить additive `ExecutionBackendRegistryRequest`, `ExecutionBackendRegistryResponse`, `ExecutionBackendRegistryEvent` и command/event oneof в `crates/desktop-ipc/proto/evohime.desktop.proto` после проверки свободных tags; сохранить major, replay/resync и bounded frame limits.
...
- UI: создать `desktop/evohime-electron/src/renderer/src/ExecutionBackendRegistryPanel.tsx` только как projection/action surface; тесты — `desktop/evohime-electron/tests/execution_backend_registry.test.tsx` и protocol/typecheck gates.

### Acceptance-to-projection matrix

- `C04` — Conversation/run фиксирует backend snapshot. → дать bounded projection и явные Core-checked actions.
