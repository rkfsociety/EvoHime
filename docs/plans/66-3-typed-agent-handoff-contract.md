# План 66.3 — Typed Agent Handoff Contract: explicit transfer of task ownership and context: IPC, client projection и UI

Статус: этап 3 для [плана 66.0](./66-0-typed-agent-handoff-contract.md); после [плана 66.2](./66-2-typed-agent-handoff-contract.md).
...
- План 66.2 — runtime vertical slice, recovery и стабильные commands/events.
...
- Proto: добавить additive `TypedAgentHandoffContractRequest`, `TypedAgentHandoffContractResponse`, `TypedAgentHandoffContractEvent` и command/event oneof в `crates/desktop-ipc/proto/evohime.desktop.proto` после проверки свободных tags; сохранить major, replay/resync и bounded frame limits.
...
- UI: создать `desktop/evohime-electron/src/renderer/src/TypedAgentHandoffContractPanel.tsx` только как projection/action surface; тесты — `desktop/evohime-electron/tests/typed_agent_handoff_contract.test.tsx` и protocol/typecheck gates.

### Acceptance-to-projection matrix

- `C06` — UI отображает полный lifecycle → дать bounded projection и явные Core-checked actions.
