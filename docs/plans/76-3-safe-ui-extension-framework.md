# План 76.3 — Safe UI Extension Framework: declarative pages, panels и themes без renderer authority: IPC, client projection и UI

Статус: этап 3 для [плана 76.0](./76-0-safe-ui-extension-framework.md); после [плана 76.2](./76-2-safe-ui-extension-framework.md).
...
- План 76.2 — runtime vertical slice, recovery и стабильные commands/events.
...
- Proto: добавить additive `SafeUiExtensionFrameworkRequest`, `SafeUiExtensionFrameworkResponse`, `SafeUiExtensionFrameworkEvent` и command/event oneof в `crates/desktop-ipc/proto/evohime.desktop.proto` после проверки свободных tags; сохранить major, replay/resync и bounded frame limits.
...
- UI: создать `desktop/evohime-electron/src/renderer/src/SafeUiExtensionFrameworkPanel.tsx` только как projection/action surface; тесты — `desktop/evohime-electron/tests/safe_ui_extension_framework.test.tsx` и protocol/typecheck gates.

### Acceptance-to-projection matrix

- `C01` — Есть versioned UiExtensionManifest. → дать bounded projection и явные Core-checked actions.
- `C03` — V1 contributions declarative/host-rendered, без unrestricted renderer code. → дать bounded projection и явные Core-checked actions.
- `C04` — Data/actions привязаны к Core-owned registries. → дать bounded projection и явные Core-checked actions.
- `C08` — Ошибка extension не ломает основной UI. → дать bounded projection и явные Core-checked actions.
