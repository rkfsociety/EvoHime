...
- Entrypoint: `crates/evohime-core/src/core_topic_subscription_event_bus.rs` + handler в `crates/evohime-core/src/lib.rs`; сервис `CoreTopicSubscriptionEventBusService` должен выполнять `validate → policy → bounded operation → typed result/event`.
...
- Тесты: `crates/evohime-core/tests/core_topic_subscription_event_bus_recovery.rs` — timeout/cancel, duplicate, stale version/lease, crash до/после dispatch, restart и optional-unavailable.

### Acceptance-to-runtime matrix

- `C01` — Topic/EventEnvelope/Subscription contracts versioned → нормализовать, дедуплицировать и ограничить вход до enqueue.
- `C02` — Ephemeral и Durable delivery поддержаны → журналировать переходы и восстановление через replay/reconciliation.
- `C03` — Durable delivery имеет ACK/NACK/retry/dead-letter → журналировать переходы и восстановление через replay/reconciliation.
- `C05` — Есть backpressure и idempotency model → разрешить Core snapshot, проверить capability/locality и закрепить его на run.
- `C06` — Publish/subscribe защищены capabilities → нормализовать, дедуплицировать и ограничить вход до enqueue.
- `C07` — Restart не теряет correctness-critical events → журналировать переходы и восстановление через replay/reconciliation.
