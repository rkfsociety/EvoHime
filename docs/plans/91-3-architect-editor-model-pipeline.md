# План 91.3 — Architect-Editor Model Pipeline: раздельные reasoning и code-editing фазы: IPC, client projection и UI

Статус: этап 3 для [плана 91.0](./91-0-architect-editor-model-pipeline.md); после [плана 91.2](./91-2-architect-editor-model-pipeline.md).
...
- План 91.2 — runtime vertical slice, recovery и стабильные commands/events.
...
- Proto: добавить additive `ArchitectEditorModelPipelineRequest`, `ArchitectEditorModelPipelineResponse`, `ArchitectEditorModelPipelineEvent` и command/event oneof в `crates/desktop-ipc/proto/evohime.desktop.proto` после проверки свободных tags; сохранить major, replay/resync и bounded frame limits.
...
- UI: создать `desktop/evohime-electron/src/renderer/src/ArchitectEditorModelPipelinePanel.tsx` только как projection/action surface; тесты — `desktop/evohime-electron/tests/architect_editor_model_pipeline.test.tsx` и protocol/typecheck gates.

### Acceptance-to-projection matrix

- `C08` — UI/metrics различают architect/editor purpose. → дать bounded projection и явные Core-checked actions.
