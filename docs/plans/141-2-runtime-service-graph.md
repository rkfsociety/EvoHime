# План 141.2 — Runtime Service Graph: runtime-интеграция и recovery

Статус: этап 2 для [плана 141.0](./141-0-runtime-service-graph.md); issue: [#122](https://github.com/rkfsociety/EvoHime/issues/122).

## Зависимости

### Блокирующие

- План 141.0 и предыдущий этап этого направления.
...
Подключить Runtime Service Graph к Core runtime через явные commands/state transitions. Реализовать cancellation, timeout, approval/policy checks, optimistic concurrency, crash recovery и last-known-safe behavior. Unknown, stale, denied, conflict и partial failure не превращаются в успешный результат; активный run pin-ит immutable revision.
