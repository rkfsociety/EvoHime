# План 61.3 — Task Worktree Isolation: отдельные Git worktrees для параллельных agent/child задач: IPC, client projection и UI

Статус: этап 3 для [плана 61.0](./61-0-task-worktree-isolation.md); после [плана 61.2](./61-2-task-worktree-isolation.md).
...
- План 61.2 — runtime vertical slice, recovery и стабильные commands/events.
...
- Proto: добавить additive `TaskWorktreeIsolationRequest`, `TaskWorktreeIsolationResponse`, `TaskWorktreeIsolationEvent` и command/event oneof в `crates/desktop-ipc/proto/evohime.desktop.proto` после проверки свободных tags; сохранить major, replay/resync и bounded frame limits.
...
- UI: создать `desktop/evohime-electron/src/renderer/src/TaskWorktreeIsolationPanel.tsx` только как projection/action surface; тесты — `desktop/evohime-electron/tests/task_worktree_isolation.test.tsx` и protocol/typecheck gates.

### Acceptance-to-projection matrix

- `C04` — Есть explicit integration actions с preflight/conflict semantics. → дать bounded projection и явные Core-checked actions.
