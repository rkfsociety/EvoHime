# План 34.4 — Event Trigger Runtime: безопасный запуск workflow по внешним событиям: verification, release-evidence и закрытие

Статус: этап 4 для [плана 34.0](./34-0-event-trigger-runtime.md); после [плана 34.3](./34-3-event-trigger-runtime.md).
...
- План 34.3 — Core, IPC и client projection.
...
6. Отдельно воспроизвести issue-кейсы: valid/invalid signature, schema/content-type/size rejection, duplicate, mapping only selected fields, workflow pinning, credential revoke, accepted pending event after restart, storm circuit breaker, self-trigger suppression, approval retention, subscription reconcile, local watcher debounce и import без auto-enable.
...
- После закрытия всего направления и переноса подтверждённого контракта удалить комплект `34-0` … `34-4`; отдельные stage-файлы до этого не удалять. Незавершённое направление оставить blocked с evidence.
...
- [issue](https://github.com/rkfsociety/EvoHime/issues/14)
