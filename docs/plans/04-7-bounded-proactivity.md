...
- Потолки (`ProactivityBudget` из 04.1): не более 3 предложений в час и 10 в
  ...
  список из общей политики. Превышение потолка — предложение **отбрасывается**
  со счётчиком, а не копится в очередь: иначе после часа тишины пользователь
  получит десять карточек разом.
...
- Принятие предложения превращает его в обычную задачу или напоминание и
  проходит штатный approval-путь Core.
