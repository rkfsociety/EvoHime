# План 41.3 — Execution Policy Profiles: sandboxed shell/process runtime с Windows-first isolation: IPC, client projection и UI

Статус: этап 3 для [плана 41.0](./41-0-execution-policy-profiles.md); после [плана 41.2](./41-2-execution-policy-profiles.md).
...
- План 41.2 — runtime vertical slice, recovery и стабильные commands/events.
...
- Proto: добавить additive `ExecutionPolicyProfilesRequest`, `ExecutionPolicyProfilesResponse`, `ExecutionPolicyProfilesEvent` и command/event oneof в `crates/desktop-ipc/proto/evohime.desktop.proto` после проверки свободных tags; сохранить major, replay/resync и bounded frame limits.
...
- UI: создать `desktop/evohime-electron/src/renderer/src/ExecutionPolicyProfilesPanel.tsx` только как projection/action surface; тесты — `desktop/evohime-electron/tests/execution_policy_profiles.test.tsx` и protocol/typecheck gates.

### Acceptance-to-projection matrix

- `C06` — Required sandbox profile fail-closed при недоступности. → дать bounded projection и явные Core-checked actions.
- `C07` — UI/trace показывает фактический resolved execution profile. → дать bounded projection и явные Core-checked actions.
