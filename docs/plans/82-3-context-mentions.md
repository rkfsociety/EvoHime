# План 82.3 — Context Mentions: typed @references для files, folders, git, diagnostics и runtime resources: IPC, client projection и UI

Статус: этап 3 для [плана 82.0](./82-0-context-mentions.md); после [плана 82.2](./82-2-context-mentions.md).
...
- План 82.2 — runtime vertical slice, recovery и стабильные commands/events.
...
- Proto: добавить additive `ContextMentionsRequest`, `ContextMentionsResponse`, `ContextMentionsEvent` и command/event oneof в `crates/desktop-ipc/proto/evohime.desktop.proto` после проверки свободных tags; сохранить major, replay/resync и bounded frame limits.
...
- UI: создать `desktop/evohime-electron/src/renderer/src/ContextMentionsPanel.tsx` только как projection/action surface; тесты — `desktop/evohime-electron/tests/context_mentions.test.tsx` и protocol/typecheck gates.

### Acceptance-to-projection matrix

- `C04` — Folder/terminal/large resources имеют bounded projections. → показывать состояние только из Core event/evidence, без локального вывода renderer.
- `C05` — Multi-root ambiguity обрабатывается явно. → дать bounded projection и явные Core-checked actions.
- `C06` — Mention UX имеет autocomplete/chips. → дать bounded projection и явные Core-checked actions.
