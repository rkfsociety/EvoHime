# План 74.3 — Declarative Agent Component Registry: versioned runtime components with schema-safe loading: IPC, client projection и UI

Статус: этап 3 для [плана 74.0](./74-0-declarative-agent-component-registry.md); после [плана 74.2](./74-2-declarative-agent-component-registry.md).
...
- План 74.2 — runtime vertical slice, recovery и стабильные commands/events.
...
- Proto: добавить additive `DeclarativeAgentComponentRegistryRequest`, `DeclarativeAgentComponentRegistryResponse`, `DeclarativeAgentComponentRegistryEvent` и command/event oneof в `crates/desktop-ipc/proto/evohime.desktop.proto` после проверки свободных tags; сохранить major, replay/resync и bounded frame limits.
...
- UI: создать `desktop/evohime-electron/src/renderer/src/DeclarativeAgentComponentRegistryPanel.tsx` только как projection/action surface; тесты — `desktop/evohime-electron/tests/declarative_agent_component_registry.test.tsx` и protocol/typecheck gates.

### Acceptance-to-projection matrix

- `C09` — Есть inspect/diff/migration UX → дать bounded projection и явные Core-checked actions.
