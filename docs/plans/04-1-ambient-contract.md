...
- Правила логирования. Существующий `StructuredLogger::write(level, event,
  fields: serde_json::Value)` (`crates/evohime-core/src/logging.rs:26`)
  принимает **произвольный** JSON — навесить на него allow-list нельзя.
  Поэтому ambient-логирование идёт через типизированный фасад: набор полей —
  фиксированная структура, а сырой логгер из ambient-пути недостижим. Тест
  «произвольный текст не пишется» проверяет именно фасад; на `StructuredLogger`
  такой тест невыразим. Отдельно учитывается, что у логгера есть fallback-путь
  `%TEMP%\evohime-log-<pid>.jsonl` (`logging.rs:54`) — он существует и для
  листенера, и privacy-тесты 04.3 обязаны его знать. Текст, хеш текста, имя процесса и заголовок окна в логи
