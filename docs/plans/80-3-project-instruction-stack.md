# План 80.3 — Project Instruction Stack: conditional rules, AGENTS.md compatibility и deterministic precedence: IPC, client projection и UI

Статус: этап 3 для [плана 80.0](./80-0-project-instruction-stack.md); после [плана 80.2](./80-2-project-instruction-stack.md).
...
- План 80.2 — runtime vertical slice, recovery и стабильные commands/events.
...
- Proto: добавить additive `ProjectInstructionStackRequest`, `ProjectInstructionStackResponse`, `ProjectInstructionStackEvent` и command/event oneof в `crates/desktop-ipc/proto/evohime.desktop.proto` после проверки свободных tags; сохранить major, replay/resync и bounded frame limits.
...
- UI: создать `desktop/evohime-electron/src/renderer/src/ProjectInstructionStackPanel.tsx` только как projection/action surface; тесты — `desktop/evohime-electron/tests/project_instruction_stack.test.tsx` и protocol/typecheck gates.

### Acceptance-to-projection matrix

- `C08` — UI показывает active rules, source и причину активации. → дать bounded projection и явные Core-checked actions.
