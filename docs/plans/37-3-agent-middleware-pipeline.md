# План 37.3 — Agent Middleware Pipeline: typed hooks вокруг model/tool execution: IPC, client projection и UI

Статус: этап 3 для [плана 37.0](./37-0-agent-middleware-pipeline.md); после [плана 37.2](./37-2-agent-middleware-pipeline.md).
...
- План 37.2 — runtime vertical slice, recovery и стабильные commands/events.
...
- Proto: добавить additive `AgentMiddlewarePipelineRequest`, `AgentMiddlewarePipelineResponse`, `AgentMiddlewarePipelineEvent` и command/event oneof в `crates/desktop-ipc/proto/evohime.desktop.proto` после проверки свободных tags; сохранить major, replay/resync и bounded frame limits.
...
- UI: создать `desktop/evohime-electron/src/renderer/src/AgentMiddlewarePipelinePanel.tsx` только как projection/action surface; тесты — `desktop/evohime-electron/tests/agent_middleware_pipeline.test.tsx` и protocol/typecheck gates.

### Acceptance-to-projection matrix

- `C08` — Built-in policies можно реализовывать поверх pipeline без специальных веток agent loop. → дать bounded projection и явные Core-checked actions.
