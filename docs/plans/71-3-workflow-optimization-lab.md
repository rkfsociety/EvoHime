# План 71.3 — Workflow Optimization Lab: offline search и benchmark-driven улучшение agent workflows: IPC, client projection и UI

Статус: этап 3 для [плана 71.0](./71-0-workflow-optimization-lab.md); после [плана 71.2](./71-2-workflow-optimization-lab.md).
...
- План 71.2 — runtime vertical slice, recovery и стабильные commands/events.
...
- Proto: добавить additive `WorkflowOptimizationLabRequest`, `WorkflowOptimizationLabResponse`, `WorkflowOptimizationLabEvent` и command/event oneof в `crates/desktop-ipc/proto/evohime.desktop.proto` после проверки свободных tags; сохранить major, replay/resync и bounded frame limits.
...
- UI: создать `desktop/evohime-electron/src/renderer/src/WorkflowOptimizationLabPanel.tsx` только как projection/action surface; тесты — `desktop/evohime-electron/tests/workflow_optimization_lab.test.tsx` и protocol/typecheck gates.

### Acceptance-to-projection matrix

- `C08` — Promotion в production только explicit и versioned. → показывать состояние только из Core event/evidence, без локального вывода renderer.
