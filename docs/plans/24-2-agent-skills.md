# План 24.2 — Agent Skills registry и progressive disclosure: runtime-интеграция и recovery

Статус: самостоятельный этап 2 для [плана 24.0](./24-0-agent-skills.md); начинается после [плана 24.1](./24-1-agent-skills.md).

## Цель

Подключить контракт «Agent Skills registry и progressive disclosure» к Core runtime одним вертикальным срезом: validated request -> policy/capability/approval -> bounded execution -> typed result/event -> recovery.

## Зависимости

### Блокирующие

- План 24.1 — contract, validators, error codes и storage policy.
