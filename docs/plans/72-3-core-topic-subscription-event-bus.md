# План 72.3 — Core Topic/Subscription Event Bus: typed pub/sub routing for agent runtime: IPC, client projection и UI

Статус: этап 3 для [плана 72.0](./72-0-core-topic-subscription-event-bus.md); после [плана 72.2](./72-2-core-topic-subscription-event-bus.md).
...
- План 72.2 — runtime vertical slice, recovery и стабильные commands/events.
...
- Proto: добавить additive `CoreTopicSubscriptionEventBusRequest`, `CoreTopicSubscriptionEventBusResponse`, `CoreTopicSubscriptionEventBusEvent` и command/event oneof в `crates/desktop-ipc/proto/evohime.desktop.proto` после проверки свободных tags; сохранить major, replay/resync и bounded frame limits.
...
- UI: создать `desktop/evohime-electron/src/renderer/src/CoreTopicSubscriptionEventBusPanel.tsx` только как projection/action surface; тесты — `desktop/evohime-electron/tests/core_topic_subscription_event_bus.test.tsx` и protocol/typecheck gates.

### Acceptance-to-projection matrix

- `C08` — Local Core implementation не требует внешнего broker → показывать состояние только из Core event/evidence, без локального вывода renderer.
