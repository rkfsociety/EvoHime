# План 88.3 — Approval Policy Profiles: granular standing decisions без blanket auto-approve: IPC, client projection и UI

Статус: этап 3 для [плана 88.0](./88-0-approval-policy-profiles.md); после [плана 88.2](./88-2-approval-policy-profiles.md).
...
- План 88.2 — runtime vertical slice, recovery и стабильные commands/events.
...
- Proto: добавить additive `ApprovalPolicyProfilesRequest`, `ApprovalPolicyProfilesResponse`, `ApprovalPolicyProfilesEvent` и command/event oneof в `crates/desktop-ipc/proto/evohime.desktop.proto` после проверки свободных tags; сохранить major, replay/resync и bounded frame limits.
...
- UI: создать `desktop/evohime-electron/src/renderer/src/ApprovalPolicyProfilesPanel.tsx` только как projection/action surface; тесты — `desktop/evohime-electron/tests/approval_policy_profiles.test.tsx` и protocol/typecheck gates.

### Acceptance-to-projection matrix

- `C02` — Rules матчат stable Core action/risk metadata и deterministic constraints. → дать bounded projection и явные Core-checked actions.
- `C03` — Core, а не модель, является authority по approval requirement. → дать bounded projection и явные Core-checked actions.
- `C04` — Есть run/conversation/workspace scopes и expiration. → дать bounded projection и явные Core-checked actions.
- `C05` — Hard approval requirements нельзя снять profile-ом. → дать bounded projection и явные Core-checked actions.
