...
- Proto: добавить additive `CodeAnchoredIntentMarkersRequest`, `CodeAnchoredIntentMarkersResponse`, `CodeAnchoredIntentMarkersEvent` и command/event oneof в `crates/desktop-ipc/proto/evohime.desktop.proto` после проверки свободных tags; сохранить major, replay/resync и bounded frame limits.
...
- UI: создать `desktop/evohime-electron/src/renderer/src/CodeAnchoredIntentMarkersPanel.tsx` только как projection/action surface; тесты — `desktop/evohime-electron/tests/code_anchored_intent_markers.test.tsx` и protocol/typecheck gates.

### Acceptance-to-projection matrix

- `C09` — Workbench/conversation умеет перейти marker -> run -> diff/result. → дать bounded projection и явные Core-checked actions.
