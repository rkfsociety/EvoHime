...
- Entrypoint: `crates/evohime-core/src/agent_git_change_sets.rs` + handler в `crates/evohime-core/src/lib.rs`; сервис `AgentGitChangeSetsService` должен выполнять `validate → policy → bounded operation → typed result/event`.
...
- Тесты: `crates/evohime-core/tests/agent_git_change_sets_recovery.rs` — timeout/cancel, duplicate, stale version/lease, crash до/после dispatch, restart и optional-unavailable.

### Acceptance-to-runtime matrix

- `C01` — Agent change set связан с exact Git/workspace baseline. → разрешить Core snapshot, проверить capability/locality и закрепить его на run.
