# План 110.3 — Message Intervention Policies: typed interceptors для agent/team message routing: IPC, client projection и UI

Статус: этап 3 для [плана 110.0](./110-0-message-intervention-policies.md); после [плана 110.2](./110-2-message-intervention-policies.md).
...
- План 110.2 — runtime vertical slice, recovery и стабильные commands/events.
...
- Proto: добавить additive `MessageInterventionPoliciesRequest`, `MessageInterventionPoliciesResponse`, `MessageInterventionPoliciesEvent` и command/event oneof в `crates/desktop-ipc/proto/evohime.desktop.proto` после проверки свободных tags; сохранить major, replay/resync и bounded frame limits.
...
- UI: создать `desktop/evohime-electron/src/renderer/src/MessageInterventionPoliciesPanel.tsx` только как projection/action surface; тесты — `desktop/evohime-electron/tests/message_intervention_policies.test.tsx` и protocol/typecheck gates.

### Acceptance-to-projection matrix

- `C01` — Есть versioned MessageInterventionPolicy pipeline. → дать bounded projection и явные Core-checked actions.
- `C04` — Projection patches ограничены typed operations. → показывать состояние только из Core event/evidence, без локального вывода renderer.
- `C06` — Есть human escalation path. → дать bounded projection и явные Core-checked actions.
