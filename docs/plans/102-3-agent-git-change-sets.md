# План 102.3 — Agent Git Change Sets: безопасные commit-кандидаты, attribution и undo boundary: IPC, client projection и UI

Статус: этап 3 для [плана 102.0](./102-0-agent-git-change-sets.md); после [плана 102.2](./102-2-agent-git-change-sets.md).
...
- План 102.2 — runtime vertical slice, recovery и стабильные commands/events.
...
- Proto: добавить additive `AgentGitChangeSetsRequest`, `AgentGitChangeSetsResponse`, `AgentGitChangeSetsEvent` и command/event oneof в `crates/desktop-ipc/proto/evohime.desktop.proto` после проверки свободных tags; сохранить major, replay/resync и bounded frame limits.
...
- UI: создать `desktop/evohime-electron/src/renderer/src/AgentGitChangeSetsPanel.tsx` только как projection/action surface; тесты — `desktop/evohime-electron/tests/agent_git_change_sets.test.tsx` и protocol/typecheck gates.

### Acceptance-to-projection matrix

- `C02` — Pre-existing/user/external changes классифицируются отдельно. → дать bounded projection и явные Core-checked actions.
- `C05` — Есть preview/message/commit flow и safe undo semantics. → дать bounded projection и явные Core-checked actions.
- `C06` — Attribution configurable и не подменяет user Git identity молча. → дать bounded projection и явные Core-checked actions.
