# План 59.4 — Incremental Change Protocol: safe requirement-delta pipeline для существующих репозиториев: verification, release-evidence и закрытие

Статус: этап 4 для [плана 59.0](./59-0-incremental-change-protocol.md); после [плана 59.3](./59-3-incremental-change-protocol.md).
...
- План 59.3 — Core, IPC и client projection.
...
6. Dirty-worktree, user-edit-between-plan/apply, scope drift, exact review/test
   fingerprint and parallel overlap/reconciliation tests.
...
- После закрытия всего направления и переноса подтверждённого контракта удалить комплект `59-0` … `59-4`; отдельные stage-файлы до этого не удалять. Незавершённое направление оставить blocked с evidence.
...
- [issue](https://github.com/rkfsociety/EvoHime/issues/39)
