# План 97.3 — Model Edit Protocol Registry: строгие patch/search-replace стратегии и repair feedback: IPC, client projection и UI

Статус: этап 3 для [плана 97.0](./97-0-model-edit-protocol-registry.md); после [плана 97.2](./97-2-model-edit-protocol-registry.md).
...
- План 97.2 — runtime vertical slice, recovery и стабильные commands/events.
...
- Proto: добавить additive `ModelEditProtocolRegistryRequest`, `ModelEditProtocolRegistryResponse`, `ModelEditProtocolRegistryEvent` и command/event oneof в `crates/desktop-ipc/proto/evohime.desktop.proto` после проверки свободных tags; сохранить major, replay/resync и bounded frame limits.
...
- UI: создать `desktop/evohime-electron/src/renderer/src/ModelEditProtocolRegistryPanel.tsx` только как projection/action surface; тесты — `desktop/evohime-electron/tests/model_edit_protocol_registry.test.tsx` и protocol/typecheck gates.

### Acceptance-to-projection matrix

- `C08` — Метрики позволяют сравнивать protocol reliability. → показывать состояние только из Core event/evidence, без локального вывода renderer.
