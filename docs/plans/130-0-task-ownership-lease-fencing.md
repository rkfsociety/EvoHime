# План 130.0 — Task Ownership & Lease Fencing

Статус: предложено по [issue #111](https://github.com/rkfsociety/EvoHime/issues/111). Это implementation contract; функционал этим документом не считается реализованным.

## Цель

Реализовать Core-owned контур **Task Ownership & Lease Fencing** как отдельный versioned/revision-aware слой поверх существующей архитектуры EvoHime. Он должен иметь bounded контракт, явного владельца состояния, проверяемые переходы, recovery и metadata-only Electron projection. Renderer не получает authority над runtime, workspace, SQLite, secrets или policy.
...
- [Этап 1 — Core-контракт, schema и storage](./130-1-task-ownership-lease-fencing.md)
- [Этап 2 — runtime-интеграция и recovery](./130-2-task-ownership-lease-fencing.md)
- [Этап 3 — IPC, projection и UI](./130-3-task-ownership-lease-fencing.md)
- [Этап 4 — verification, release evidence и закрытие](./130-4-task-ownership-lease-fencing.md)
...
- [#111 Task Ownership & Lease Fencing](https://github.com/rkfsociety/EvoHime/issues/111)
