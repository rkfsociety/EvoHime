...
- Entrypoint: `crates/evohime-core/src/integration_provider_sdk.rs` + handler в `crates/evohime-core/src/lib.rs`; сервис `IntegrationProviderSdkService` должен выполнять `validate → policy → bounded operation → typed result/event`.
...
- `C06` — Webhook capability можно объявить отдельно от runtime. → нормализовать, дедуплицировать и ограничить вход до enqueue.
