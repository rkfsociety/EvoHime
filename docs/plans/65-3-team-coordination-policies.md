# План 65.3 — Team Coordination Policies: pluggable routing for multi-agent collaboration: IPC, client projection и UI

Статус: этап 3 для [плана 65.0](./65-0-team-coordination-policies.md); после [плана 65.2](./65-2-team-coordination-policies.md).
...
- План 65.2 — runtime vertical slice, recovery и стабильные commands/events.
...
- Proto: добавить additive `TeamCoordinationPoliciesRequest`, `TeamCoordinationPoliciesResponse`, `TeamCoordinationPoliciesEvent` и command/event oneof в `crates/desktop-ipc/proto/evohime.desktop.proto` после проверки свободных tags; сохранить major, replay/resync и bounded frame limits.
...
- UI: создать `desktop/evohime-electron/src/renderer/src/TeamCoordinationPoliciesPanel.tsx` только как projection/action surface; тесты — `desktop/evohime-electron/tests/team_coordination_policies.test.tsx` и protocol/typecheck gates.

### Acceptance-to-projection matrix

- `C06` — UI показывает routing history и причины решений → дать bounded projection и явные Core-checked actions.
