# План 63.3 — Composable Termination Conditions: first-class stop policies for agent and team runs: IPC, client projection и UI

Статус: этап 3 для [плана 63.0](./63-0-composable-termination-conditions.md); после [плана 63.2](./63-2-composable-termination-conditions.md).
...
- План 63.2 — runtime vertical slice, recovery и стабильные commands/events.
...
- Proto: добавить additive `ComposableTerminationConditionsRequest`, `ComposableTerminationConditionsResponse`, `ComposableTerminationConditionsEvent` и command/event oneof в `crates/desktop-ipc/proto/evohime.desktop.proto` после проверки свободных tags; сохранить major, replay/resync и bounded frame limits.
...
- UI: создать `desktop/evohime-electron/src/renderer/src/ComposableTerminationConditionsPanel.tsx` только как projection/action surface; тесты — `desktop/evohime-electron/tests/composable_termination_conditions.test.tsx` и protocol/typecheck gates.

### Acceptance-to-projection matrix

- `C02` — Есть минимум перечисленные built-ins → дать bounded projection и явные Core-checked actions.
- `C06` — UI показывает условия и причину остановки → дать bounded projection и явные Core-checked actions.
