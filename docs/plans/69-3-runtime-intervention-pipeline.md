# План 69.3 — Runtime Intervention Pipeline: Core-owned middleware for agent messages and tool boundaries: IPC, client projection и UI

Статус: этап 3 для [плана 69.0](./69-0-runtime-intervention-pipeline.md); после [плана 69.2](./69-2-runtime-intervention-pipeline.md).
...
- План 69.2 — runtime vertical slice, recovery и стабильные commands/events.
...
- Proto: добавить additive `RuntimeInterventionPipelineRequest`, `RuntimeInterventionPipelineResponse`, `RuntimeInterventionPipelineEvent` и command/event oneof в `crates/desktop-ipc/proto/evohime.desktop.proto` после проверки свободных tags; сохранить major, replay/resync и bounded frame limits.
...
- UI: создать `desktop/evohime-electron/src/renderer/src/RuntimeInterventionPipelinePanel.tsx` только как projection/action surface; тесты — `desktop/evohime-electron/tests/runtime_intervention_pipeline.test.tsx` и protocol/typecheck gates.

### Acceptance-to-projection matrix

- `C08` — UI объясняет intervention пользователю → дать bounded projection и явные Core-checked actions.
