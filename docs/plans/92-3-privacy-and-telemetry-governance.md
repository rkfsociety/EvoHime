# План 92.3 — Privacy & Telemetry Governance: consent, typed analytics events и sensitive-data boundaries: IPC, client projection и UI

Статус: этап 3 для [плана 92.0](./92-0-privacy-and-telemetry-governance.md); после [плана 92.2](./92-2-privacy-and-telemetry-governance.md).
...
- План 92.2 — runtime vertical slice, recovery и стабильные commands/events.
...
- Proto: добавить additive `PrivacyAndTelemetryGovernanceRequest`, `PrivacyAndTelemetryGovernanceResponse`, `PrivacyAndTelemetryGovernanceEvent` и command/event oneof в `crates/desktop-ipc/proto/evohime.desktop.proto` после проверки свободных tags; сохранить major, replay/resync и bounded frame limits.
...
- UI: создать `desktop/evohime-electron/src/renderer/src/PrivacyAndTelemetryGovernancePanel.tsx` только как projection/action surface; тесты — `desktop/evohime-electron/tests/privacy_and_telemetry_governance.test.tsx` и protocol/typecheck gates.

### Acceptance-to-projection matrix

- `C08` — Diagnostics и external telemetry остаются разными механизмами. → показывать состояние только из Core event/evidence, без локального вывода renderer.
