# План 38.3 — Adaptive Tool Catalog: dynamic selection и deferred tool schemas: IPC, client projection и UI

Статус: этап 3 для [плана 38.0](./38-0-adaptive-tool-catalog.md); после [плана 38.2](./38-2-adaptive-tool-catalog.md).
...
- План 38.2 — runtime vertical slice, recovery и стабильные commands/events.
...
- Proto: добавить additive `AdaptiveToolCatalogRequest`, `AdaptiveToolCatalogResponse`, `AdaptiveToolCatalogEvent` и command/event oneof в `crates/desktop-ipc/proto/evohime.desktop.proto` после проверки свободных tags; сохранить major, replay/resync и bounded frame limits.
...
- UI: создать `desktop/evohime-electron/src/renderer/src/AdaptiveToolCatalogPanel.tsx` только как projection/action surface; тесты — `desktop/evohime-electron/tests/adaptive_tool_catalog.test.tsx` и protocol/typecheck gates.

### Acceptance-to-projection matrix

- `C01` — Есть compact tool catalog projection. → показывать состояние только из Core event/evidence, без локального вывода renderer.
