# План 72.4 — Core Topic/Subscription Event Bus: typed pub/sub routing for agent runtime: verification, release-evidence и закрытие

Статус: этап 4 для [плана 72.0](./72-0-core-topic-subscription-event-bus.md); после [плана 72.3](./72-3-core-topic-subscription-event-bus.md).
...
- План 72.3 — Core, IPC и client projection.
...
- После закрытия всего направления и переноса подтверждённого контракта удалить комплект `72-0` … `72-4`; отдельные stage-файлы до этого не удалять. Незавершённое направление оставить blocked с evidence.
...
- [issue](https://github.com/rkfsociety/EvoHime/issues/52)
