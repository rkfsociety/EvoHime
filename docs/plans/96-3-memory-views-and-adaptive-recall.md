...
- Proto: добавить additive `MemoryViewsAndAdaptiveRecallRequest`, `MemoryViewsAndAdaptiveRecallResponse`, `MemoryViewsAndAdaptiveRecallEvent` и command/event oneof в `crates/desktop-ipc/proto/evohime.desktop.proto` после проверки свободных tags; сохранить major, replay/resync и bounded frame limits.
...
- UI: создать `desktop/evohime-electron/src/renderer/src/MemoryViewsAndAdaptiveRecallPanel.tsx` только как projection/action surface; тесты — `desktop/evohime-electron/tests/memory_views_and_adaptive_recall.test.tsx` и protocol/typecheck gates.

### Acceptance-to-projection matrix

- `C08` — UI позволяет инспектировать scope/view и причины retrieval. → дать bounded projection и явные Core-checked actions.
