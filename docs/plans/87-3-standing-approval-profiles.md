# План 87.3 — Standing Approval Profiles: scoped auto-approval по action class, path, risk и времени: IPC, client projection и UI

Статус: этап 3 для [плана 87.0](./87-0-standing-approval-profiles.md); после [плана 87.2](./87-2-standing-approval-profiles.md).
...
- План 87.2 — runtime vertical slice, recovery и стабильные commands/events.
...
- Proto: добавить additive `StandingApprovalProfilesRequest`, `StandingApprovalProfilesResponse`, `StandingApprovalProfilesEvent` и command/event oneof в `crates/desktop-ipc/proto/evohime.desktop.proto` после проверки свободных tags; сохранить major, replay/resync и bounded frame limits.
...
- UI: создать `desktop/evohime-electron/src/renderer/src/StandingApprovalProfilesPanel.tsx` только как projection/action surface; тесты — `desktop/evohime-electron/tests/standing_approval_profiles.test.tsx` и protocol/typecheck gates.

### Acceptance-to-projection matrix

- `C02` — Rules scoped по subject, action class, resource и risk. → дать bounded projection и явные Core-checked actions.
- `C06` — Пользователь может понять, почему action прошёл без prompt. → дать bounded projection и явные Core-checked actions.
