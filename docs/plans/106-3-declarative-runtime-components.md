# План 106.3 — Declarative Runtime Components: versioned component config, provider registry и safe rehydration: IPC, client projection и UI

Статус: этап 3 для [плана 106.0](./106-0-declarative-runtime-components.md); после [плана 106.2](./106-2-declarative-runtime-components.md).
...
- План 106.2 — runtime vertical slice, recovery и стабильные commands/events.
...
- Proto: добавить additive `DeclarativeRuntimeComponentsRequest`, `DeclarativeRuntimeComponentsResponse`, `DeclarativeRuntimeComponentsEvent` и command/event oneof в `crates/desktop-ipc/proto/evohime.desktop.proto` после проверки свободных tags; сохранить major, replay/resync и bounded frame limits.
...
- UI: создать `desktop/evohime-electron/src/renderer/src/DeclarativeRuntimeComponentsPanel.tsx` только как projection/action surface; тесты — `desktop/evohime-electron/tests/declarative_runtime_components.test.tsx` и protocol/typecheck gates.

### Acceptance-to-projection matrix

- `C07` — Config hashes пригодны для provenance/evals. → показывать состояние только из Core event/evidence, без локального вывода renderer.
