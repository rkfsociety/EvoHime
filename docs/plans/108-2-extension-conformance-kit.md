...
- Entrypoint: `crates/evohime-core/src/extension_conformance_kit.rs` + handler в `crates/evohime-core/src/lib.rs`; сервис `ExtensionConformanceKitService` должен выполнять `validate → policy → bounded operation → typed result/event`.
...
- Тесты: `crates/evohime-core/tests/extension_conformance_kit_recovery.rs` — timeout/cancel, duplicate, stale version/lease, crash до/после dispatch, restart и optional-unavailable.

### Acceptance-to-runtime matrix

- `C03` — Instance isolation проверяется отдельно от provider identity. → разрешить Core snapshot, проверить capability/locality и закрепить его на run.
- `C05` — Disabled path гарантированно не создаёт runtime side effects. → провести через typed outcome, timeout, cancellation и idempotency.
