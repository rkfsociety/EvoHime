# План 135.2 — Code Review Lane: runtime-интеграция и recovery

Статус: этап 2 для [плана 135.0](./135-0-code-review-lane.md); issue: [#116](https://github.com/rkfsociety/EvoHime/issues/116).

## Зависимости

### Блокирующие

- План 135.0 и предыдущий этап этого направления.
...
Подключить Code Review Lane к Core runtime через явные commands/state transitions. Реализовать cancellation, timeout, approval/policy checks, optimistic concurrency, crash recovery и last-known-safe behavior. Unknown, stale, denied, conflict и partial failure не превращаются в успешный результат; активный run pin-ит immutable revision.
