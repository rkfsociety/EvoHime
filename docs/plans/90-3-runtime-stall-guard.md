# План 90.3 — Runtime Stall Guard: static blocking-I/O detection и CI runtime anchors: IPC, client projection и UI

Статус: этап 3 для [плана 90.0](./90-0-runtime-stall-guard.md); после [плана 90.2](./90-2-runtime-stall-guard.md).
...
- План 90.2 — runtime vertical slice, recovery и стабильные commands/events.
...
- Proto: добавить additive `RuntimeStallGuardRequest`, `RuntimeStallGuardResponse`, `RuntimeStallGuardEvent` и command/event oneof в `crates/desktop-ipc/proto/evohime.desktop.proto` после проверки свободных tags; сохранить major, replay/resync и bounded frame limits.
...
- UI: создать `desktop/evohime-electron/src/renderer/src/RuntimeStallGuardPanel.tsx` только как projection/action surface; тесты — `desktop/evohime-electron/tests/runtime_stall_guard.test.tsx` и protocol/typecheck gates.

### Acceptance-to-projection matrix

- `C07` — Diagnostics не содержат sensitive payload. → показывать только redacted projection и provenance без raw payload.
