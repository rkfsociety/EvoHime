# План 64.3 — Workspace Bootstrap Manifest: безопасная подготовка project environment перед agent run: IPC, client projection и UI

Статус: этап 3 для [плана 64.0](./64-0-workspace-bootstrap-manifest.md); после [плана 64.2](./64-2-workspace-bootstrap-manifest.md).
...
- План 64.2 — runtime vertical slice, recovery и стабильные commands/events.
...
- Proto: добавить additive `WorkspaceBootstrapManifestRequest`, `WorkspaceBootstrapManifestResponse`, `WorkspaceBootstrapManifestEvent` и command/event oneof в `crates/desktop-ipc/proto/evohime.desktop.proto` после проверки свободных tags; сохранить major, replay/resync и bounded frame limits.
...
- UI: создать `desktop/evohime-electron/src/renderer/src/WorkspaceBootstrapManifestPanel.tsx` только как projection/action surface; тесты — `desktop/evohime-electron/tests/workspace_bootstrap_manifest.test.tsx` и protocol/typecheck gates.

### Acceptance-to-projection matrix

- `C08` — Agent получает компактный environment summary, а не raw scripts. → показывать состояние только из Core event/evidence, без локального вывода renderer.
