# План 75.3 — Typed Context References: адресные @refs на файлы, diff, diagnostics, terminal и artifacts: IPC, client projection и UI

Статус: этап 3 для [плана 75.0](./75-0-typed-context-references.md); после [плана 75.2](./75-2-typed-context-references.md).
...
- План 75.2 — runtime vertical slice, recovery и стабильные commands/events.
...
- Proto: добавить additive `TypedContextReferencesRequest`, `TypedContextReferencesResponse`, `TypedContextReferencesEvent` и command/event oneof в `crates/desktop-ipc/proto/evohime.desktop.proto` после проверки свободных tags; сохранить major, replay/resync и bounded frame limits.
...
- UI: создать `desktop/evohime-electron/src/renderer/src/TypedContextReferencesPanel.tsx` только как projection/action surface; тесты — `desktop/evohime-electron/tests/typed_context_references.test.tsx` и protocol/typecheck gates.

### Acceptance-to-projection matrix

- `C06` — `@` autocomplete работает через typed resources. → дать bounded projection и явные Core-checked actions.
- `C08` — Model-call provenance содержит использованные refs/projections. → показывать состояние только из Core event/evidence, без локального вывода renderer.
