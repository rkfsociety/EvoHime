# План 134.2 — Host Resource Telemetry & Pressure Guard: runtime-интеграция и recovery

Статус: этап 2 для [плана 134.0](./134-0-host-resource-telemetry-pressure-guard.md); issue: [#115](https://github.com/rkfsociety/EvoHime/issues/115).
...
Подключить Host Resource Telemetry & Pressure Guard к Core runtime через явные commands/state transitions. Реализовать cancellation, timeout, approval/policy checks, optimistic concurrency, crash recovery и last-known-safe behavior. Unknown, stale, denied, conflict и partial failure не превращаются в успешный результат; активный run pin-ит immutable revision.
