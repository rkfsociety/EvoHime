# План 62.3 — Team Resource Budget: shared cost envelope, per-role allocations и reserved verification budget: IPC, client projection и UI

Статус: этап 3 для [плана 62.0](./62-0-team-resource-budget.md); после [плана 62.2](./62-2-team-resource-budget.md).
...
- План 62.2 — runtime vertical slice, recovery и стабильные commands/events.
...
- Proto: добавить additive `TeamResourceBudgetRequest`, `TeamResourceBudgetResponse`, `TeamResourceBudgetEvent` и command/event oneof в `crates/desktop-ipc/proto/evohime.desktop.proto` после проверки свободных tags; сохранить major, replay/resync и bounded frame limits.
...
- UI: создать `desktop/evohime-electron/src/renderer/src/TeamResourceBudgetPanel.tsx` только как projection/action surface; тесты — `desktop/evohime-electron/tests/team_resource_budget.test.tsx` и protocol/typecheck gates.

### Acceptance-to-projection matrix

- `C08` — Budget state durable/recoverable. → показывать состояние только из Core event/evidence, без локального вывода renderer.
