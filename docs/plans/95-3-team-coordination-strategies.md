# План 95.3 — Team Coordination Strategies: pluggable selector, round-robin, swarm и graph routing: IPC, client projection и UI

Статус: этап 3 для [плана 95.0](./95-0-team-coordination-strategies.md); после [плана 95.2](./95-2-team-coordination-strategies.md).
...
- План 95.2 — runtime vertical slice, recovery и стабильные commands/events.
...
- Proto: добавить additive `TeamCoordinationStrategiesRequest`, `TeamCoordinationStrategiesResponse`, `TeamCoordinationStrategiesEvent` и command/event oneof в `crates/desktop-ipc/proto/evohime.desktop.proto` после проверки свободных tags; сохранить major, replay/resync и bounded frame limits.
...
- UI: создать `desktop/evohime-electron/src/renderer/src/TeamCoordinationStrategiesPanel.tsx` только как projection/action surface; тесты — `desktop/evohime-electron/tests/team_coordination_strategies.test.tsx` и protocol/typecheck gates.

### Acceptance-to-projection matrix

- `C07` — Strategy snapshot durable и pinned к TeamSession. → показывать состояние только из Core event/evidence, без локального вывода renderer.
