...
- Entrypoint: `crates/evohime-core/src/team_coordination_policies.rs` + handler в `crates/evohime-core/src/lib.rs`; сервис `TeamCoordinationPoliciesService` должен выполнять `validate → policy → bounded operation → typed result/event`.
...
- Тесты: `crates/evohime-core/tests/team_coordination_policies_recovery.rs` — timeout/cancel, duplicate, stale version/lease, crash до/после dispatch, restart и optional-unavailable.

### Acceptance-to-runtime matrix

- `C02` — RoundRobin, Selector, DirectedHandoff и RoleRouter доступны → разрешить Core snapshot, проверить capability/locality и закрепить его на run.
- `C03` — Selector output всегда валидируется Core → разрешить Core snapshot, проверить capability/locality и закрепить его на run.
- `C04` — Policy state persist/recover поддержан → журналировать переходы и восстановление через replay/reconciliation.
- `C05` — Есть loop/repeated-speaker guards → провести через typed outcome, timeout, cancellation и idempotency.
- `C07` — Existing child grants/approvals не обходятся → повторить authorization непосредственно перед dispatch/effect.
