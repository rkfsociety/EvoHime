# План 142.2 — Agent Program Optimizer: runtime-интеграция и recovery

Статус: этап 2 для [плана 142.0](./142-0-agent-program-optimizer.md); issue: [#123](https://github.com/rkfsociety/EvoHime/issues/123).
...
Подключить Agent Program Optimizer к Core runtime через явные commands/state transitions. Реализовать cancellation, timeout, approval/policy checks, optimistic concurrency, crash recovery и last-known-safe behavior. Unknown, stale, denied, conflict и partial failure не превращаются в успешный результат; активный run pin-ит immutable revision.
