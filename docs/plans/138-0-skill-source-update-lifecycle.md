# План 138.0 — Skill Source & Update Lifecycle

Статус: предложено по [issue #119](https://github.com/rkfsociety/EvoHime/issues/119). Это implementation contract; функционал этим документом не считается реализованным.

## Цель

Реализовать Core-owned контур **Skill Source & Update Lifecycle** как отдельный versioned/revision-aware слой поверх существующей архитектуры EvoHime. Он должен иметь bounded контракт, явного владельца состояния, проверяемые переходы, recovery и metadata-only Electron projection. Renderer не получает authority над runtime, workspace, SQLite, secrets или policy.
...
- [Этап 1 — Core-контракт, schema и storage](./138-1-skill-source-update-lifecycle.md)
- [Этап 2 — runtime-интеграция и recovery](./138-2-skill-source-update-lifecycle.md)
- [Этап 3 — IPC, projection и UI](./138-3-skill-source-update-lifecycle.md)
- [Этап 4 — verification, release evidence и закрытие](./138-4-skill-source-update-lifecycle.md)
...
- [#119 Skill Source & Update Lifecycle](https://github.com/rkfsociety/EvoHime/issues/119)
