...
- Entrypoint: `crates/evohime-core/src/agent_benchmark_matrix.rs` + handler в `crates/evohime-core/src/lib.rs`; сервис `AgentBenchmarkMatrixService` должен выполнять `validate → policy → bounded operation → typed result/event`.
...
- Тесты: `crates/evohime-core/tests/agent_benchmark_matrix_recovery.rs` — timeout/cancel, duplicate, stale version/lease, crash до/после dispatch, restart и optional-unavailable.

### Acceptance-to-runtime matrix

- `C03` — Можно запускать matrix по model + agent profiles. → разрешить Core snapshot, проверить capability/locality и закрепить его на run.
- `C08` — Runs изолированы и parallelism bounded. → провести через typed outcome, timeout, cancellation и idempotency.
