# План 56.2 — Artifact Handoff Registry: typed deliverables, lineage и freshness для multi-agent работы: runtime-интеграция и recovery

Статус: этап 2 для [плана 56.0](./56-0-artifact-handoff-registry.md); после [плана 56.1](./56-1-artifact-handoff-registry.md).

## Цель

Провести «Artifact Handoff Registry: typed deliverables, lineage и freshness для multi-agent работы» через Core runtime: validation -> authorization -> bounded operation -> typed result/event -> recovery.
...
- Entrypoint: `crates/evohime-core/src/artifact_handoff_registry.rs` + handler в `crates/evohime-core/src/lib.rs`; сервис `ArtifactHandoffRegistryService` должен выполнять `validate → policy → bounded operation → typed result/event`.
...
- Тесты: `crates/evohime-core/tests/artifact_handoff_registry_recovery.rs` — timeout/cancel, duplicate, stale version/lease, crash до/после dispatch, restart и optional-unavailable.

### Acceptance-to-runtime matrix

- `C01` — ArtifactStore дополнен semantic ProjectArtifact registry. → проверить exact revision/hash перед mutation и сохранить observed evidence.
