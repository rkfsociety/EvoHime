# План 99.3 — Composable Termination Conditions: typed stop algebra для agent/team runs: IPC, client projection и UI

Статус: этап 3 для [плана 99.0](./99-0-composable-termination-conditions-follow-up.md); после [плана 99.2](./99-2-composable-termination-conditions-follow-up.md).
...
- План 99.2 — runtime vertical slice, recovery и стабильные commands/events.
...
- Proto: добавить additive `ComposableTerminationConditionsFollowUpRequest`, `ComposableTerminationConditionsFollowUpResponse`, `ComposableTerminationConditionsFollowUpEvent` и command/event oneof в `crates/desktop-ipc/proto/evohime.desktop.proto` после проверки свободных tags; сохранить major, replay/resync и bounded frame limits.
...
- UI: создать `desktop/evohime-electron/src/renderer/src/ComposableTerminationConditionsFollowUpPanel.tsx` только как projection/action surface; тесты — `desktop/evohime-electron/tests/composable_termination_conditions_follow_up.test.tsx` и protocol/typecheck gates.

### Acceptance-to-projection matrix

- `C07` — UI показывает reason/counters. → дать bounded projection и явные Core-checked actions.
