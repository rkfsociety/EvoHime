# План 150.0 — Minimal Change Policy

Статус: предложено по [issue #130](https://github.com/rkfsociety/EvoHime/issues/130). Это implementation contract; функционал этим документом не считается реализованным.

## Цель

Реализовать Core-owned контур **Minimal Change Policy** как versioned/revision-aware capability поверх существующей архитектуры EvoHime. Контур должен иметь bounded contract, одного владельца состояния, безопасные transitions, recovery и metadata-only Electron projection. Renderer не получает authority над runtime, workspace, SQLite, secrets или policy.
...
- [Этап 1 — Core-контракт, schema и storage](./150-1-minimal-change-policy.md)
- [Этап 2 — runtime-интеграция и recovery](./150-2-minimal-change-policy.md)
- [Этап 3 — IPC, projection и UI](./150-3-minimal-change-policy.md)
- [Этап 4 — verification, release evidence и закрытие](./150-4-minimal-change-policy.md)
...
- [#130 Minimal Change Policy](https://github.com/rkfsociety/EvoHime/issues/130)
