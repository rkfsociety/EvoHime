# План 93.4 — Headless CLI Client: NDJSON streaming, one-shot runs и automation поверх существующего Core: verification, release-evidence и закрытие

Статус: этап 4 для [плана 93.0](./93-0-headless-cli-client.md); после [плана 93.3](./93-3-headless-cli-client.md).
...
- План 93.3 — Core, IPC и client projection.
...
- После закрытия всего направления и переноса подтверждённого контракта удалить комплект `93-0` … `93-4`; отдельные stage-файлы до этого не удалять. Незавершённое направление оставить blocked с evidence.
...
- [issue](https://github.com/rkfsociety/EvoHime/issues/73)
