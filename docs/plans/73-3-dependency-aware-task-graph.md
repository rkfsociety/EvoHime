# План 73.3 — Dependency-Aware Task Graph: selective replanning и downstream invalidation: IPC, client projection и UI

Статус: этап 3 для [плана 73.0](./73-0-dependency-aware-task-graph.md); после [плана 73.2](./73-2-dependency-aware-task-graph.md).
...
- План 73.2 — runtime vertical slice, recovery и стабильные commands/events.
...
- Proto: добавить additive `DependencyAwareTaskGraphRequest`, `DependencyAwareTaskGraphResponse`, `DependencyAwareTaskGraphEvent` и command/event oneof в `crates/desktop-ipc/proto/evohime.desktop.proto` после проверки свободных tags; сохранить major, replay/resync и bounded frame limits.
...
- UI: создать `desktop/evohime-electron/src/renderer/src/DependencyAwareTaskGraphPanel.tsx` только как projection/action surface; тесты — `desktop/evohime-electron/tests/dependency_aware_task_graph.test.tsx` и protocol/typecheck gates.

### Acceptance-to-projection matrix

- `C08` — Team SOP может использовать тот же task substrate. → показывать состояние только из Core event/evidence, без локального вывода renderer.
