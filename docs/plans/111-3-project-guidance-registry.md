# План 111.3 — Project Guidance Registry: scoped coding conventions и read-only instruction layers: IPC, client projection и UI

Статус: этап 3 для [плана 111.0](./111-0-project-guidance-registry.md); после [плана 111.2](./111-2-project-guidance-registry.md).
...
- План 111.2 — runtime vertical slice, recovery и стабильные commands/events.
...
- Proto: добавить additive `ProjectGuidanceRegistryRequest`, `ProjectGuidanceRegistryResponse`, `ProjectGuidanceRegistryEvent` и command/event oneof в `crates/desktop-ipc/proto/evohime.desktop.proto` после проверки свободных tags; сохранить major, replay/resync и bounded frame limits.
...
- UI: создать `desktop/evohime-electron/src/renderer/src/ProjectGuidanceRegistryPanel.tsx` только как projection/action surface; тесты — `desktop/evohime-electron/tests/project_guidance_registry.test.tsx` и protocol/typecheck gates.

### Acceptance-to-projection matrix

- `C01` — Есть ProjectGuidanceDocument registry и discovery adapters. → дать bounded projection и явные Core-checked actions.
- `C03` — Effective guidance resolution deterministic и hierarchical. → дать bounded projection и явные Core-checked actions.
- `C05` — Guidance не может повышать security authority. → дать bounded projection и явные Core-checked actions.
- `C06` — Context projection bounded и target-aware. → показывать состояние только из Core event/evidence, без локального вывода renderer.
- `C07` — Prompt Cache Planner может стабильно cache-ить guidance projection. → дать bounded projection и явные Core-checked actions.
- `C08` — UI показывает sources/scope/conflicts/revision. → дать bounded projection и явные Core-checked actions.
