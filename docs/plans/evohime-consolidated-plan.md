...
- [x] Реализовать ручное создание, decomposition, complexity analysis, dependency graph и `next_ready`.
- [x] В WinUI показать Projects/Tasks: ready, blocked, done, граф, карточку, подзадачи, acceptance criteria и event history.
- [x] Добавить действия Следующая задача, Разблокировать, Отложить, Запустить, Остановить, Повторить и Отметить готовой; Core подтверждает каждый переход.
...
- [x] Добавить IPC `GetBuildPolicy`/`SaveBuildPolicy` и WinUI policy panel для allowed paths, operations, budgets, timeout и risk с optimistic version check.
