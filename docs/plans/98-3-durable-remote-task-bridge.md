...
- Proto: добавить additive `DurableRemoteTaskBridgeRequest`, `DurableRemoteTaskBridgeResponse`, `DurableRemoteTaskBridgeEvent` и command/event oneof в `crates/desktop-ipc/proto/evohime.desktop.proto` после проверки свободных tags; сохранить major, replay/resync и bounded frame limits.
...
- UI: создать `desktop/evohime-electron/src/renderer/src/DurableRemoteTaskBridgePanel.tsx` только как projection/action surface; тесты — `desktop/evohime-electron/tests/durable_remote_task_bridge.test.tsx` и protocol/typecheck gates.

### Acceptance-to-projection matrix

- `C08` — Unknown outcomes не вызывают blind retry side effects. → показывать состояние только из Core event/evidence, без локального вывода renderer.
