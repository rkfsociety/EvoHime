# План 51.2 — Causal Collaboration Bus: typed pub/sub для team agents поверх child mailbox: runtime-интеграция и recovery

Статус: этап 2 для [плана 51.0](./51-0-causal-collaboration-bus.md); после [плана 51.1](./51-1-causal-collaboration-bus.md).

## Цель

Провести «Causal Collaboration Bus: typed pub/sub для team agents поверх child mailbox» через Core runtime: validation -> authorization -> bounded operation -> typed result/event -> recovery.
...
- Entrypoint: `crates/evohime-core/src/causal_collaboration_bus.rs` + handler в `crates/evohime-core/src/lib.rs`; сервис `CausalCollaborationBusService` должен выполнять `validate → policy → bounded operation → typed result/event`.
...
- `C02` — Sender identity и routing Core-owned. → провести через typed outcome, timeout, cancellation и idempotency.
- `C03` — Есть protocol-scoped subscriptions и peer routes. → нормализовать, дедуплицировать и ограничить вход до enqueue.
- `C08` — Artifact authority не расширяется фактом подписки. → проверить exact revision/hash перед mutation и сохранить observed evidence.
