# План 45.3 — External Coding Agent Adapter: подключение Codex/Claude/Gemini-подобных executors через typed protocol: IPC, client projection и UI

Статус: этап 3 для [плана 45.0](./45-0-external-coding-agent-adapter.md); после [плана 45.2](./45-2-external-coding-agent-adapter.md).
...
- План 45.2 — runtime vertical slice, recovery и стабильные commands/events.
...
- Proto: добавить additive `ExternalCodingAgentAdapterRequest`, `ExternalCodingAgentAdapterResponse`, `ExternalCodingAgentAdapterEvent` и command/event oneof в `crates/desktop-ipc/proto/evohime.desktop.proto` после проверки свободных tags; сохранить major, replay/resync и bounded frame limits.
...
- UI: создать `desktop/evohime-electron/src/renderer/src/ExternalCodingAgentAdapterPanel.tsx` только как projection/action surface; тесты — `desktop/evohime-electron/tests/external_coding_agent_adapter.test.tsx` и protocol/typecheck gates.

### Acceptance-to-projection matrix

- `C05` — Conversation фиксирует immutable agent snapshot. → дать bounded projection и явные Core-checked actions.
- `C07` — Параллельные conversations изолируют mutable agent state где возможно. → дать bounded projection и явные Core-checked actions.
- `C08` — UI различает уровень Core control над external executor. → дать bounded projection и явные Core-checked actions.
