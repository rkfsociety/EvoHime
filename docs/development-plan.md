...
Статус: исполняемый план текущего desktop-цикла. Миграция оболочки описана в [`plans/0-electron-shell-migration.md`](plans/0-electron-shell-migration.md). Для фактического состояния используйте [`current-state.md`](current-state.md), для долгосрочных направлений — [`roadmap.md`](roadmap.md).
...
2. Desktop shell — Electron task workspace, replay/reconnect, tray and notifications. Миграция запланирована; текущая WinUI оболочка — временный legacy runtime.
...
6. Release cleanup — единый installer, retention релизов, диагностика и чистый Windows CI. Основная часть завершена; Electron migration — release prerequisite.

## Текущий статус перехода на Electron

Legacy web UI, browser launcher, HTTP server и PostgreSQL persistence удалены. Rust Core + SQLite + named-pipe IPC сохраняются. Пользовательская оболочка переводится с WinUI 3 на Electron; до завершения migration acceptance WinUI считается временным compatibility runtime.
...
| Git | ✅ Завершены bounded read-only status/diff через Core IPC; Electron page входит в migration | `ea3f065`, `28d850d` |
...
| Credentials, расширенный backup/restore и crash recovery UI | ✅ Реализован Core/UI MVP | текущий Core/legacy UI-срез; `cargo test` и IPC tests |
