...
Статус: текущая утверждённая архитектура продукта. Фактическое состояние реализации см. в [`current-state.md`](current-state.md); незавершённые работы — в [`plans/`](plans/).
