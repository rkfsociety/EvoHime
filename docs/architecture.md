...
- installer-обновление берёт целевой commit из `EvoHime-Setup.json` постоянного release, сверяет его branch, размер и SHA-256 с EXE, а при `requireGreenCommit` читает его точный CI verdict. Новый commit в `main` не отменяет уже опубликованный release; pending, failure или unknown CI manifest commit откладывает обновление. Для dev-only локальной пересборки клиент по-прежнему берёт самый свежий зелёный commit в окне `greenCommitDepth` (по умолчанию 10); если checks не читаются, обновление откладывается. `requireGreenCommit` отключает CI gate;
