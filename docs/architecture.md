...
- подмену выполняет `evohime-transaction.exe --apply-staging`: он копирует себя во временный каталог, дожидается выхода оболочки, делает полный backup установки, переносит staging и при любой ошибке восстанавливает прежнюю установку. Незавершённая транзакция откатывается при следующем запуске.
