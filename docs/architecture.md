...
## Надёжная запись событий и границы payload

Обязательные события Core не используют `broadcast` как транспорт записи.
`TaskCoordinator` принимает их через bounded `tokio::mpsc`-очередь с ожиданием
свободного места; только после извлечения события из этой очереди и успешной
записи в durable journal оно отправляется в отдельный `broadcast` для
подписчиков. Потеря уведомления у медленного подписчика (`Lagged`) поэтому не
может остановить запись или скрыть событие от журнала. SQL-операция записи
выполняется в `spawn_blocking`, а backup/restore сохраняют общий database mutex
и также не выполняют синхронный SQLite на Tokio worker.

Для события, привязанного к разговору, основной journal row, все записи
conversation history и соответствующие `conversation.event` delivery rows
вставляются в одной SQLite-транзакции с одним commit. Ошибка между вставками
откатывает весь bundle; после перезапуска не возникает частичной проекции.

Ошибки journal и audit не отбрасываются: Core сохраняет последнюю ошибку в
состоянии и публикует `EventPersistenceFailed`. UI может показать этот
диагностический статус, а выполнение задачи не получает ложный успешный
результат. Для проверки есть stress-тест с очередью размера 1, быстрым потоком
событий и проверкой durable финального состояния.

В Electron bridge ограничения payload задаются явно в байтах UTF-8 для каждого
крупного поля. Общий helper коротких bounded-строк не используется для
полей, которым разрешены payload-ы от 64 KiB до 4 MiB; границы и Unicode
проверяются отдельными тестами. Доменные фасады `evohime-local-storage` вводятся
постепенно: memory bounded context уже экспортируется явно, остальные legacy
store-модули мигрируются без разрыва текущего Core API.
...
`#343b4d`, основной акцент `#9b8cff`, вторичный акцент `#7595ff`. Композер
оформлен как компактная тёмная панель без свечения: строка контекста содержит
проект, ветку и состояние Git, режим доступа, провайдера, модель и индикатор
контекста. При нехватке ширины элементы строки переносятся, не сжимаясь и не
обрезая индикатор контекста; ветка и состояние Git остаются на экране, пока
выбран workspace. Поле задачи содержит прикрепление файлов, переключатель
веб-поиска и отправку. Клавиатурные подсказки и состояние Core находятся в
тонком футере.
Прикрепление открывает выбор нескольких файлов текущего workspace и передаёт в
запрос только их относительные пути через существующий preload bridge. Режим
доступа выбирается непосредственно в строке контекста и применяется через
`core.setPermissionMode`; веб-поиск включает явную инструкцию для следующего
запроса. Эти действия не
создают новый IPC-контракт и не переносят authority из Core. Sidebar
...
привязанным к выбранному workspace. Временные вложения и флаг веб-поиска живут
в composer renderer до отправки; IPC-контракт, Core-owned state, approvals,
recovery, updates и настройки компонентов не меняются.
...
- провайдер `ollama` использует loopback-only OpenAI-compatible chat endpoint; `ModelCatalogRequest` возвращает установленные модели и Core-owned рекомендации по CPU, RAM, свободному диску и VRAM, а скачивание проходит через redacted `ollama_pull` projection;
- установка самого Ollama остаётся main-owned desktop-операцией: только allowlisted `https://ollama.com/download/OllamaSetup.exe` с разрешёнными официальными redirect-hosts, потоковая запись bounded installer во временный каталог, запуск без shell и последующая проверка `http://127.0.0.1:11434/api/version`; при `net::ERR_BLOCKED_BY_CLIENT` Chromium-загрузка повторяется через Node transport, а все URL/размер/header-проверки сохраняются; renderer получает только typed status/progress, а найденная Ollama не переустанавливается автоматически;
...
**Оценка токенов.** Estimator версионируется и обязан быть консервативным: занижение считается дефектом. При недоступности основного используется fallback-estimator (`ceil(utf8_bytes / 2) + 16`) с порогами профиля, масштабированными на 0.70; при недоступности обоих сборка завершается отказом, а не оценкой по умолчанию. Оценка контента и tool schema кэшируется bounded LRU по `content_hash`, форме данных и версиям tokenizer, нормализатора и chat-template, поэтому смена любой из них не даёт стухший кэш-хит.
...
| `chats.json` | чаты, привязанные к workspace, и отправленные промпты | 100 чатов на workspace, 500 сообщений на чат, 4096 символов на промпт; кешируется в main, записи последовательно выполняются асинхронно через временный файл и атомарный `rename` |
...
Отсутствующий файл означает пустое состояние. Ошибка чтения или повреждённый
JSON не подменяются пустым состоянием: операция получает typed failure, а
исходный повреждённый файл сохраняется рядом как recovery-копия. Это не даёт
последующей записи затереть данные, которые ещё можно восстановить.
...
(LiteRouter, OpenAI Compatible, OpenAI Responses или Ollama) либо `codex_cli`. Для API
выбор активного профиля перезапускает supervisor/Core с его изолированными
credentials; выбор уже настроенного профиля в настройках автоматически
сохраняется и сразу применяет профиль через этот перезапуск; каталог моделей
рядом с композером запрашивается у выбранного провайдера. Выбор Codex передаёт отдельный IPC intent, Core принимает Codex только
при явном этом режиме и запускает bounded `codex exec` в каноническом workspace.
Electron распознаёт штатную WinGet-установку в `%LOCALAPPDATA%\OpenAI\Codex\bin\<version>\codex.exe`,
фиксированный legacy-путь и executable из `PATH`; ответ WinGet об отсутствии
более новой версии не считается ошибкой, если CLI уже найден.
...
module release каждого компонента, его версию, artifact, размер, SHA-256,
зависимости, restart class и минимальную версию updater. Поэтому updater не
смешивает независимые последние версии; публикация этого манифеста не требует
пересборки installer. Если установлен updater ниже требования, сначала
скачивается и атомарно заменяется только updater, затем новый updater получает
...
- production-обновление использует `compatibility` manifest и конкретные независимые module releases `module-<module>-v<semver>`; перед скачиванием проверяются identity набора, зависимости, размер и SHA-256 каждого артефакта. Решение об обновлении принимается по этому набору, а не по несогласованным «последним» версиям или commit. Commit-based source rebuild сохранён только как явно включаемый dev-only режим;
...
- установленный ярлык и post-install запуск вызывают `updater\EvoHimeUpdater.exe` с аргументом `--evohime-updater`. Это самостоятельный Electron package внутри модуля updater: он не поднимает Core, supervisor, трей или обычное окно. Проверку, скачивание и применение он передаёт невидимому Rust worker `evohime-updater.exe`, внутри которого статически доступен transaction/recovery engine; видимое окно остаётся открытым и получает статус с прогрессом на протяжении проверки, скачивания и верификации. Только после отдельного `requires_exit` от worker окно кратко закрывается для освобождения файлов перед транзакцией или self-update, затем новый updater снова открывает окно либо запускает `EvoHime.exe`. Перед запуском shell gate дожидается проверки, а при наличии модульных релизов автоматически передаёт их в apply и не предоставляет режима запуска без обновления. Main-процесс дополнительно проводит тот же невидимый safety gate до старта Core и supervisor, но renderer shell больше не рисует legacy full-window `UpdateGate`; внутри приложения остаётся только компактная иконка для фонового обновления уже работающего shell;
...
Повторная детерминированная оценка snapshot маршрутизации использует bounded
LRU-кэш с ключом из запроса, snapshot, поколения health overlay, каталога,
попытки и точного времени оценки; обновление overlay автоматически меняет
ключ и не допускает устаревшего выбора.
...
Builtin manifests материализуются один раз при регистрации инструмента и
переиспользуются registry-кэшем для loadout, preflight и IPC-выдачи; изменение
регистрации заменяет соответствующую запись кэша.
...
Для renderer event-поток обрабатывается курсором новых элементов: страницы и
вложенные conversation events индексируются по стабильным ключам и ID, поэтому
