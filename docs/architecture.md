...
оформлен как широкая glass/glow-панель с вложенным полем задачи, иконками
действий «Файлы», «Инструменты» и «Веб-поиск», а также нижним рядом выбора
проекта, режима доступа, провайдера, модели и индикатора контекста. «Файлы»
открывает выбор нескольких файлов текущего workspace и передаёт в запрос только
их относительные пути через существующий preload bridge; «Инструменты» раскрывает
существующий выбор режима доступа и применяет его через `core.setPermissionMode`;
«Веб-поиск» включает явную инструкцию для следующего запроса. Эти действия не
