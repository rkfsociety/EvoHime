...
ветке не блокируют уже проверенный release. Локальная пересборка остаётся
dev-only fallback и сравнивает коммит своей сборки с вершиной отслеживаемой
ветки.
...
- production-обновление использует независимые module releases `module-<module>-v<semver>` и проверяет manifest, размер и SHA-256 каждого артефакта; решение об обновлении принимается по версии модуля и его зависимостям, а не по commit. Commit-based source rebuild сохранён только как явно включаемый dev-only режим;
