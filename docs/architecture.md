...
- Модель извлекателя задаётся `EVOHIME_MEMORY_EXTRACTION_MODEL`; при отсутствии используется модель маршрута. Проверка файловой evidence сверяет content hash; tool/API-валидация возвращает `unknown` до появления Local Agentic RAG (`docs/plans/02-0-local-agentic-rag.md`), поэтому такие записи остаются pending, а не подтверждаются вслепую.
