...
- **Корень доверия к рантайму — хеш, а release signing — дополнительная проверка.** Единственный локальный корень доверия — SHA-256 каждого файла из `listener-runtime.json`, полученного тем же релизным каналом GitHub, что и установщик продукта (по образцу `release-installer.ts`, с теми же потолками размера и таймаутом). Для release installer и shipped binaries добавлен fail-closed Authenticode pipeline: `scripts/sign-windows-release.ps1` использует Windows SDK `signtool`, SHA-256, RFC3161 timestamp и `/pa` verification, а `.github/workflows/windows.yml` публикует installer только при наличии certificate secrets. Подпись Authenticode остаётся обязательной для `onnxruntime.dll`, где отсутствие или недоверенная подпись означают отказ; собственные `whisper.dll` и остальные артефакты получают release-подпись только после предоставления доверенного сертификата. До этого certificate-backed evidence отсутствует, поэтому release status остаётся `RELEASE_BLOCKED`. Манифест приходит по сети, поэтому путь из него проверяется на выход за каталог, а любая необъявленная `*.dll` рядом с `whisper.dll` блокирует загрузку: иначе загрузчик Windows подтянул бы её как зависимость мимо проверки хеша. Раскладка `whisper_full_params` зеркалится в коде и сверяется с размерами из манифеста (`48`/`304`) до первого вызова; чужой ABI — `abi_unsupported`, а не попытка вызвать.
