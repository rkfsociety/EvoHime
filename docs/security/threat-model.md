...
клиенты соответствующих контуров. Core без launch context отказывается запускать
IPC, кроме явно включённого dev-режима (`EVOHIME_DEV_MODE=1`). В этом режиме он
работает в неаутентифицированном режиме и явно помечает это в `core.started` и
в логе соединения.
