...
| Persistent Analysis Kernel (in progress) | schema v38 backup, metadata-only session/object manifest, ephemeral runtime state and allowlisted worker identity | reset/stop on limit or crash; no process-memory rehydrate and no blind external retry | disable by withholding additive `analysis_kernel` capability; preserve manifest/audit metadata; keep stage files until full evidence | storage/runtime/supervisor focused tests; full 28.4 matrix pending |
