...
suite, `build:shell`, `build:updater` и `check:bundle`. Shell preview успешно
...
- Локально на Linux прошли `cargo test --locked -p evohime-cli-protocol` (6/6),
  ...
  bounds и control-byte rejection,
  ...
  `cargo clippy --locked -p evohime-cli-protocol --all-targets -- -D warnings`,
...
- Platform-neutral HMAC/framing/command-envelope client вынесен в отдельный
...
- Windows named-pipe endpoint, reconnect loop и CLI event output вынесены из
  ...
  `0.0.000062`.
- CLI library разделён на фасад и внутренние модули `args`/`events`, сохранив
  прежние публичные re-export paths; marker CLI повышен до `0.0.000063`.
...
пригодным для работы: при отсутствии component marker updater считает базовые
...
и typed `Unavailable` admission без versioned inference-stream adapter.
Worker execution и scheduler performance не заявляются.
...
or stale observations remain unknown; gateway, credentials and retry ownership
...
## Plan 146 — Voice Input & Dictation v1 (2026-09-09)

Implemented bounded dictation profile, schema v114, idempotent metadata
storage, typed listener-unavailable result, authenticated IPC 273/118 and
redacted Electron projection. Local tests/builds/linters/smoke/E2E/runtime
were not run; exact-commit CI is `UNAVAILABLE` until push.
...
diagnostic vocabulary before rendering or writing Markdown. A raw URL or
...
Workflow storage tests вынесены из `workflow_store.rs` в
`workflow_store_tests.rs`; production-файл уменьшен до 984 строк, dispatch
marker, monotonic replay, lease, cancellation и crash-recovery contracts
сохранены. Core marker повышен `0.0.000336 -> 0.0.000337`; 8 workflow-store
...
Automation storage tests вынесены из `automation_store.rs` в
`automation_store_tests.rs`; production-файл уменьшен до 667 строк,
owner-scoped idempotency, immutable preset snapshot, schedule CAS fencing,
archive/restore checksum и retention contracts сохранены. Core marker
повышен `0.0.000343 -> 0.0.000344`; 4 automation-store tests,
format/diff-check прошли локально. CI evidence для новой commit-группы будет
добавлено после её десятого коммита.

Child storage tests вынесены из `child_store.rs` в `child_store_tests.rs`;
production-файл уменьшен до 608 строк, bounded handoff/request/report
contracts, sequence atomicity и checkpoint round-trip сохранены. Core marker
повышен `0.0.000344 -> 0.0.000345`; 5 child-store tests,
format/diff-check прошли локально. CI evidence для новой commit-группы будет
добавлено после её десятого коммита.

Feedback storage tests вынесены из `feedback_store.rs` в
`feedback_store_tests.rs`; production-файл уменьшен до 365 строк,
field bounds, redaction, telemetry gate, signal aggregation и SQLite
round-trip contracts сохранены. Core marker повышен `0.0.000345 -> 0.0.000346`;
...
Analysis kernel storage tests вынесены из `analysis_kernel.rs` в
`analysis_kernel_tests.rs`; production-файл уменьшен до 611 строк, canonical
hash, secret/ephemeral object rejection, stale revision fencing, bounded
listing и crash recovery contracts сохранены. Core marker повышен
`0.0.000349 -> 0.0.000350`; 6 analysis-kernel tests, format/diff-check
прошли локально. CI evidence для новой commit-группы будет добавлено после
её десятого коммита.
...
Execution-environment profile tests вынесены из
`execution_environment_profiles_store.rs` в
`execution_environment_profiles_store_tests.rs`; production-файл уменьшен до
232 строк, monotonic revisions, atomic activation, stale snapshot fencing,
run binding и idempotency replay contracts сохранены. Core marker повышен
`0.0.000355 -> 0.0.000356`; 6 execution-environment tests,
format/diff-check прошли локально. CI evidence для новой commit-группы будет
добавлено после её десятого коммита.
...
Conversation bridge adapter tests вынесены из
`conversation_bridge_adapters_store.rs` в
`conversation_bridge_adapters_store_tests.rs`; production-файл уменьшен до
200 строк, bridge/binding identity checks, idempotency, cascading cleanup,
revision fencing и payload bounds contracts сохранены. 7 conversation-bridge
...
`0.0.000066 -> 0.0.000067`; 35 library tests и 3 transaction-binary tests,
strict clippy, format/diff-check и updater check прошли локально. CI evidence
для новой commit-группы будет добавлено после её десятого коммита.
