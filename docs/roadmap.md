...
| 7.99 | Cloud sync / remote workspace | L | 🟡 | wave 1 ✅ push `BackupDump` на remote (`/api/sync/*`, `sync_runs`); wave 2 ✅ идемпотентный restore (`restore_backup` + CLI `evohime-import`); wave 3 ✅ remote pull (`/api/sync/pull`, direction в runs, лимит 64 MiB + checksum-сверка); остаётся авто-sync по расписанию |
...
**Актуальный статус 2026-07-24:** `7.93`–`7.98` ✅ — request context, task timeline telemetry, log safety, deep health checks, backup/export и multi-operator authz; `7.99` waves 1–3 ✅ — cloud sync push (`sync_runs` + owner-only `/api/sync/*`), идемпотентный restore (`restore_backup` + CLI `evohime-import`) и remote pull (`/api/sync/pull` с direction, лимитом 64 MiB и checksum-сверкой); остаток `7.99` — авто-sync по расписанию, либо переход к `7.100`.
