...
| 4 | **Test and lifecycle foundation**: HTTP/WS integration harness, scheduler regression tests, graceful shutdown, bounded cleanup `session_buses` | `crates/server/tests/`, `crates/server/src/startup.rs`, `app.rs`, `ws.rs`, `scheduler.rs` | 🟡 graceful shutdown ✅, session bus cleanup ✅, integration harness docs ✅, scheduler regression docs ✅ — остаток: full E2E tests при database (EVOHIME_REQUIRE_DB) |
