...
| 7.103 | Online continual learning (still no weight fine-tune; stronger experience) | L | 🟡 | wave 1 ✅: обучение на провалах — ограниченная полоса extract (≤2 кандидата `failure_pattern`/`verification_rule`, scope experience, confidence cap 0.6 → только Ask-гейт, без auto-promote); harmful-фидбек использованной памяти уже был; остаток — эскалация повторов, retrieval-приоритизация уроков |
