...
| 7.101 | Eval harness (golden tasks, regression agents) | L | 🟡 | wave 1 ✅: крейт `evohime-evals` — golden tasks против реального agent loop + tools с mock-моделью; раннер в `cargo test --workspace` (CI) и CLI `evohime-eval`; остаток — LLM-as-judge, live-provider сравнение |
