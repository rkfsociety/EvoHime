...
Обновлено: 2026-09-04. Планы 01–25 завершены и
удалены из каталога временных планов. План 19.0 добавил только пользовательский
self-repair/self-update цикл; автоматического ремонта, push и фонового
перезапуска работающей сессии нет.
...
- Windows package smoke tests, автоматический workflow «Быстрые проверки EvoHime» и ручной workflow «Ручной выпуск EvoHime»;
- единый Inno Setup installer с одним desktop shortcut; установленный клиент сам поднимает supervisor, а supervisor — Core;
- обновление из постоянного GitHub Release: клиент сверяет зелёный commit, скачивает `EvoHime-Setup.exe` только при совпадении манифеста и SHA-256, а при запуске показывает отдельный экран с этапами и передаёт установщик `evohime-transaction.exe --installer` только после полной загрузки. Обычный интерфейс, Core и supervisor не запускаются до завершения транзакции; после успешного health-handshake transaction worker запускает новую Еву. Уже работающий клиент продолжает использовать фоновую загрузку со своим баннером подтверждения. Production использует CI installer, локальная пересборка через `launchPolicy: "build"` сохранена только для разработки;
...
- один постоянный релиз `installer` с описанием из `installer/release-notes.md`: `EvoHime-Setup.exe` и `EvoHime-Setup.json` в нём перезаписываются после успешного ручного полного workflow «Ручной выпуск EvoHime» (`Run workflow`), новых релизов и версионных тегов не создаётся. Установщик нужен для первой установки, запускных обновлений и фоновой подготовки обновления работающего клиента;
...
В GitHub Actions доступны два независимых workflow. Автоматический «Быстрые
проверки EvoHime» запускается на каждом push и pull request и параллельно
...
План 109 закрывает gap collection bindings поверх registry v1: Core хранит
versioned `KnowledgeCollection` только как bounded уникальные ссылки на
...
План 105 закрыт 3 сентября 2026 года: Prompt Cache Planner реализует hashed
PromptSegments, deterministic stable/dynamic ordering, profile-driven cache
...
План 116: Local Model Runtime Manager добавил Core-owned hardware snapshot,
immutable model/runtime/artifact/session records, conservative fit, bounded
verified resumable download, atomic promotion, supervisor loopback health probe,
local-managed gateway/resilience mapping, policy fence, recovery metadata и
authenticated IPC 257/102 с bounded Electron projection. Rust check/clippy,
focused manager tests, protocol/typecheck, полный Electron suite и старые
release/security/automation гейты подтверждены.
