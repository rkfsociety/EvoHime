...
- Windows package smoke tests и workflow «Быстрые проверки и ручной выпуск EvoHime»;
...
- один постоянный релиз `installer` с описанием из `installer/release-notes.md`: `EvoHime-Setup.exe` и `EvoHime-Setup.json` в нём перезаписываются после успешного ручного полного workflow «Быстрые проверки и ручной выпуск EvoHime» (`Run workflow`), новых релизов и версионных тегов не создаётся. Установщик нужен для первой установки и фоновых обновлений клиента;
...
Workflow «Быстрые проверки и ручной выпуск EvoHime» имеет два контура. На каждом push и pull request после `Изменения`
параллельно запускаются быстрые `Документация`, `Rust — быстрые проверки` и
`Electron — protocol и typecheck`. Полный контур запускается только вручную
через `Run workflow`: после успешных быстрых проверок он последовательно
выполняет Rust security-gates, Electron E2E/package, Windows compatibility,
native acceptance и публикацию постоянного installer release. Каждый этап
останавливает последующие при ошибке, а job-фильтр сохраняет корректные
пропуски для незатронутых областей.
