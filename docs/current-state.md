...
- в настройках добавлен отдельный движок coding-задач `Codex CLI`: флажок `Coding-задача (Codex CLI)` в composer передаёт coding intent через additive IPC-поле, Core запускает `codex exec --json` в выбранном workspace и показывает bounded stdout/stderr в ленте; обычный диалог остаётся Core-owned, ошибки Codex не переключаются молча на другой backend;
...
Проверки 2026-08-26: Node.js 22.22.2, Electron `npm test` — 420 тестов прошли, 24 optional real-Core/update/pipe теста пропущены без собранного Core; `check:protocol`, `typecheck`, production build и bundle checks прошли. Локальный Rust прогон собрал и выполнил 535/541 тестов; 5 receipt-тестов требуют Windows key backend (`UnsupportedPlatform`), голосовой IPC-тест зависит от Windows app catalog. `cargo check -p evohime-supervisor` прошёл. Installer/upgrade и остальные Windows gates подтверждаются Windows CI.
