...
| 27 | [Farama-Foundation/chatarena](https://github.com/Farama-Foundation/chatarena) | Исследовано | Arena coordinator, Environment/TimeStep lifecycle, player/backend separation, turn/parallel scheduling, scoped message visibility, moderator termination, config-driven environments и CLI/Web UI | Адаптировать Core-owned scenario state machine, typed observation/step/terminal contract, private/public message projections и deterministic scheduler; deprecated Python runtime, raw LLM backends, Supabase persistence и multi-agent game logic в production не подключать |
| 28 | [a16z-infra/ai-town](https://github.com/a16z-infra/ai-town) | Исследовано | TypeScript/Convex persistent simulation, single-threaded per-world engine, queued typed inputs, generation-based overlap protection, tick/step batching, diff persistence, async agent operations, conversations и vector memories | Адаптировать Core-owned input queue, generation/lease semantics, tick-vs-step scheduling, isolated state snapshots/diffs, async operation receipts, archival/history separation и local embedding cache; Convex cloud, Pixi/React UI, external LLM/Replicate egress и single-document world state напрямую не подключать |
| 29 | [joonspk-research/generative_agents](https://github.com/joonspk-research/generative_agents) | Исследовано | Perceive/retrieve/plan/reflect/execute cognitive loop, memory stream with event/thought/chat nodes, spatial memory, scratch state, recency/relevance/importance retrieval, hierarchical daily planning, evidence-linked reflection и fork/replay simulation | Адаптировать provenance-aware memory records, separate scratch/context/long-term layers, multi-factor retrieval, bounded reflection, hierarchical plan preview, virtual-clock fixtures и snapshot/fork semantics; legacy OpenAI/Django/Python, prompt-generated actions, raw filesystem state, static assets и unrestricted human-like memory не подключать |
| 30 | [OpenHands/OpenHands](https://github.com/OpenHands/OpenHands) | Исследовано | Agent Canvas control center поверх отдельного Agent Server/Automation Server, сменяемые local/remote/cloud backends, typed action/observation/event protocol, ACP subprocess boundary, runtime-services context, workspace resolution, Docker workspace sandbox, Electron lifecycle и backend health checks | Адаптировать Core-owned execution boundary, canonical event ledger, typed tool receipts, backend registry, capability/version negotiation, bounded workspace/sandbox profiles, secret references, durable automation jobs и ACP-style adapter; Agent Canvas, uvx/Python runtime, внешний HTTP ingress и host-full-access режим напрямую не подключать |
...
- **Источник:** [Farama-Foundation/ChatArena](https://github.com/Farama-Foundation/chatarena), [README и описание lifecycle](https://github.com/Farama-Foundation/chatarena/blob/main/README.md), [Arena coordinator](https://github.com/Farama-Foundation/chatarena/blob/main/chatarena/arena.py), [Environment contract](https://github.com/Farama-Foundation/chatarena/blob/main/chatarena/environments/base.py), [MessagePool/visibility](https://github.com/Farama-Foundation/chatarena/blob/main/chatarena/message.py)
- **Дата проверки:** 2026-08-21
- **Ревизия/commit:** `a15802dd89c0d69165bb0b07e70c2bac5a7c4e36`; последний commit `2025-08-11` (`Update README.md`); checkout `main` чистый; `py -m compileall -q chatarena` прошёл без ошибок
- **Лицензия:** Apache-2.0
- **Состав:** Python package `chatarena 0.1.18`, `Arena` coordinator, `Environment`/`TimeStep` interfaces, `Player`/`Moderator` agents, pluggable language backends, config/registry loader, conversation/chameleon/PettingZoo environments, CLI/Web UI, CSV/JSON history export и optional Supabase datastore
- **Назначение:** исследовательская библиотека multi-agent language-game environments: несколько игроков получают natural-language observations, выбирают действия через backend, а environment обновляет скрытое состояние, сообщения, rewards и terminal status
- **Краткий вывод:** ChatArena полезна как компактный reference для разделения coordinator → environment → player/backend, turn/parallel scheduling, typed timestep, private/public message projections и moderator-controlled termination. Прямое подключение не подходит: проект официально deprecated с 2025-08-11, требует устаревший Python/LLM stack, хранит состояние преимущественно в памяти, имеет слабую schema/security boundary и не соответствует Core-first Windows runtime EvoHime.

#### Что изучено

- `Arena` создаёт environment и players, выполняет `reset`, выбирает next player, получает observation, вызывает player, проверяет action и применяет `environment.step`. После `invalid_actions_retry=5` подряд Arena бросает `TooManyInvalidActions`.
- `Environment` задаёт минимальный lifecycle: `reset`, `get_next_player`, `get_observation`, `check_action`, `step`, `is_terminal`, `print`; `TimeStep` возвращает observation, per-player reward и terminal flag. `num_players` выводится из `player_names`.
- `Conversation` хранит game state в `MessagePool`, поддерживает round-robin и parallel режим. При parallel несколько игроков действуют в одном turn; сообщения текущего turn не попадают в обычное player observation до перехода к следующему turn.
- `ModeratedConversation` добавляет `Moderator`, который видит историю, пишет сообщения с заданной visibility и периодически проверяет natural-language terminal condition через отдельный backend. Это позволяет отделить game dynamics от судьи/модератора.
- `Chameleon` демонстрирует hidden-role environment: moderator messages имеют адресную видимость, фазы `give clues → accuse → guess`, приватные votes и per-player rewards. Это reference для hidden observations и фазовой state machine, а не для production authorization.
- `Message` содержит sender, content, turn, timestamp, visible_to, msg_type, logged и `msg_hash`; `MessagePool.get_visible_messages` фильтрует сообщения по turn и recipient. Moderator имеет особое право видеть историю независимо от recipient.
- `Player` отделяет role description и global prompt от `IntelligenceBackend`; backend можно выбрать из registry или передать объектом. Есть Human backend, OpenAI/Anthropic/Cohere/HuggingFace/Bard/LangChain adapters и optional async API.
- Config classes (`ArenaConfig`, `EnvironmentConfig`, `AgentConfig`, `BackendConfig`) рекурсивно загружают JSON и через string registries создают arena. Environment subclasses регистрируются декоратором `register_env`, backends — `register_backend`.
- README предлагает кастомизировать Arena для собственного loop, Environment для game rules/state/rewards/observations, Backend для message formatting и Player для policy/backend interaction. Это хорошее разделение ответственности для evaluation harness.
- Arena умеет сохранять config и history в CSV/JSON, но persistence не является event-sourced: history берётся из текущего `environment.get_observation()` и сериализуется как plain content/metadata.
- `database.py` содержит optional Supabase integration: отдельные таблицы Arena/Moderator/Player/Message, deterministic UUID5 message ids и `logged` flag для повторной отправки. Сырые message contents и role/config data уходят во внешний datastore.
- Tests покрывают регистрацию/загрузку environments, базовую MessagePool visibility и game setup; LLM Arena tests пропускаются без API keys. Полный unit collection в текущем изолированном окружении не стартовал из-за отсутствующей зависимости `tenacity`; это dependency issue, а не результат тестов проекта.
- README сообщает, что проект deprecated из-за недостаточного community use и больше не планирует updates/support. Требования проекта — Python `>=3.7`, OpenAI API для LLM agents и старые optional pins вроде `gradio 3.34.0`, `pydantic 1.10.13`, `langchain >=0.0.340`.

#### Что можем использовать в Еве

- **Coordinator/environment/player separation.** Перенять разбиение evaluation/scenario runner на Core-owned coordinator, isolated environment state и model/player adapter. Electron получает только projections, а Core запускает lifecycle и проверяет каждую transition.
- **Typed step contract.** Адаптировать `TimeStep` как Rust/Core сущность `observation + reward/verdict + terminal`, расширив её run/task/step IDs, state hash, policy decision, approval link, cancellation и durable receipt. UI не должен вычислять terminal/reward самостоятельно.
- **State machine environments.** Для будущего offline evaluation описывать сценарии фазами и явными переходами: например `observe → propose → approval → execute → verify → complete/escalate`. Фазы должны быть typed/allow-listed, а не определяться свободным текстом модератора.
- **Turn and scheduling policy.** Перенять round-robin, parallel-round и moderator-period как варианты deterministic scheduler. Добавить fixed seed, bounded concurrency, ordering guarantees, cancellation barrier и replayable schedule; параллельные действия не должны создавать недетерминированные side effects.
- **Private/public observation projections.** MessagePool показывает полезную модель: один canonical event stream и разные observations для получателей. В Еве Core должен строить redacted projections по workspace/chat/agent capability scope; `all` не должен быть default для секретных/tool/approval events.
- **Moderator/judge stage.** Использовать отдельный evaluator/moderator stage для проверки terminal condition, policy compliance и scenario acceptance. Его verdict advisory, пока не подтверждён deterministic Core assertions; judge не получает права менять state или обходить approval.
- **Role description vs capability.** Сохранять role/global prompt как context для модели, но отделить его от authorization. `Player`/agent role не расширяет filesystem, network, secret или provider capabilities; реальные права задаются Core policy и supervisor.
- **Environment registry как ограниченный сценарный каталог.** Идею `register_env` адаптировать в versioned allow-listed registry для встроенных evaluation fixtures. Не поддерживать произвольный import по имени, Python plugin code или конфигурации, которая может получить доступ к host.
- **Config round-trip.** Перенять `to_config/from_config` для воспроизводимого scenario manifest: environment type/version, participants, scheduler, backend/model identity, seed, budgets, policy revision и fixture snapshot. Перед запуском manifest должен проходить schema/hash validation.
- **Human-in-the-loop backend.** Human backend и moderator handoff полезны как evaluation paths для approval/escalation: агент может остановиться и ждать пользователя, а не имитировать подтверждение. Human input сохраняется с provenance и не смешивается с model-generated action.
- **Message kinds and recipients.** Развить `msg_type` в typed event kinds: user instruction, model proposal, tool request, tool result, policy denial, approval request, approval receipt, evaluator verdict и error. Recipient visibility и redaction должны быть Core-enforced и проверяться тестами leakage.
- **Invalid-action handling.** Идею ограниченных повторов сохранить для malformed structured output, но вместо `TooManyInvalidActions` создавать typed failure, budget/deadline accounting, retry receipt и безопасную terminal outcome. Invalid model output не должен скрывать причину или повторять side effect.
- **Offline scenario UI.** CLI/Web UI идею использовать только как presentation/evaluation frontend для просмотра state transitions, observation projections и replay. В продукте это должно идти через существующий Electron IPC, а не через встроенный HTTP/Gradio server.
- **History export.** Перенять ограниченный redacted export trajectory в JSONL/CSV для AgentBench/OpenLLMetry-like evaluation, добавив field-level capture policy, secret redaction, retention/forget и stable event ordering.

#### Ограничения и риски

- **Проект deprecated.** README прямо заявляет отсутствие дальнейшей поддержки с 2025-08-11. Нельзя принимать его API, dependency pins или текущие environments как долгосрочную external dependency.
- **Python/LLM stack устарел.** Package требует Python `>=3.7`, использует старые tenacity/OpenAI/LangChain/Gradio/Pydantic сочетания и необязательные provider packages. Это не соответствует Rust Core и добавляет maintenance/packaging burden.
- **State in memory.** MessagePool и environments держат canonical state в Python objects; отсутствуют SQLite event ledger, transactional transition, crash recovery, migration, replay receipt и forget/redaction contract.
- **Message visibility не является security boundary.** `visible_to` — обычное поле сообщения; нет Core ACL, capability check, authenticated sender или enforcement against malicious code/model. Moderator может видеть всё по специальному имени.
- **Неаутентифицированный sender.** `Environment.step(player_name, action)` принимает player name от coordinator и самими базовыми контрактами не доказывает identity/capability. В Еве sender, run scope и approval binding должны проверяться Core перед переходом.
- **Небезопасный dynamic registry/config.** String registry и Configurable сохраняют произвольные kwargs, но не дают строгой schema/version/hash boundary. В production нельзя загружать неизвестные environment/backend классы из внешнего JSON.
- **Natural-language moderator judge.** `Moderator.is_terminal` вызывает модель и парсит ответ regex по `yes/y/...`; это вероятностный terminal decision, vulnerable к prompt injection и false positive/negative. Deterministic policy и state predicates должны быть источником истины.
- **Слабый action protocol.** Action — plain string; `check_action` реализуется environment-ом, а retry повторяет backend call без typed parse/error semantics. Нельзя переносить это в tool gateway без canonical JSON/schema validation.
- **Ошибки и terminal semantics.** `is_terminal` в нескольких environments фактически возвращает `True` только для end signal, иначе `None`; Arena не проверяет terminal до ручного `step`, а invalid-action exhaustion бросает exception без структурированного result/receipt.
- **Message timestamp defect.** В dataclass `Message` default задан как `time.time_ns()` при объявлении класса, а не `field(default_factory=time.time_ns)`, поэтому сообщения без явного timestamp получают один и тот же timestamp. Это ломает его пригодность как durable ordering/identity primitive.
- **MessagePool lifecycle incomplete.** `reset()` очищает список, но не обновляет `conversation_id`; `_last_message_idx` объявлен, но не используется, а queue/thread-safety оставлена TODO. Для concurrent Core это неприемлемо.
- **Raw content persistence.** Supabase integration сохраняет raw message content, prompts/config и visibility с service secret key; отсутствуют redaction, field-level consent, retention/forget, encryption contract и local-only guarantee. В EvoHime этот datastore напрямую не подключать.
- **External egress and secret risk.** OpenAI/Anthropic/Cohere/Bard/LangChain backends отправляют observations/history внешним провайдерам; backend selection не равен user consent или data policy.
- **Неполная проверка environment transitions.** Chameleon использует substring/regex parsing для votes и code guess, random.choice без seed, assert для turn validation и plain strings для game phases. Это полезно для demo, но не для security/evaluation ground truth.
- **Parallel semantics ограничены.** Parallel mode лишь задерживает увеличение turn и не даёт transaction/barrier/merge conflict rules. Реальные concurrent tools могут привести к races и duplicate side effects.
- **Покрытие тестами недостаточно.** Существенная часть Arena tests skip при отсутствии API keys; нет заметных проверок cancellation, crash recovery, visibility leakage under hostile input, replay determinism, authorization или persistence durability.
- **Лицензия и внешние компоненты.** Apache-2.0 применима к ChatArena, но PettingZoo, Supabase, provider SDKs, LangChain, Gradio и модели имеют отдельные условия. При заимствовании кода/компонентов нужно вести attribution/NOTICE и проверять каждую зависимость.

#### Предварительное решение

`адаптировать` coordinator/environment/player separation, typed timestep, explicit phase state machines, deterministic turn/parallel scheduler, scoped observation projections, moderator/evaluator as advisory stage, versioned scenario manifests, human handoff backend и redacted trajectory export; `наблюдать` только как исторический reference; `не подключать` deprecated Python runtime, dynamic registry, natural-language terminal as authority, raw LLM backends, Supabase datastore и ChatArena game environments в production Core Евы.

#### Связь с EvoHime

- ChatArena хорошо подтверждает текущую Core-first границу: Electron может показывать Arena-like timeline, но Environment state, scheduler, observations, model calls, approvals и terminal verdict должны принадлежать Rust Core.
- Возможные typed сущности для будущего evaluation layer: `ScenarioManifest`, `ScenarioEnvironment`, `Participant`, `ObservationProjection`, `TurnPolicy`, `ScenarioStep`, `ScenarioVerdict`, `EscalationRequest` и `ScenarioEvent`.
- Первый полезный fixture для Евы: два scoped participants и evaluator в synthetic workspace; один участник видит user request, второй — ограниченный tool result, evaluator — redacted state predicates. Проверить, что `all`/broadcast не раскрывает секреты и что approval event виден только разрешённым ролям.
- Multi-agent capability должна быть выключена по умолчанию, запускаться с bounded turns/parallelism/budget, иметь explicit cancellation и durable terminal receipt. Model-generated moderator не может выдать authorization или завершить опасное действие.
- Критерии готовности: deterministic reset/replay; authenticated participant identity; typed actions; recipient/visibility enforcement; no secret leakage; policy/approval before side effects; transaction-safe parallel merge; cancellation; local durable event ledger; redacted export; terminal verdict backed by Core assertions.

### 28. a16z AI Town

- **Источник:** [a16z-infra/ai-town](https://github.com/a16z-infra/ai-town), [ARCHITECTURE.md](https://github.com/a16z-infra/ai-town/blob/main/ARCHITECTURE.md), [engine scheduler и generation guard](https://github.com/a16z-infra/ai-town/blob/main/convex/engine/abstractGame.ts), [engine lifecycle](https://github.com/a16z-infra/ai-town/blob/main/convex/aiTown/main.ts), [agent loop](https://github.com/a16z-infra/ai-town/blob/main/convex/aiTown/agent.ts), [memory/vector search](https://github.com/a16z-infra/ai-town/blob/main/convex/agent/memory.ts)
- **Дата проверки:** 2026-08-21
- **Ревизия/commit:** `7b242334bfbfef02f7718bded120d431e8f307df`; последний commit `2026-06-12` (`Merge pull request #299 from hankchnag/fix/windows-install-unused-hnswlib`); checkout `main` чистый; `npm run build` прошёл; 50 Jest-тестов прошли через Windows-совместимый запуск
- **Лицензия:** MIT
- **Состав:** TypeScript/React/Vite frontend с PixiJS, Convex server functions/database/vector search, reusable tick-based game engine, AI Town world/player/conversation/agent model, Ollama/OpenAI/Together/custom OpenAI-compatible LLM adapter, embeddings cache, memory/reflection layer, Docker/Fly/Vercel deployment и optional Clerk auth
- **Назначение:** deployable starter kit для виртуального мира, где AI-персонажи и люди перемещаются, находят друг друга, разговаривают, сохраняют воспоминания и продолжают simulation в общем persistent world. Проект вдохновлён Generative Agents и отделяет generic engine от game-specific rules и UI.
- **Краткий вывод:** это самый полезный из исследованных repos для паттернов длительно живущего Core runtime: queued inputs, single-owner state machine, generation-based stale-run protection, tick/step batching, state diff persistence, async LLM operations с operation lock, архивирование завершённых сущностей и разделение high-frequency messages от основного simulation state. Прямое подключение не подходит: Convex cloud и generated server model заменяют существующий Rust Core/SQLite/supervisor, world state ограничен одним небольшим документом, auth optional, а LLM/memory/persistence paths требуют собственной security, budget, cancellation и redaction модели.

#### Что изучено

- `AbstractGame` задаёт `tickDuration`, `stepDuration`, `maxTicksPerStep`, `maxInputsPerStep`, `handleInput`, `tick`, `beginStep` и `saveStep`. AI Town тикает каждые 16 ms, группирует до 600 ticks в step примерно на секунду и обрабатывает не более 32 inputs за step.
- Вход пользователя или агента не меняет game state напрямую: `engineInsertInput` создаёт запись в `inputs` с монотонным номером, engineId, name, args и best-effort `received` timestamp. `AbstractGame` выбирает очередь по номеру, исполняет input через typed handler, сохраняет ok/error return value и продвигает processed input number.
- Engine state имеет `currentTime`, `lastStepTs`, `processedInputNumber`, `running` и `generationNumber`. При каждом save generation увеличивается; scheduled run передаёт ожидаемое поколение, а stale run отклоняется при mismatch. Это защищает от overlapping engine steps при kick/stop/restart race.
- `runStep` загружает snapshot world, строит in-memory `Game`, выполняет steps до deadline, сохраняет diff и планирует следующий run. `saveWorld` сначала применяет engine update с generation check, затем сохраняет game diff и запускает запланированные agent operations.
- `Game` делает generic engine независимым от AI Town rules: `tick` вызывает player logic, pathfinding, position updates, conversation ticks и agent ticks; `handleInput` маршрутизирует только зарегистрированные `inputHandler` definitions.
- Основной world document содержит небольшие serialized collections players, conversations и agents. Map и человекочитаемые player/agent descriptions вынесены в отдельные таблицы. Завершённые players/conversations/agents архивируются отдельно, а `participatedTogether` хранит историю связей для cooldown и memory lookup.
- World state является единственным engine-owned state: внешние компоненты не должны patch-ить engine tables, а отправляют inputs. Это близко к Core-owned state boundary EvoHime.
- High-frequency chat messages лежат в отдельной `messages` table и не проходят через game engine; это уменьшает размер snapshot и latency для streaming. Основной conversation state хранит membership, typing lock, lastMessage и message count.
- Conversation membership имеет состояния `invited`, `walkingOver`, `participating`; input handlers проверяют player/conversation IDs, запрет self-invite, single active conversation и transitions. Typing state имеет UUID/owner/time и timeout, чтобы два агента не писали одновременно.
- Agent tick не вызывает LLM синхронно внутри simulation. Он создаёт `inProgressOperation` с name/operationId/started, добавляет operation в diff, а после save Convex scheduler запускает long-running internal action. Action читает данные, вызывает LLM/embedding, сохраняет результат и отправляет input обратно в engine.
- Agent operation lock не позволяет одному agent иметь несколько async operations; timeout снимает зависшую operation после `ACTION_TIMEOUT`. После operation результат возвращается в engine только через `finish...` input, что отделяет внешнюю работу от канонического state transition.
- `agentGenerateMessage` разделяет start/continue/leave conversation prompts, загружает current history, personality/plan и related memories, а затем записывает message и `finishSendingMessage` input. `agentDoSomething` выбирает activity, wander destination или candidate conversation; несколько random sleeps используются для снижения collision/OCC contention.
- Memory layer после разговора генерирует LLM summary от лица agent, оценивает importance, получает embedding и сохраняет memory + vector row. При поиске выполняется vector overfetch, затем rank по relevance, importance и recency; lastAccess throttled. После достаточного cumulative importance создаются LLM-generated reflection memories.
- `embeddingsCache` хеширует текст SHA-256 и сохраняет embedding по text hash, чтобы повторные prompt fragments не вычислялись заново. Размерность embedding является compile-time-like constant, которую нужно менять согласованно с provider.
- LLM adapter поддерживает Ollama локально по умолчанию, OpenAI, Together и произвольный OpenAI-compatible URL. `retryWithBackoff` повторяет transient 429/5xx, поддерживает streaming response parser и умеет автоматически `pull` отсутствующую Ollama model.
- Crons останавливают inactive worlds, перезапускают dead worlds и вакуумируют старые inputs/memories/embeddings. Archived conversations/messages намеренно не vacuum-ятся по умолчанию, чтобы сохранять историю для UI/agents.
- README описывает local Convex, self-hosted Docker Compose, Fly.io, Vercel и Windows через WSL2. Default LLM — local Ollama `llama3`, но production/cloud deployment легко направляет prompts к OpenAI/Together/custom endpoint.
- `npm ci --ignore-scripts` установил 748 packages; npm audit сообщил 24 vulnerabilities: 2 low, 6 moderate, 15 high и 1 critical. `npm test` в cmd сломан POSIX-синтаксисом `NODE_OPTIONS=...`; прямой запуск Jest с `set NODE_OPTIONS=...` дал 6 suites/50 tests passed.

#### Что можем использовать в Еве

- **Single-owner Core engine.** Перенять правило, что только Core владеет canonical run/workspace/task state. UI, provider adapters, background workers и optional evaluators отправляют typed inputs/commands, а не изменяют SQLite или state напрямую.
- **Queued input protocol.** Использовать bounded input queue с `input_id`, monotonic sequence, received timestamp, canonical name/args, processing status, typed return value и error receipt. Это естественное расширение текущих desktop IPC/core event contracts и даёт replayable user/tool/agent requests.
- **Generation/lease stale-run protection.** Добавить generation или run epoch к supervisor/core worker lifecycle: каждый scheduled continuation обязан предъявить ожидаемый generation, а stop/restart/cancel атомарно увеличивает epoch. Старый процесс/continuation fail-closed и не применяет diff.
- **Batch ticks versus durable steps.** Разделить высокочастотную in-memory scheduling/streaming от bounded durable commit. Для Евы это может быть `run step` с deadline и max actions, который обрабатывает queued inputs, cancellation и time-based tasks, затем одной транзакцией сохраняет state/event diff.
- **Typed handler registry.** Перенять `inputHandler` pattern: имена операций и validators известны Core, args проверяются до dispatch, unknown operation отклоняется. Registry должен быть compiled/allow-listed Rust code, а не dynamic module import или произвольный model-provided function.
- **Snapshot → simulate → diff → commit.** Использовать Game.load/saveDiff как mental model для evaluation fixtures, plan previews и isolated runs: загрузить snapshot, исполнять transitions в памяти, вычислить field-level diff, проверить policy/approval, затем атомарно commit-ить только разрешённые изменения.
- **Operation receipts для async work.** Перенять `inProgressOperation` + operationId: network/LLM/RAG/browser/voice jobs работают вне state transition, имеют owner, start/deadline/cancel generation, retry count и durable status; callback возвращается как typed input и повторная доставка idempotent по operationId.
- **Cancellation и recovery.** Объединить generation bump, operation timeout, supervisor restart и explicit user cancel. После restart Core восстанавливает pending operations из SQLite, переводит просроченные в typed timeout/cancelled outcome и не повторяет side effect без idempotency key/approval binding.
- **Separate high-frequency channels.** По аналогии с messages table отделить streaming tokens, typing/partial transcript, progress и UI telemetry от canonical execution events. Они могут быть lossy/bounded projections, тогда как approvals, tool calls, file changes и terminal receipts durable и ordered.
- **Archive active/history state separately.** Держать активный run/workspace projection маленьким, а завершённые conversations, operations, tool traces и summaries — в append-only/history tables с retention, redaction и explicit forget. Это сочетается с текущим SQLite/export design EvoHime.
- **World/workspace bounded snapshots.** Применить ограничение AI Town: state, передаваемый в каждый simulation step, должен быть малым и scoped. Большие документы, raw transcripts, embeddings и binary artifacts выносить в отдельные tables/files, а в active state хранить IDs, hashes и bounded summaries.
- **Explicit lifecycle states.** Перенять `running/stoppedByDeveloper/inactive` и engine `running` как основу для Core run/workspace lifecycle: `starting`, `running`, `draining`, `paused`, `cancelling`, `stopped`, `failed`, `archived`. Переходы должны иметь reason, actor, generation и durable event.
- **Local embedding cache.** Идею text-hash cache адаптировать в Core-owned RAG cache с provider/model/embedding-dimension/config revision в ключе. Cache hit не должен смешивать embeddings разных models или scopes, а запись должна быть redacted/forgettable.
- **Memory pipeline как advisory projection.** Разделить conversation summary, embedding retrieval, importance/recency ranking и reflection в отдельные jobs. Сохранять provenance, source event IDs, model/config revision и confidence; LLM summary не является фактом, permission или irreversible memory.
- **Observability of async loops.** На каждый step/input/operation писать sequence, generation, duration, retries, deadline, provider, tokens/cost, outcome и links между initiating command, external call и callback. Это напрямую соединяется с AgentOps/OpenLLMetry findings, но остаётся local typed event vocabulary.
- **Maintenance controls.** Перенять explicit stop/resume/kick/archive/vacuum operations для supervisor/admin UI, но реализовать их через authenticated Core IPC, approval/role policy и audit events. Destructive wipe должен быть отдельной подтверждаемой операцией с backup/restore path.
- **Optional local inference.** Default Ollama path показывает полезную privacy-preserving mode для Евы: provider gateway может работать локально, а cloud providers подключаются через explicit user configuration, egress policy и secret handling.

#### Ограничения и риски

- **Convex не переносится напрямую.** Backend functions, scheduler, validators, reactive queries, vector DB и optimistic concurrency требуют Convex runtime/cloud/self-hosted server. Встраивание их в Electron/Rust нарушило бы текущую Core architecture и добавило второй state owner.
- **Одно большое world state.** Документ `worlds` намеренно ограничен несколькими десятками KB и полностью загружается в каждый step. Для EvoHime нельзя масштабировать так workspace/database state; нужен SQLite snapshot/query scope и bounded context ledger.
- **Single-threaded engine.** Один world serializes all transitions, что упрощает races, но ограничивает throughput и не решает parallelism разных workspace/provider jobs. В Еве parallel async work должно быть за пределами Core transition, с bounded worker pool и deterministic commit ordering.
- **Input timestamp best-effort.** Sequence монотонен, но `received` timestamp не гарантирует строгую монотонность и input может попасть в соседний step. Для approvals/security важнее IPC sequence, causal parent, call hash и explicit ordering contract.
- **Generation guard не является полной cancellation.** Stale scheduled run отклоняется, но уже выполняющийся action/LLM request может продолжать расходовать ресурсы и ждать network. Еве нужны cancellation tokens, provider abort, supervisor kill boundary и cleanup receipts.
- **Async operation callback risks.** Operation state и result распределены между engine diff, scheduler action, messages table и callback input. Нужны idempotency, deduplication, bounded retries и защита от callback после cancel/changed conversation; текущий пример не является готовым transactional saga.
- **LLM egress and raw prompt logging.** `chatCompletion` делает `console.log(body)`, где есть messages и потенциально чувствительный context; cloud provider/custom URL получает prompts. В production EvoHime raw prompts, secrets, full tool args и private memory запрещены в logs/export по умолчанию.
- **Automatic model pull is side effect.** `tryPullOllama` может скачать модель при ответе 404. Для Евы загрузка моделей должна быть explicit install/update flow с disk budget, signature/provenance, approval и не происходить внутри пользовательского tool/action path.
- **Memory is LLM-generated.** Summary, importance и reflection могут ошибаться/галлюцинировать, а embedding retrieval не является truth. Нужны source-linked claims, confidence, user correction/forget and no authorization from memory.
- **Embedding dimension/provider coupling.** Один constant `EMBEDDING_DIMENSION` и vector index связывают deployment с конкретной model dimension; смена provider требует очистки данных/пересоздания embeddings. В Еве cache key должен включать provider/model/revision/dimension и поддерживать migration.
- **Memory ranking edge cases.** Min-max normalization может получить нулевой диапазон при одинаковых scores и produce NaN; reflection parser доверяет JSON от модели и индексы statements. Это advisory ranking, а не deterministic retrieval/security layer.
- **Auth optional.** README предлагает вернуть Clerk через git revert; базовый starter kit не делает обязательную identity/tenant boundary. Для локальной Евы identity/session secret/role/capability должны быть обязательны в Core IPC и supervisor launch context.
- **Data retention/forget incomplete.** Cron vacuums inputs/memories/embeddings, но conversations/messages по умолчанию сохраняются; archive и `wipeAllTables` не являются field-level redaction/forget protocol. Нельзя переносить retention policy без explicit user/data scope semantics.
- **External media/services.** Pixi assets, Replicate music generation, Fly/Vercel/Convex, Clerk and optional providers increase supply-chain and egress surface. MIT repo license не покрывает условия моделей, hosted services, assets и dependencies.
- **Dependency health.** `npm ci` сообщает 24 vulnerabilities, включая 1 critical; build/test passing не означает безопасную поставку. Перед использованием компонентов нужен отдельный npm audit/SBOM/license review и, вероятно, обновление старых React/Pixi/Convex pins.
- **Windows workflow mismatch.** README ориентируется на WSL2/Node 18, тогда как EvoHime требует native Windows Electron + Rust Core и Node 22 только для development. AI Town нельзя считать готовым Windows desktop runtime.
- **Frontend is a web game UI.** React/Pixi/Vite и Convex reactive queries подходят для browser visualization, но не заменяют встроенный Electron renderer/IPC и не должны получить прямой доступ к EvoHime workspace/state.
- **No production approval/sandbox.** AI Town inputs and agent actions are game mechanics; there is no EvoHime-equivalent approval receipt, sandbox, filesystem policy, call_hash, secret redaction or supervisor Job Object. Game-engine invariants нельзя принимать за security controls.
- **Public raw debug operations.** `testing:wipeAllTables`, `stop`, `resume`, `kick`, random position/debug operations are useful development hooks, but require strict internal/admin gating and destructive confirmation before any product adaptation.

#### Предварительное решение

`адаптировать` single-owner Core engine, bounded queued inputs, sequence/return receipts, generation-based stale-run protection, tick/step batching, snapshot/diff commit, async operation records, callback inputs, archive/history split, explicit lifecycle controls, local text-hash embedding cache и advisory memory pipeline; `наблюдать` Convex engine patterns only as architecture reference; `не подключать` Convex runtime, React/Pixi UI, cloud vector DB, optional Clerk/Replicate integrations, automatic model downloads, raw prompt logging и single-document world state в production EvoHime.

#### Связь с EvoHime

- AI Town подтверждает полезность существующей архитектуры `Electron → authenticated IPC → Rust Core → SQLite`, но предлагает усилить её generation/lease contract для supervisor restarts и async provider callbacks.
- Возможные Core сущности: `CoreInput`, `InputReceipt`, `RunGeneration`, `SimulationStep`, `StateDiff`, `AsyncOperation`, `OperationCallback`, `ExecutionArchive`, `ProjectionEvent`, `MemoryCandidate` и `MemoryEmbeddingCacheEntry`.
- Первый offline fixture: synthetic workspace agent с bounded input queue, один Core-owned state snapshot, delayed provider operation и forced restart между request/callback. Проверить, что stale generation не применяет callback, operation получает timeout/cancel receipt, а replay не повторяет side effect.
- Для ambient/voice/RAG layers high-frequency partial events можно хранить как bounded projections, но transcript finalization, approvals, tool calls, file changes, memory writes и forget operations должны идти через durable Core ledger.
- Критерии готовности: one owner per state scope; monotonic sequence; generation-safe resume; typed args/results; bounded batch/deadline; idempotent callbacks; cancellation reaches provider and supervisor; atomic diff commit; durable archive; redacted projections; provider/model-scoped memory cache; explicit retention/forget; no UI/direct database access.

### 29. Generative Agents: Interactive Simulacra of Human Behavior

- **Источник:** [joonspk-research/generative_agents](https://github.com/joonspk-research/generative_agents), [README и запуск simulation](https://github.com/joonspk-research/generative_agents/blob/main/README.md), [Persona cognitive loop](https://github.com/joonspk-research/generative_agents/blob/main/reverie/backend_server/persona/persona.py), [associative memory](https://github.com/joonspk-research/generative_agents/blob/main/reverie/backend_server/persona/memory_structures/associative_memory.py), [retrieval](https://github.com/joonspk-research/generative_agents/blob/main/reverie/backend_server/persona/cognitive_modules/retrieve.py), [reflection](https://github.com/joonspk-research/generative_agents/blob/main/reverie/backend_server/persona/cognitive_modules/reflect.py)
- **Дата проверки:** 2026-08-21
- **Ревизия/commit:** `fe05a71d3e4ed7d10bf68aa4eda6dd995ec070f4`; последний commit `2023-08-11` (`Merge pull request #24 from eltociear/main-1`); checkout содержит крупный demo dataset/assets; `py -m compileall -q reverie` прошёл без ошибок
- **Лицензия:** Apache-2.0 для репозитория; included visual assets и credits художников требуют отдельной проверки прав/атрибуции перед redistribution
- **Состав:** Python 3.9-oriented Reverie simulation server, Django frontend environment, `Persona`, Maze/pathfinder, spatial memory tree, associative memory stream, scratch state, cognitive modules `perceive/retrieve/plan/reflect/execute/converse`, OpenAI 0.27 wrapper, JSON/CSV prompt templates, forked simulation folders, precomputed base simulations, replay/demo endpoints и static game assets
- **Назначение:** reference implementation и demo для paper “Generative Agents: Interactive Simulacra of Human Behavior”. Персонажи живут в Smallville-like environment, воспринимают локальные события, сохраняют memories, вспоминают релевантное, строят дневные/часовые планы, отражают важные события, взаимодействуют и перемещаются по карте.
- **Краткий вывод:** это первоисточник наиболее полезной для Евы memory/cognition схемы: immutable-like event records → retrieval context → generated thought/reflection with evidence → plan → bounded execution. Переносить нужно не код и не «человечность», а contracts: memory provenance, semantic/temporal/importance retrieval, short-term scratch versus durable memory, reflection budget и plan-preview boundary. Прямое подключение непригодно: legacy OpenAI/Django/Python stack, raw prompt-driven state, mutable files, no authenticated Core boundary, огромный cost и отсутствие production-grade deletion/replay/security.

#### Что изучено

- `Persona.move` реализует главный cognitive loop: обновляет current time/tile, определяет first/new day, затем вызывает `perceive`, `retrieve`, `plan`, `reflect` и `execute`. Итогом является следующая tile/emoji/description, а не произвольный host side effect.
- `Scratch` — краткоживущий mutable state: current time/tile, identity stable set, current status/lifestyle, daily requirements, decomposed daily schedule, current action, path, chat state и reflection hyperparameters. Это отделено от long-term associative memory и spatial memory.
- `SpatialMemory.MemoryTree` хранит иерархию world → sector → arena → game objects, увиденных персонажем. Она помогает выбирать допустимые места действия и связывает planning с наблюдаемым окружением.
- `AssociativeMemory`/`ConceptNode` — memory stream. Узел содержит `type` (`event`, `thought`, `chat`), created/expiration, subject-predicate-object triple, natural-language description, embedding key/vector, poignancy, keywords, depth и evidence/filling links.
- Для быстрого доступа associative memory поддерживает chronological sequences по event/thought/chat, keyword indexes и keyword strength maps. При загрузке/сохранении состояние сериализуется в JSON nodes, embeddings и keyword strengths.
- Perception сначала строит spatial memory из nearby tiles, затем выбирает события внутри текущей arena по vision radius и attention bandwidth. Retention не позволяет заносить одну и ту же SPO-ситуацию в memory stream на каждом шаге.
- Новое событие нормализуется в SPO + description, получает keywords, embedding и LLM-generated poignancy. Chat event дополнительно сохраняет conversation rows как chat node, а event node может ссылаться на chat node через filling.
- `new_retrieve` для каждой focal point рассматривает events/thoughts, вычисляет recency decay, cosine relevance и poignancy/importance, нормализует компоненты и выбирает top-N по взвешенной сумме. В текущей ревизии веса явно заданы как `0.5` recency, `3` relevance и `2` importance.
- Retrieval обновляет `last_accessed` выбранных nodes, то есть чтение memory меняет её состояние и влияет на будущую recency score. Это полезно как модель attention trace, но требует отдельной durable event semantics в Еве.
- Reflection запускается после накопления заданной суммы importance. Сначала LLM предлагает focal points, затем retrieval выбирает evidence, другая prompt-функция генерирует insights, а каждый thought сохраняется с evidence node IDs и expiration примерно 30 дней.
- После завершения разговора создаются planning/memo thoughts, связанные с последним chat node. Таким образом, память хранит не только transcript, но и derived thought с указанием источника.
- Planning иерархичен: на first/new day генерируются wake-up hour, broad daily plan, hourly schedule и затем task decomposition в короткие действия. Внутри дня план выбирает текущий task, может вставлять conversational/reaction actions и пересобирать schedule.
- Plan output остаётся natural-language address вроде `world:sector:arena:object` с маркерами `<persona>`, `<waiting>`, `<random>`. Execute преобразует его в target tiles, pathfinder path и следующий шаг движения; выбор кандидата и предотвращение tile overlap частично random.
- Conversation generation каждый ход повторно делает retrieval relationship/context, суммирует ideas, генерирует utterance и прекращает беседу по model-provided end flag. В память попадают chat node, conversation summary, relationship/planning thoughts.
- `ReverieServer` fork-ит целую simulation directory, загружает meta/environment/persona JSON, ждёт environment step files, запускает persona cognition, записывает movement JSON и увеличивает виртуальное время. Один game step в README равен 10 секундам.
- Frontend Django и backend simulation server связаны через файловые JSON snapshots: frontend пишет environment state, backend пишет movement/meta, browser replay читает сохранённые step files. `save` сохраняет meta и persona memory; `exit` удаляет текущий forked simulation folder.
- Prompt wrapper использует старый `openai.ChatCompletion`, `openai.Completion` и `openai.Embedding`; structured output реализован через prompt + JSON parsing + retries/fallback. Embedding model по умолчанию `text-embedding-ada-002`.
- README требует вручную создать `utils.py` с OpenAI API key, owner name, filesystem paths и debug flag. Requirements pin Python 3.9-era packages, Django 2.2 и `openai==0.27.0`; авторы предупреждают о API hangs/rate limits, стоимости и необходимости часто сохранять simulation.
- В репозитории много precomputed memories, simulation folders, frontend assets и prompt templates. Это удобно для paper demo/replay, но не должно автоматически попадать в EvoHime package или память пользователя.

#### Что можем использовать в Еве

- **Provenance-aware memory record.** Перенять разделение `event/thought/chat` и поля `source_event_ids`, `subject/predicate/object`, `created_at`, `expires_at`, `importance`, `embedding_ref`, `keywords`, `derivation_depth`, `last_accessed` и `capture_policy`. Для EvoHime canonical event ledger остаётся источником, а thought/summary — derived projection.
- **Три слоя памяти.** Разделить `scratch/context` для текущего run, durable user/workspace memory для подтверждённых фактов и semantic index/cache для retrieval. Это дополняет уже реализуемый local Agentic RAG, context ledger, citations и redaction/forget rules.
- **Event → thought evidence graph.** Каждый generated summary/reflection должен ссылаться на конкретные event IDs, transcript spans или tool receipts. UI может показывать “почему это в контексте”, а forget/redaction должен удалять/пересчитывать derived nodes, а не оставлять orphan embeddings.
- **Multi-factor retrieval.** Использовать relevance + recency + importance как baseline scoring, но реализовать его в Core/RAG worker с bounded candidate set, deterministic tie-break, scope filters, zero-range handling, model/config fingerprint и explainable score components. Retrieval не получает права выполнять tools.
- **Access trace.** Фиксировать retrieval request, selected memory IDs, scores, redaction decisions и context assembly revision. Само `last_accessed` не должно бесконтрольно мутировать memory или продлевать retention; access semantics должны быть отделены от user-visible audit.
- **Importance budget for reflection.** Ввести bounded trigger по token/importance/event budget, чтобы background reflection запускалась после накопления достаточного материала, а не после каждого сообщения. Reflection job имеет deadline, provider budget, retry limit и cancel path.
- **Reflection as advisory synthesis.** Перенять pipeline focal points → evidence retrieval → insight generation, но хранить результат как `derived/advisory`, с confidence/provenance/model revision и возможностью user correction. Reflection не меняет identity, permission, approval или policy автоматически.
- **Expiration and retention.** Поле expiration полезно для временных thoughts, session hypotheses и stale observations. Ева должна реально применять expiration/forget transactionally, поддерживать user/workspace scope и удалять embeddings/index rows с durable receipt.
- **Stable identity versus current state.** Отделить immutable-ish profile/identity facts, versioned learned preferences и current status/context. Любое изменение identity-sensitive memory требует provenance, confidence и, для пользовательских фактов, подтверждения/редактирования.
- **Hierarchical planning.** Перенять daily/long-horizon goal → short task decomposition → next action как планировочную projection. В Еве план должен быть typed/previewable, иметь dependencies, budgets, cancellation and approval gates; natural-language plan не становится исполняемым host command без Core validation.
- **Plan/execute boundary.** Идея `plan` выдаёт намерение, а `execute` разрешает его в concrete target/action. Для Евы это означает отдельные `PlanStep` и canonical tool calls: path/scope/schema/approval проверяются Core, а модель не может напрямую придумать filesystem path или shell command.
- **Virtual-clock evaluation.** Использовать virtual time и deterministic scenario fixtures для проверки reminder, expiry, retry, timeout, daily compaction, reflection trigger и scheduled work. Это лучше переносит когнитивный loop в offline AgentBench harness без реальных sleep/API calls.
- **Perception bandwidth.** Ограничивать количество новых observations по scope, relevance и attention budget, записывая skipped/filtered counts. Для Евы это можно связать с context budget/ledger: model получает bounded evidence, но UI/audit сохраняет provenance полного retrieval decision.
- **Conversation memory compaction.** После завершения диалога создавать bounded summary + salient facts + relationship/context links, сохраняя source transcript separately. Raw transcript не должен автоматически становиться long-term memory и не должен попадать в prompts без scope/redaction.
- **Fork/replay snapshots.** Адаптировать forked simulations как copy-on-write scenario snapshots для план review, regression и evaluation. Реализовать через SQLite backup/transaction/run parent, immutable input sequence и redacted artifacts, а не через `shutil.copytree` живого state.
- **Human-facing introspection.** UI может показывать current goal, next action, retrieved evidence, reflection status и memory provenance как bounded projections. Это повысит объяснимость Евы без раскрытия raw system prompts, private memory или secrets.
- **Import of authored history.** Идею history CSV/whisper использовать как explicit import/migration path: пользователь выбирает scope, записи получают source/import revision и consent, проходят dedup/redaction, а не тайно влияют на persona.

#### Ограничения и риски

- **Legacy runtime.** Repository ориентирован на Python 3.9.12, Django 2.2 и `openai==0.27.0`; используется старый ChatCompletion/Completion/Embedding API. Это не совместимо с Rust Core и не должно быть runtime dependency EvoHime.
- **Raw API key in source-adjacent file.** README просит создать `utils.py` с OpenAI key, owner name, filesystem paths и debug flag. Такая модель секретов неприемлема для Евы; нужны DPAPI/safeStorage, supervisor environment и redacted logs.
- **LLM output as state transition.** Daily schedules, focal points, insights, SPO triples, actions, conversations и end flags генерируются prompts и парсятся loose JSON/text. Нет canonical schema, strong validation, permission check или approval boundary.
- **Prompt retry/fallback weak.** Wrapper ловит broad exceptions, возвращает строки `ChatGPT ERROR`/`TOKEN LIMIT EXCEEDED` или default responses, а некоторые validators принимают model output после нескольких попыток. Ошибка provider не должна превращаться в действительное действие/факт.
- **Cost and latency.** Один cognitive step может вызвать embeddings, poignancy scoring, retrieval-related prompts, planning, reflection, path/action prompts и conversation calls; 25 agents быстро дают огромную стоимость и serial latency. README отдельно предупреждает о rate-limit hangs и необходимости часто сохранять.
- **Mutable filesystem persistence.** Simulation state, memory JSON, CSV, environment and movement files пишутся напрямую, без SQLite transaction, atomic rename protocol, locking, schema migration, integrity hash или crash recovery. Frontend/backend file polling может прочитать частичный файл.
- **No authenticated process boundary.** Django server и simulation server обмениваются файлами/HTTP без EvoHime-equivalent named-pipe HMAC, role separation, workspace ACL, supervisor Job Object или network egress policy.
- **Dangerous destructive command.** `exit` делает `shutil.rmtree(sim_folder)`; interactive CLI commands и debug operations не имеют approval/role gate. Это допустимо для local demo, но запрещено как product control.
- **No deterministic replay guarantee.** Simulation folders позволяют replay snapshots, но random choices, unseeded sampling, live OpenAI responses, current locale/time and mutable prompts prevent bitwise deterministic reproduction. Для Евы replay должен работать по recorded canonical actions/provider fixtures.
- **Expiration not enforcement.** Concept nodes имеют expiration, но retrieval/associative indexes не показывают общей transactional очистки expired nodes. Простое наличие поля не обеспечивает retention/forget.
- **Memory privacy and hallucination.** Raw chats, derived summaries, embeddings, identity traits and relationship judgments сохраняются вместе; model-generated thought может стать ложным “фактом”. Нужны source-linked claim types, confidence, user correction, scope ACL и full redaction/forget.
- **Retrieval mutates memory.** Обновление `last_accessed` во время чтения смешивает ranking side effect с обычным query, усложняет replay и может бессрочно продлевать активность чувствительной памяти. В Еве access telemetry и retention mutation разделяются.
- **Natural-language addresses are unsafe.** `execute` принимает model-generated address и использует его для pathfinding; fallback может обращаться к жёстко заданному адресу, а target selection частично random. В EvoHime path/scope/schema проходят Core validation до запуска.
- **Spatial/game-specific coupling.** Maze, tiles, emojis, Smallville assets и collision pathfinder полезны для embodied simulation, но не соответствуют workspace/browser/tool execution и увеличивают package footprint.
- **Timezone/date assumptions.** Naive `datetime`, string formatting `%B %d, %Y`, locale-dependent prompts и virtual dates создают ошибки при timezone/DST/restart. Еве нужны UTC/RFC3339 bounds и explicit user timezone.
- **Broad exception handling.** В simulation loop и prompt layers много bare `except`, debug prints и продолжение после ошибки. Это скрывает потерю memory/action и мешает durable failure classification.
- **Dataset/assets licensing.** Apache-2.0 code license не означает, что included character/background/furniture assets, fonts, map data и external URLs можно свободно перераспределять. Нужен asset inventory и отдельная attribution/license decision.
- **No tests comparable to product needs.** `compileall` проходит, но repository is demo/research code with no meaningful Core-level tests for cancellation, concurrent writes, authorization, redaction, migration, replay determinism or memory correctness.

#### Предварительное решение

`адаптировать` provenance-aware event/thought/chat records, scratch/context/long-term memory separation, multi-factor retrieval, access telemetry, bounded reflection trigger, evidence-linked insights, expiration semantics, hierarchical plan preview, plan/execute boundary, virtual-clock evaluation, conversation compaction и SQLite fork/replay snapshots; `наблюдать` paper/research concepts и prompt templates как design reference; `не подключать` legacy Python/Django/OpenAI runtime, raw filesystem simulation, model-generated host actions, unbounded human-like memory, static assets, direct API keys и unauthenticated frontend/backend protocol в EvoHime.

#### Связь с EvoHime

- Для текущего Electron + Rust Core продукта наиболее естественный перенос — отдельный Core-owned `MemoryRecord`/`DerivedMemory` pipeline поверх существующего local Agentic RAG, context ledger, citations и redaction/forget contracts.
- Возможные typed сущности: `MemoryRecord`, `MemoryDerivation`, `MemoryEvidence`, `RetrievalRequest`, `RetrievalCandidate`, `ReflectionJob`, `PlanDraft`, `PlanStep`, `ActionResolution`, `MemoryRetentionEvent` и `MemoryAccessEvent`.
- Первый offline fixture: пользовательский запрос → durable observation/tool receipt → retrieval с score breakdown → plan draft → approval-gated action → derived summary с citations. Проверить, что model-generated thought без evidence не попадает в trusted memory и forget удаляет все projections/embeddings.
- Reflection и compaction запускать как cancellable Core background operations с provider budget, snapshot revision и idempotency key. UI показывает status/evidence, но не меняет memory или plan напрямую.
- Критерии готовности: typed schema; source/evidence links; scope and consent; deterministic retrieval tie-break; bounded context; explicit expiry/forget; no secret leakage; plan preview before side effect; approval/cancellation; atomic SQLite writes; replay from recorded inputs; model failures are typed unknowns, not facts/actions.

### 30. OpenHands / Agent Canvas

- **Источник:** [OpenHands/OpenHands](https://github.com/OpenHands/OpenHands), [README и quickstart](https://github.com/OpenHands/OpenHands/blob/main/README.md), [архитектура Agent Canvas](https://github.com/OpenHands/OpenHands/blob/main/docs/architecture.md), [типизированные action/event contracts](https://github.com/OpenHands/OpenHands/tree/main/src/types/agent-server/core), [Agent Server adapter](https://github.com/OpenHands/OpenHands/blob/main/src/api/agent-server-adapter.ts), [ACP agents](https://github.com/OpenHands/OpenHands/blob/main/docs/ACP_AGENTS.md), [self-hosting/security](https://github.com/OpenHands/OpenHands/blob/main/docs/SELF_HOSTING.md)
- **Дата проверки:** 2026-08-21
- **Ревизия/commit:** `bad1687dec93c5b3edbef837ab2dc12638964031`; `main` совпадает с удалённой веткой; последний commit `fix: render the VSCode button on self-hosted (local) (#16106)`
- **Лицензия:** MIT для репозитория; лицензии зависимостей `@openhands/typescript-client`, `@openhands/extensions`, Electron/React и внешнего Agent Server требуют отдельной проверки при заимствовании компонентов
- **Состав:** Agent Canvas как React/TypeScript frontend и npm package `@openhands/agent-canvas`, typed client для OpenHands Agent Server, локальные/удалённые/cloud backends, optional Automation Server, browser/files/terminal/conversation UI, MCP/skills, ACP subprocess integration и Electron packaging
- **Назначение:** self-hosted control center для запуска и наблюдения coding agents. Текущая ветка — не единый встраиваемый агент-runtime: Canvas переводит пользовательские действия в API отдельного Agent Server, который владеет выполнением, workspace, credentials и sandbox/backend lifecycle.
- **Краткий вывод:** один из самых полезных reference для границы `UI → typed agent backend → execution runtime`: хорошо разделены action/observation/event contracts, approval/status/streaming/condensation signals, capability/version discovery, workspace anchoring и provider/agent adapters. Переносить нужно контракты и инварианты в Rust Core/IPC Евы; React/Node frontend, внешний SDK/runtime, Docker host access и cloud control plane напрямую не подходят.

#### Что изучено

- `docs/architecture.md` прямо ограничивает Canvas: он отображает conversation/terminal/browser/files/settings/automations, управляет frontend state и вызывает Agent Server, но не исполняет agent actions, не предоставляет sandbox и не владеет LLM credentials. Agent Server может быть локальным, удалённым или cloud, а Automation Server вынесен в отдельный сервис.
- Canvas получает runtime-services info от backend и добавляет в agent context URL/назначение сервисов с точки зрения sandbox. Это предотвращает угадывание портов и различия между host view и agent view. В `agent-server-adapter.ts` одновременно видны typed request builder, workspace payload, tool sets, settings separation, profile launch и secret references.
- API-доступ централизован через `@openhands/typescript-client`; отдельный тест запрещает прямые axios/fetch/low-level HTTP calls в source-модулях. Это оформляет SDK adapter как единую точку transport/auth/error handling, а не набор разрозненных HTTP-вызовов из компонентов.
- Event model имеет `id`, `timestamp`, `source` (`agent/user/environment/hook`), kind-discriminated action/observation, `tool_call_id`, `action_id`, tool name и typed payload. `ActionEvent` также хранит thought/reasoning, исходный LLM tool call, `llm_response_id`, security risk, optional critic result и summary.
- Tool surface типизирована отдельно: terminal/bash с timeout, input и reset; file editor с view/create/replace/insert/undo; browser navigation/click/type/state/content/tabs; grep/glob; MCP; task tracker; skill invocation; child task; LLM profile switch и Canvas client tools. Observation хранит output/content, exit code, timeout/error, process metadata, screenshots или structured result.
- Conversation lifecycle представлен отдельными событиями: execution status, token/cost/latency metrics, streaming deltas, pause/server errors, user rejection, hook execution, goal status и condensation с `forgotten_event_ids`/summary. Это позволяет UI быть projection event stream, не читая внутреннее mutable состояние агента.
- Hooks имеют `PreToolUse`/`PostToolUse`/`UserPromptSubmit`/`SessionStart`/`SessionEnd`/`Stop`, success/blocked/exit code/reason, related action/message ID и stdout/stderr. Это полезный observability/policy seam, но не замена Core approval и sandbox.
- Backend registry отделяет список backends от active selection, хранит health/status и выбрасывает stale data при смене backend. При удалении активного backend выбирается валидный fallback; локальные protocol calls не должны занимать cloud backend. Есть version probe и explicit errors для unavailable/unsupported/unknown Agent Server.
- Workspace upload path разрешается относительно `/api/file/home`, а не простым добавлением `/`; абсолютный путь пропускается без изменения. Это исправляет расхождение CWD/worktree и предотвращает запись в read-only filesystem root.
- ACP boundary запускает внешний agent CLI как subprocess и разговаривает JSON-RPC over stdio. Agent Server владеет subprocess и credential store, Canvas отправляет только agent settings и secret references. В контейнере секреты materialize-ятся server/SDK, а не hand-rolled UI-кодом.
- Automations вынесены в backend с расписанием и webhook triggers, отдельными run summaries/activity logs/health и git sync. Canvas показывает конфигурацию и состояние, но не становится scheduler/executor.
- Self-hosting guide честно предупреждает: режим без sandbox даёт агенту полный доступ к filesystem, environment и network. Для public mode предусмотрены firewall, HTTPS/reverse proxy и `LOCAL_BACKEND_API_KEY`; это operational hardening, а не полноценная замена локальному authenticated IPC.

#### Что можем использовать в Еве

- **Core-owned event ledger.** Перенять базовую форму `Event { id, timestamp, source, kind }` и строгие discriminated payloads. Для каждой операции хранить связь `action_id ↔ tool_call_id ↔ observation`, provider/model response ID, session/run ID и monotonic sequence. UI получает replayable stream, а не «текущее состояние из памяти».
- **Typed tool receipts.** Использовать отдельные `ToolCall`/`ToolResult` contracts для shell, filesystem, browser, MCP, search и background jobs. В receipt фиксировать command/arguments hash, scope, timeout, cancellation, exit code, stdout/stderr redaction, artifact references и error class; не передавать произвольный JSON как единственный audit trail.
- **Approval and risk signals.** Адаптировать `security_risk`, confirmation policy, critic/advisory result и `UserRejectObservation` как входы в уже существующий EvoHime approval/call_hash contract. Отказ должен быть terminal auditable event с причиной; модельная оценка риска не может сама разрешить опасное действие.
- **Hook/policy seam.** Ввести Core-owned preflight/postflight hooks для secret redaction, path/network policy, telemetry и validation. `blocked`, `reason`, related action ID и bounded stdout/stderr полезны для объяснения решения, но hook не получает возможность обходить supervisor/sandbox.
- **Capability and version negotiation.** Перенять `/server_info`-подобный typed `CoreInfo`: protocol major, build/runtime revision, available tools, feature flags, limits и service endpoints. Проверять совместимость до запуска сессии, явно различать unavailable/unsupported/unknown и сбрасывать кэш при рестарте Core.
- **Scoped runtime context.** Сформировать из Core подтверждённый `CapabilitySnapshot`/`RuntimeServices` блок: какие workspace paths, local services, browser sessions и network routes доступны именно текущему run. Добавлять его в model context с provenance и revision; запрещать модели угадывать localhost-порты и host paths.
- **Workspace resolver.** Перенять invariant: абсолютный workspace path разрешается только Core, относительно — через canonical data/workspace anchor. Upload, file tools, child tasks и artifacts должны пользоваться одной функцией резолвинга с scope check, а не строить пути в renderer.
- **Provider/agent adapter boundary.** ACP показывает полезную общую идею: внешний agent/provider — отдельный адаптер с typed settings, explicit capability discovery и framed protocol. Для Евы это может стать будущим Core worker/pipe adapter, но credentials должны идти через supervisor environment/DPAPI references, а не через UI или command-line arguments.
- **Condensation как явное событие.** Перенять `Condensation`/summary с перечнем забытых event IDs. Сжатие контекста должно быть отдельной versioned projection с source links и bounded summary, а не незаметным удалением истории; RAG/context ledger могут ссылаться на исходные события.
- **Streaming и lifecycle projections.** Разделить durable events и transient streaming deltas, а также явные `running/paused/waiting_for_confirmation/finished/error/stuck` states. Это пригодится для supervisor restart/reconnect, sequence replay и корректного отображения ожидания approval.
- **Automation contract.** Перенять разделение `AutomationDefinition`, trigger, run, activity log, health и cancellation. Запускать scheduled/webhook work только через Core-owned queue, idempotency key, permission snapshot и approval policy; UI лишь редактирует definition и показывает projection.
- **Backend/target selection invariants.** Идею active backend registry применить к provider/profile/workspace targets: смена цели атомарно меняет query scope, очищает устаревшие projections и не допускает отправки команды в неактивный runtime.
- **Typed adapter tests.** Перенять тест-инвариант «никаких ad-hoc transport calls»: renderer обращается только к Electron/main IPC adapter, main — к authenticated Core protocol, Core — к инструментам. Добавить contract tests для version negotiation, sequence replay, stale session, scope/path resolution и error mapping.

#### Ограничения и риски

- **Это не готовый Rust runtime.** Текущий репозиторий — в основном Canvas; реальное выполнение вынесено в OpenHands Agent Server и software-agent-sdk. Нельзя получить из него автономный Core для Windows или просто заменить EvoHime Rust crates.
- **Host access по умолчанию опасен.** README допускает запуск Agent Server без sandbox и прямо предупреждает о полном доступе к filesystem. Docker/VM deployment и `PROJECTS_PATH` — operational patterns, не эквивалент EvoHime Job Object, ACL, named pipe HMAC и Core-owned policy.
- **HTTP/public control plane.** API key, reverse proxy, firewall и HTTPS защищают self-hosted HTTP deployment, но не дают той же локальной границы, что authenticated `desktop-ipc-v1`. В Еве нельзя заменять локальный IPC localhost API только ради совместимости.
- **Credential isolation ещё не идеальна.** ACP guide отдельно отмечает race shared HOME между concurrent same-provider conversations и пока не включает `acp_isolate_data_dir` из-за SDK compatibility. Это сигнал не копировать общий process/home state без per-run isolation.
- **Broad tool surface.** Terminal, file editor, browser, MCP, skills, child agents и automation резко расширяют attack surface. В Еве каждую capability нужно включать permission-gated, scoped и cancellable; наличие typed schema само по себе не является sandbox.
- **Frontend state is not durable truth.** Zustand/React Query stores и backend registry удобны как UI projection, но не должны заменить Rust Core/SQLite. Canvas хранит local metadata и active selection, тогда как durable execution/audit state должен принадлежать Core.
- **Context injection can become prompt authority.** Runtime-services suffix помогает агенту ориентироваться, но model-readable text не является доказательством разрешения. Paths, endpoints, secrets и capabilities нужно проверять повторно в Core на каждой операции.
- **Automation/cloud coupling.** Slack/GitHub/Linear, cloud sandboxes, remote backends и organization flows требуют сетевого egress, auth, retention и multi-tenant policy. Для local-first Евы это отдельные permissioned integrations, не базовый runtime.
- **Version/API drift.** Canvas зависит от конкретной версии `@openhands/typescript-client` и minimum Agent Server version; profile path, client tools и ACP fields меняются независимо. Заимствование wire shapes без собственного schema versioning создаст хрупкую совместимость.
- **License surface.** MIT разрешает рассматривать код как reference, но bundled dependencies, SDK, extensions, browser/editor components и внешние runtimes требуют отдельного inventory/NOTICE. Не переносить копипастой код до проверки лицензий и security history.
- **Тесты не доказывают EvoHime security.** У проекта сильная frontend/API test surface, но эти тесты не заменяют Windows process containment, supervisor recovery, DPAPI, IPC proof, SQLite migration, cancellation и local egress tests Евы.

#### Предварительное решение

`адаптировать` Core/agent boundary, canonical action-observation event ledger, typed tool receipts, approval/risk/rejection events, hook seam, capability/version negotiation, runtime-services context, canonical workspace resolver, explicit condensation/streaming lifecycle, provider adapter и automation run contracts; `наблюдать` ACP и multi-backend patterns как design reference; `не подключать` Agent Canvas frontend как источник runtime state, внешний Python/software-agent-sdk runtime, Docker host access, public HTTP API key model, cloud automation control plane и unbounded tool surface в EvoHime Core.

#### Связь с EvoHime

- Текущая схема `Electron → authenticated IPC → Rust Core → SQLite → supervisor` уже соответствует наиболее сильной идее OpenHands: UI — control/projection layer, execution runtime — отдельная доверенная граница. OpenHands подсказывает, какие поля и состояния должны быть видны в этой границе, но не требует менять продуктовую архитектуру.
- Возможные typed сущности для будущего плана: `AgentEvent`, `ActionRequest`, `ToolReceipt`, `Observation`, `ApprovalRequest`, `RejectionEvent`, `HookReceipt`, `CapabilitySnapshot`, `CoreInfo`, `RuntimeService`, `CondensationRecord`, `ProviderAdapter`, `AutomationDefinition` и `AutomationRun`.
- Первый полезный offline fixture: Core принимает typed tool request → проверяет capability/path/risk → создаёт approval event → после решения запускает sandboxed tool → пишет redacted receipt → отдаёт sequence replay после reconnect. Отдельно проверить condensation с `forgotten_event_ids`, stale Core revision и невозможность renderer вызвать tool напрямую.
- ACP-подобный внешний worker следует рассматривать только после завершения базового authenticated IPC/approval/sandbox контракта. Даже тогда worker получает capability-scoped session и secret references, а не прямой доступ к SQLite, supervisor secrets или произвольному workspace.
- Критерии готовности: event IDs и pair links; major-version negotiation; canonical path/scope; per-run capability snapshot; approval/rejection receipts; cancellation/timeout; redaction; durable sequence replay; atomic SQLite writes; supervisor restart recovery; отсутствие ad-hoc transport calls и model-generated authority over host actions.
