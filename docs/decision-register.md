...
Закрытые планы `01–147` и `144` не являются текущей очередью и представлены
только перенесёнными контрактами. Незавершённый каталог — планы `148–167` и
`148–167`; порядок выбирается по blocking dependencies в их overview, а не по
