...
В `score_item`: сейчас все experience-kind (`SuccessPattern | FailurePattern | VerificationRule | Playbook`) получают одинаковый `+0.3`. Добавляется дополнительный `+0.05` конкретно для `FailurePattern | VerificationRule` (итого `+0.35` против `+0.3` у `SuccessPattern`/`Playbook`), так что при равной лексической/семантической релевантности урок о провале ранжируется выше внутри experience-группы. Значение намеренно держится ниже 0.1: реализация вскрыла, что существующий тест `active_fact_outranks_experience_candidate_when_query_ties` держится на марже ровно 0.1 между active-статусом (+0.5) и candidate+experience (+0.4) — бонус `+0.2` эту маржу перекрывал и давал candidate `failure_pattern` (который почти всегда остаётся Candidate, так как auto-promote из провала невозможен) обгонять несвязанный active `Fact` при точном совпадении запроса. `+0.05` сохраняет обе гарантии: лессоны выше success_pattern/playbook внутри experience, но не выше active fact вовне. В сочетании с ростом `importance` от эскалации (`score += item.importance` уже в формуле) — оба пункта roadmap закрываются одним изменением скоринга плюс эскалацией.
