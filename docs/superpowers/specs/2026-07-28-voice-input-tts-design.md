...
`stop()` завершает recognition и резолвится только после `onend`, включая последний финальный результат. Это является обязательной синхронизацией для отправки сообщения во время записи.
...
Реализация TTS использует общий менеджер `speechSynthesis`, поэтому запуск другого сообщения вызывает `cancel()` для предыдущего utterance. `SpeechSynthesisUtterance` обрабатывает `end` и `error`, чтобы сбросить `isSpeaking` и показать безопасное состояние ошибки.
...
- Пока идёт запись, поле composer `readOnly`; ручное редактирование и перемещение курсора не выполняются.
...
- SpeechRecognition не поддерживается в iOS Safari; на iPhone кнопка микрофона остаётся disabled, а обычный ручной ввод продолжает работать.
