...
$allowed = @('evohime-updater.exe', 'evohime-transaction.exe', 'evohime.bootstrap.json', 'updater')
...
foreach ($required in @('evohime-updater.exe', 'evohime-transaction.exe', 'evohime.bootstrap.json', 'updater\EvoHimeUpdater.exe', 'updater\resources\app.asar')) {
...
if (@($marker.requiredFiles).Count -ne 4) { throw 'Bootstrap marker requiredFiles is incomplete.' }
if ($SetupPath) {
