...
import type {
  ...
  OutputGuardrailPipelineProjection,
...
export class CorePipeClient extends EventEmitter<PipeClientEvents> {
  ...
  private emitCoreEvent(event: evohime.desktop.v1.EventEnvelope): void {
    this.emit('core-event', {
      ...
      , outputGuardrailPipeline: decodeOutputGuardrailPipeline(event.outputGuardrailPipeline)
    })
  }
...
function decodeMessageInterventionPolicies(projected: evohime.desktop.v1.IMessageInterventionPoliciesEvent | null | undefined): MessageInterventionPoliciesProjection | null {
  if (!projected) return null
  const raw = decodePayload(projected.projectionJson)
  let projection: unknown = null
  try { projection = JSON.parse(raw) } catch { projection = null }
...
function decodePersistentAgentOrganizationRegistry(projected: evohime.desktop.v1.IPersistentAgentOrganizationRegistryEvent | null | undefined): PersistentAgentOrganizationRegistryProjection | null { if (!projected) return null; const raw=decodePayload(projected.projectionJson); let projection: unknown=null; try { projection=JSON.parse(raw) } catch { projection=null }; return { schemaVersion:Number(projected.schemaVersion??0), requestId:projected.requestId??'', agentId:projected.agentId??'', operation:projected.operation??'', revision:Number(projected.revision??0), status:projected.status??'', errorCode:projected.errorCode??'', projection } }
...
function decodeOutputGuardrailPipeline(projected: evohime.desktop.v1.IOutputGuardrailPipelineEvent | null | undefined): OutputGuardrailPipelineProjection | null { if (!projected) return null; let projection: unknown = null; try { projection = JSON.parse(Buffer.from(projected.projectionJson ?? new Uint8Array()).toString('utf8')) } catch { projection = null }; return { schemaVersion: projected.schemaVersion ?? 1, pipelineId: projected.pipelineId ?? '', operation: projected.operation ?? '', version: Number(projected.version ?? 0), status: projected.status ?? '', errorCode: projected.errorCode ?? '', projection } }
