...
// Ядро больше не режет ответ рецензента — вывод не ограничен. Это лишь
// эвристическая прикидка «типичного» объёма ответа для предупреждения о том,
// что синтез может не влезть в окно модели; реальный ответ может быть длиннее.
...
// Ответу нужно место в том же окне; резерв на выход отдельной модели-рецензента,
// ограниченный сверху разумной прикидкой (реальный вывод ничем не ограничен).
 ...
 * рецензентов; вывод рецензента ничем не ограничен, поэтому размер
 * `MAX_REVIEW_OUTPUT_BYTES` — лишь эвристическая прикидка на рецензента.
 * Первое можно проверить точно и потому блокируется; второе зависит от того,
 * насколько многословными окажутся рецензенты, поэтому остаётся предупреждением.
 */
function preflight(
