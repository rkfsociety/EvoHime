...
9. `git push` выполняй как `GIT_TERMINAL_PROMPT=0 git push origin main`: без этого флага credential manager может ждать ввода, недоступного в неинтерактивной сессии, и команда виснет до таймаута. Результат подтверждай сравнением `git ls-remote origin main` с `git rev-parse HEAD`, а не выводом самой команды — прерванный по таймауту push ничего не доказывает.
