...
Все планы лежат в `docs/plans/`; порядок и правило нумерации описаны в `docs/plans/README.md`. Новый план создаётся файлом в этом каталоге, а не в корне `docs/`, получает следующий свободный номер и обязан разделить зависимости на блокирующие и опциональные. Блокирующая зависимость от плана с большим номером означает, что номера расставлены неверно.
