...
4. Полное локальное тестирование запрещено: не запускай `cargo test` без
   ограничения затронутыми пакетами, `cargo test --workspace`, полный набор
   Electron-тестов или эквивалентный прогон всего проекта. На рабочей машине
   запускай только узкие проверки изменённых crates/модулей. Полный прогон
   выполняется CI либо только по отдельному прямому запросу Романа.
