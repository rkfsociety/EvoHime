...
### Обязательный режим работы через Еву

Все улучшения EvoHime, исправления его кода, изменения инструментов, prompt-ов и native-архитектуры сначала выполняются самой Евой через штатный Core и его инструменты. Если Ева не смогла выполнить задачу, не подменяй её ручной реализацией: зафиксируй причину, настрой prompt, модель, parser, delivery-gate или инструментальный контур и повтори попытку через Еву. Повторяй такой цикл до успешного выполнения поставленной задачи. Каждая отдельная попытка должна иметь конечный timeout и оставлять trace; timeout означает начало следующей исправленной попытки, а не успешное завершение.
