...
pub async fn execute(
    ...
    match name {
        BASE64_ENCODE => {
            ...
            result(
                name,
                json!({"encoding":"base64","value":value}),
                value,
                "pure_deterministic",
            )
        }
        BASE64_DECODE => {
            let text = text_input(name, &input)?;
            let bytes = STANDARD.decode(text).map_err(|_| ToolError::InvalidInput {
                tool: name.into(),
                message: "invalid_encoding".into(),
            })?;
            let value = String::from_utf8(bytes).map_err(|_| ToolError::InvalidInput {
                tool: name.into(),
                message: "invalid_utf8".into(),
            })?;
            result(
                name,
        ...
        HASH_SHA512 => {
            ...
            result(
                name,
                json!({"algorithm":"sha512","digest":digest}),
                digest,
                "pure_deterministic",
            )
        }
        JSON_FORMAT | JSON_MINIFY => {
            let text = text_input(name, &input)?;
            let parsed: Value =
                serde_json::from_str(text).map_err(|error| ToolError::InvalidInput {
                    tool: name.into(),
                    message: format!("parse_error:{}", error.line()),
                })?;
        ...
        UUID_V4 => {
            if !input.as_object().is_some_and(|object| object.is_empty()) {
                return Err(ToolError::InvalidInput {
                    tool: name.into(),
                    message: "no_options_allowed".into(),
                });
            }
            let value = Uuid::new_v4().to_string();
            result(
                name,
                json!({"value":value,"random":true}),
                value,
                "secure_random",
            )
        }
        TOKEN_GENERATE => {
            ...
            if !(1..=128).contains(&length)
                || input
                    .as_object()
                    .map(|object| object.keys().any(|key| key != "bytes"))
                    .unwrap_or(true)
            {
                return Err(ToolError::InvalidInput {
                    tool: name.into(),
                    message: "invalid_byte_length".into(),
                });
            }
            ...
            result(
                name,
                json!({"value":value,"bytes":length,"telemetry":"redacted"}),
                value,
                "secure_random",
            )
        }
...
mod tests {
    ...
    async fn pure_operations_are_stable_and_bounded() {
        let first = execute(&context(), HASH_SHA256, json!({"text":"hello"}))
            .await
            .unwrap();
        let second = execute(&context(), HASH_SHA256, json!({"text":"hello"}))
            .await
            .unwrap();
    ...
    async fn malformed_and_oversized_inputs_fail_closed() {
        assert!(execute(&context(), BASE64_DECODE, json!({"text":"%%%"}))
            .await
            .is_err());
        assert!(execute(
            &context(),
            HASH_SHA256,
            json!({"text":"x".repeat(MAX_INPUT_BYTES + 1)})
        )
        .await
        .is_err());
        assert!(execute(&context(), JSON_FORMAT, json!({"text":"{"}))
            .await
            .is_err());
    }
    ...
    async fn random_outputs_are_marked_and_not_reused_as_deterministic() {
        let value = execute(&context(), TOKEN_GENERATE, json!({"bytes":16}))
            .await
            .unwrap();
