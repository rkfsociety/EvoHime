...
impl ProviderSessionCapability {
    ...
    pub fn redeem(&mut self, token: &[u8], request_id: &str, audience: &str, now_ms: u64) -> Result<(), ProviderError> {
        ...
        if self.redeemed || now_ms > self.expires_at_ms || request_id != self.request_id || audience != self.audience || !constant_time_eq(actual.as_slice(), &self.token_hash) {
...
impl LocalProvider {
    pub fn new(config: LiteRouterConfig) -> Result<Self, ProviderError> {
        ...
        let capability = LocalCapabilityMetadata { schema_version: "local-capability-v1".into(), model_id: config.model.clone(), capability_epoch: 1, cancellation: true, tools: Vec::new() };
    ...
    pub fn validate_capability(capability: &LocalCapabilityMetadata) -> Result<(), ProviderError> {
        if capability.schema_version.split('-').last() != Some("v1") || capability.model_id.trim().is_empty() || capability.capability_epoch == 0 { return Err(ProviderError::Config("capability_probe_failed".into())); }
        let mut names = std::collections::BTreeSet::new();
        for tool in &capability.tools {
            if tool.name.trim().is_empty() || !names.insert(&tool.name) || !tool.arguments_schema.is_object() || (tool.requires_approval && tool.version.trim().is_empty()) { return Err(ProviderError::Config("capability_probe_failed".into())); }
        }
        Ok(())
    }

    pub fn validate_tool_call(&self, call: &NativeToolCall) -> Result<(), ProviderError> {
        if call.id.trim().is_empty() || call.name.trim().is_empty() { return Err(ProviderError::Config("tool_call_malformed".into())); }
        let args: serde_json::Value = serde_json::from_str(&call.arguments).map_err(|_| ProviderError::Config("tool_call_malformed".into()))?;
        if !args.is_object() { return Err(ProviderError::Config("tool_call_malformed".into())); }
        let Some(capability) = self.capability.tools.iter().find(|tool| tool.name == call.name) else { return Err(ProviderError::Config("tool_call_malformed".into())); };
        if capability.requires_approval && capability.version.is_empty() { return Err(ProviderError::Config("tool_call_malformed".into())); }
        if !schema_accepts_object(&capability.arguments_schema, &args) { return Err(ProviderError::Config("tool_call_malformed".into())); }
        Ok(())
    }
...
fn schema_accepts_object(schema: &serde_json::Value, value: &serde_json::Value) -> bool {
    ...
    if schema.get("type").and_then(serde_json::Value::as_str).is_some_and(|kind| kind != "object") { return false; }
    if let Some(required) = schema.get("required").and_then(serde_json::Value::as_array) {
        if required.iter().filter_map(serde_json::Value::as_str).any(|name| !object.contains_key(name)) { return false; }
    }
        ...
        for (name, definition) in properties {
            if let Some(actual) = object.get(name) {
                if let Some(kind) = definition.get("type").and_then(serde_json::Value::as_str) {
                    let valid = match kind { "string" => actual.is_string(), "boolean" => actual.is_boolean(), "number" | "integer" => actual.is_number(), "object" => actual.is_object(), "array" => actual.is_array(), _ => true };
...
mod tests {
    ...
    fn malformed_and_unknown_tool_calls_are_rejected() {
        let provider = LocalProvider::new(LiteRouterConfig { api_key: "cap".into(), base_url: "http://127.0.0.1:1/v1".into(), model: "slm".into() }).unwrap();
        assert!(provider.validate_tool_call(&NativeToolCall { id: "".into(), name: "x".into(), arguments: "{}".into() }).is_err());
        assert!(provider.validate_tool_call(&NativeToolCall { id: "1".into(), name: "x".into(), arguments: "[]".into() }).is_err());
    }
}
