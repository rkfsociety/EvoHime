...
pub fn save(
    ...
    let prior:Option<(u64,String)>=c.query_row("SELECT revision,content_hash FROM project_knowledge_notebook WHERE notebook_id=?1 AND idempotency_key=?2",params![id,key],|r|Ok((r.get(0)?,r.get(1)?))).optional()?;
    if let Some((r, h)) = prior {
        if r == revision && h == hash {
            return Ok(());
        }
        return Err(rusqlite::Error::InvalidParameterName(
            "notebook idempotency conflict".into(),
        ));
    }
    let current: Option<u64> = c
        .query_row(
            "SELECT MAX(revision) FROM project_knowledge_notebook WHERE notebook_id=?1",
            params![id],
            |r| r.get(0),
        )
        .optional()?
    ...
    if revision != current.unwrap_or(0) + 1 {
        return Err(rusqlite::Error::InvalidParameterName(
            "notebook revision conflict".into(),
        ));
    }
    c.execute(
        "INSERT INTO project_knowledge_notebook VALUES(?1,?2,?3,?4,?5,?6)",
...
pub fn pin(
    ...
    let prior:Option<(String,u64,String)>=c.query_row("SELECT notebook_id,revision,content_hash FROM project_knowledge_notebook_pin WHERE run_id=?1",params![run_id],|r|Ok((r.get(0)?,r.get(1)?,r.get(2)?))).optional()?;
    if let Some((pid, pr, ph)) = prior {
        if pid == id && pr == revision && ph == hash {
            return Ok(());
        }
        return Err(rusqlite::Error::InvalidParameterName(
            "notebook run already pinned".into(),
        ));
    }
    c.execute(
        "INSERT INTO project_knowledge_notebook_pin VALUES(?1,?2,?3,?4,?5)",
