...
pub fn install_schema(c: &Connection) -> rusqlite::Result<()> {
    for (name, definition) in [
        ("background_kind", "TEXT NOT NULL DEFAULT 'registered_background_task'"),
        ("background_snapshot_json", "BLOB NOT NULL DEFAULT '{}'"),
        ("background_next_wakeup_at_ms", "INTEGER"),
        ("background_queue_ref", "TEXT NOT NULL DEFAULT 'workflow-default'"),
        ("background_concurrency_key", "TEXT"),
        ("background_priority", "TEXT NOT NULL DEFAULT 'background_workflow'"),
        ...
        if !exists {
            c.execute(&format!("ALTER TABLE automation_runs ADD COLUMN {name} {definition}"), [])?;
        }
    }
    c.execute_batch(
...
fn bounded(bytes: &[u8]) -> bool { !bytes.is_empty() && bytes.len() <= MAX_JSON_BYTES }
...
pub fn get_queue(c: &Connection, queue_id: &str, owner_scope: &str) -> rusqlite::Result<Option<QueueRecord>> {
    c.query_row(
...
pub fn queue_load(c: &Connection, queue_id: &str, owner_scope: &str) -> rusqlite::Result<(u32, u32)> {
    c.query_row(
...
pub fn drop_oldest_queued(c: &mut Connection, queue_id: &str, owner_scope: &str, now_ms: i64) -> rusqlite::Result<Option<String>> {
    ...
    let generation = crate::automation_store::get_run(c, &run_id)?.map(|run| run.generation).unwrap_or(0);
    let changed = crate::automation_store::transition_run(c, crate::automation_store::RunTransition { run_id: &run_id, from_state: "queued", to_state: "dead_lettered", generation, event_type: "background.queue_overflow", payload_json: "{}", now_ms })?;
    Ok(changed.then_some(run_id))
}

pub fn find_queued_by_concurrency_key(c: &Connection, queue_id: &str, owner_scope: &str, concurrency_key: &str) -> rusqlite::Result<Option<String>> {
    c.query_row(
...
pub fn find_active_by_concurrency_key(c: &Connection, queue_id: &str, owner_scope: &str, concurrency_key: &str) -> rusqlite::Result<Option<String>> {
    c.query_row(
...
pub fn save_queue(c: &Connection, record: &QueueRecord, now_ms: i64) -> rusqlite::Result<bool> {
    if record.queue_id.is_empty() || record.owner_scope.is_empty() || record.revision == 0 || record.max_active == 0 || record.max_queued > MAX_QUEUE_ROWS || record.content_hash.is_empty() { return Ok(false); }
    Ok(c.execute(
...
pub fn list_queues(c: &Connection, owner_scope: &str, limit: u32) -> rusqlite::Result<Vec<QueueRecord>> {
    ...
    s.query_map(params![owner_scope, limit.clamp(1, 256)], |row| Ok(QueueRecord { queue_id: row.get(0)?, owner_scope: row.get(1)?, revision: row.get::<_, i64>(2)? as u64, max_active: row.get::<_, i64>(3)? as u32, max_queued: row.get::<_, i64>(4)? as u32, priority: row.get(5)?, overflow_policy: row.get(6)?, content_hash: row.get(7)? }))?.collect()
}

pub fn put_wait(c: &mut Connection, record: &WaitRecord, now_ms: i64) -> rusqlite::Result<bool> {
    if !bounded(&record.condition_json) || record.run_id.is_empty() || record.revision == 0 { return Ok(false); }
    ...
    if let Some(wake) = record.wake_at_ms { tx.execute("INSERT OR REPLACE INTO automation_wakeups(wake_key,run_id,wake_at_ms,kind,active,created_at_ms) VALUES(?1,?2,?3,'wait',1,?4)", params![format!("wait:{}:{}", record.run_id, record.revision), record.run_id, wake, now_ms])?; }
    tx.commit()?;
    Ok(true)
}

pub fn due_wakeups(c: &Connection, now_ms: i64, limit: u32) -> rusqlite::Result<Vec<(String, String)>> {
    ...
    s.query_map(params![now_ms, limit.clamp(1, 256)], |row| Ok((row.get(0)?, row.get(1)?)))?.collect()
}
...
pub fn list_waits(c: &Connection, limit: u32) -> rusqlite::Result<Vec<WaitRecord>> {
    ...
    s.query_map([limit.clamp(1, 256)], |row| Ok(WaitRecord { run_id: row.get(0)?, revision: row.get::<_, i64>(1)? as u64, condition_json: row.get(2)?, wake_at_ms: row.get(3)?, state: row.get(4)? }))?.collect()
}

pub fn complete_wait(c: &Connection, run_id: &str, revision: u64, now_ms: i64) -> rusqlite::Result<bool> {
    ...
    c.execute("UPDATE automation_wakeups SET active=0,created_at_ms=?1 WHERE run_id=?2 AND active=1", params![now_ms, run_id])?;
    Ok(changed == 1)
}

pub fn mark_wakeup_consumed(c: &Connection, wake_key: &str, now_ms: i64) -> rusqlite::Result<bool> {
    Ok(c.execute("UPDATE automation_wakeups SET active=0,created_at_ms=?1 WHERE wake_key=?2 AND active=1", params![now_ms, wake_key])? == 1)
}

pub fn insert_attempt(c: &Connection, attempt: &AttemptRecord, now_ms: i64) -> rusqlite::Result<bool> {
    if attempt.attempt_id.is_empty() || attempt.run_id.is_empty() || attempt.dispatcher_id.is_empty() || attempt.generation == 0 { return Ok(false); }
...
pub fn list_attempts(c: &Connection, run_id: &str, limit: u32) -> rusqlite::Result<Vec<AttemptRecord>> {
    ...
    s.query_map(params![run_id, limit.clamp(1, 256)], |row| Ok(AttemptRecord { attempt_id: row.get(0)?, run_id: row.get(1)?, generation: row.get::<_, i64>(2)? as u64, dispatcher_id: row.get(3)?, state: row.get(4)?, outcome_code: row.get(5)?, started_at_ms: row.get(6)?, ended_at_ms: row.get(7)? }))?.collect()
}
...
pub fn background_snapshot(c: &Connection, run_id: &str) -> rusqlite::Result<Option<Vec<u8>>> {
    c.query_row("SELECT background_snapshot_json FROM automation_runs WHERE run_id=?1", [run_id], |row| row.get(0)).optional()
}
...
pub fn save_background_schedule(
    ...
    if spec_json.is_empty() || spec_json.len() > MAX_JSON_BYTES || missed_fire_policy.is_empty() || overlap_policy.is_empty() {
        return Ok(false);
    }
...
pub fn list_background_schedules(c: &Connection, owner_scope: &str, limit: u32) -> rusqlite::Result<Vec<BackgroundScheduleRecord>> {
    ...
    statement.query_map(params![owner_scope, limit.clamp(1, 256)], |row| Ok(BackgroundScheduleRecord {
        schedule_id: row.get(0)?, definition_id: row.get(1)?, revision: row.get::<_, i64>(2)? as u64,
        owner_scope: row.get(3)?, spec_json: row.get(4)?, missed_fire_policy: row.get(5)?, overlap_policy: row.get(6)?, enabled: row.get::<_, i64>(7)? != 0,
        last_slot: row.get(8)?, hour: row.get::<_, i64>(9)? as u8, minute: row.get::<_, i64>(10)? as u8, timezone_minutes: row.get(11)?,
    }))?.collect()
}
...
mod tests {
    ...
    fn metadata_and_waits_are_bounded_and_idempotent() {
        ...
        automation_store::insert_definition(&c, &AutomationDefinitionRecord { definition_id: "d".into(), revision: 1, owner_scope: "o".into(), definition_json: "{}".into(), definition_hash: "h".into() }, 1).unwrap();
        let run = automation_store::AutomationRunRecord { run_id: "r".into(), definition_id: "d".into(), revision: 1, owner_scope: "o".into(), idempotency_key: "k".into(), payload_hash: "p".into(), state: "accepted".into(), generation: 1, permission_snapshot: "p".into(), approval_snapshot: "a".into() };
        automation_store::insert_run(&c, &run, 1).unwrap();
        assert!(save_run_metadata(&c, "r", "workflow_run", br#"{}"#, "q", "background_workflow", None, "hash", Some(10), 1).unwrap());
        let mut c = c;
        assert!(put_wait(&mut c, &WaitRecord { run_id: "r".into(), revision: 1, condition_json: br#"{}"#.to_vec(), wake_at_ms: Some(10), state: "waiting".into() }, 1).unwrap());
        assert_eq!(due_wakeups(&c, 10, 10).unwrap().len(), 1);
        assert!(save_queue(&c, &QueueRecord { queue_id: "q".into(), owner_scope: "o".into(), revision: 1, max_active: 2, max_queued: 4, priority: "background_workflow".into(), overflow_policy: "reject_new".into(), content_hash: "hash".into() }, 1).unwrap());
        assert_eq!(list_queues(&c, "o", 10).unwrap().len(), 1);
        let attempt = AttemptRecord { attempt_id: "a".into(), run_id: "r".into(), generation: 1, dispatcher_id: "test".into(), state: "blocked".into(), outcome_code: "runtime_adapter_unavailable".into(), started_at_ms: Some(1), ended_at_ms: Some(1) };
