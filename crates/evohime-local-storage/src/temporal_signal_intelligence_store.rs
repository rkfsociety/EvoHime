...
pub fn save(
    ...
    let old:Option<(u64,String)>=c.query_row("SELECT revision,content_hash FROM temporal_signal_intelligence WHERE id=?1 AND idempotency_key=?2",params![id,k],|x|Ok((x.get(0)?,x.get(1)?))).optional()?;
    ...
    c.execute(
        "INSERT INTO temporal_signal_intelligence VALUES(?1,?2,?3,?4,?5,?6)",
        params![id, r, h, j, k, n],
    )?;
    Ok(())
}
