...
pub fn install_schema(connection: &Connection) -> rusqlite::Result<()> {
    connection.execute_batch(
        ...
        CREATE TABLE IF NOT EXISTS visual_workflow_versions (
            ...
            definition_json BLOB NOT NULL, execution_hash TEXT NOT NULL, created_at_ms INTEGER NOT NULL,
...
pub fn save_draft(
    ...
    tx.execute(
        "INSERT INTO visual_workflow_drafts(draft_id,owner_scope,revision,state,definition_json,layout_json,execution_hash,layout_hash,updated_at_ms) VALUES(?1,?2,?3,'valid',?4,?5,?6,?7,?8) ON CONFLICT(draft_id) DO UPDATE SET owner_scope=excluded.owner_scope,revision=excluded.revision,state='valid',definition_json=excluded.definition_json,layout_json=excluded.layout_json,execution_hash=excluded.execution_hash,layout_hash=excluded.layout_hash,updated_at_ms=excluded.updated_at_ms",
        rusqlite::params![input.draft_id, input.owner_scope, revision, input.definition_json, input.layout_json, input.execution_hash, input.layout_hash, input.updated_at_ms],
    )?;
    tx.commit()?;
    Ok(Ok(revision))
}
...
pub fn publish_from_handoff(
    ...
    let version = definition
        ...
        .ok_or(rusqlite::Error::InvalidQuery)?;
    tx.execute("INSERT INTO visual_workflow_versions(graph_id,version,owner_scope,definition_json,execution_hash,created_at_ms) VALUES(?1,?2,?3,?4,?5,?6)", rusqlite::params![graph_id,version,owner_scope,row.1,row.2,created_at_ms])?;
    tx.commit()?;
    Ok(Ok(row))
}
