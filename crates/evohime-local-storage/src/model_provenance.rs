...
pub fn install_schema(connection: &Connection) -> Result<()> {
    connection.execute_batch(
        "PRAGMA foreign_keys=ON;
         ...
         CREATE TABLE IF NOT EXISTS model_responses (
           ...
           status TEXT NOT NULL CHECK(status IN ('complete','interrupted','failed')), output TEXT, output_hash TEXT,
    ...
    let _ = connection.execute("ALTER TABLE context_ledger_receipts ADD COLUMN request_id TEXT", []);
    let _ = connection.execute("ALTER TABLE context_ledger_receipts ADD COLUMN request_envelope_hash TEXT", []);
    let _ = connection.execute("ALTER TABLE context_ledger_receipts ADD COLUMN receipt_domain TEXT", []);
    let _ = connection.execute("ALTER TABLE context_ledger_receipts ADD COLUMN receipt_type TEXT", []);
...
impl<'a> ModelProvenanceRepository<'a> {
    ...
    pub fn commit_envelope(&self, envelope: &ModelRequestEnvelopeV1, mode: CommitMode) -> Result<ModelRequestRecord> {
        envelope.validate()?;
        let bytes = envelope.canonical_bytes()?;
        validate_no_credentials(&serde_json::to_value(envelope)?)?;
        let envelope_hash = match mode { CommitMode::FullForDispatch => Some(envelope.envelope_hash()?), CommitMode::HashOnlyStorage => None };
        if matches!(mode, CommitMode::FullForDispatch) && bytes.len() > MAX_REQUEST_ENVELOPE_BYTES { return Err(ModelProvenanceError::CommitFailed("full payload missing".into())); }
        let tx = self.connection.unchecked_transaction()?;
        let result = commit_tx(&tx, envelope, &bytes, envelope_hash.as_deref(), mode);
        match result { Ok(record) => { tx.commit()?; Ok(record) }, Err(error) => Err(error) }
    }
    ...
    pub fn mark_dispatch(&self, request_id: &str, at: i64) -> Result<()> {
        ...
        if changed != 1 { return Err(ModelProvenanceError::CommitFailed("dispatch marker was not committed".into())); }
        Ok(())
    }
    ...
    pub fn link_request_receipt(
        ...
        if request.1 != receipt.request_envelope_hash || canonical_payload.is_empty() {
            return Err(ModelProvenanceError::Contract(ProvenanceError::ReceiptLinkageMismatch));
        }
        let payload: ModelRequestReceiptV1 = serde_json::from_slice(canonical_payload)?;
        payload.validate()?;
        if payload.request_id != receipt.request_id || payload.request_envelope_hash != receipt.request_envelope_hash {
            return Err(ModelProvenanceError::Contract(ProvenanceError::ReceiptLinkageMismatch));
        }
    ...
    pub fn set_status(&self, request_id: &str, status: RequestStatus, completed_at: Option<i64>) -> Result<()> {
        let status = serde_json::to_string(&status)?.trim_matches('"').to_owned();
        if matches!(status.as_str(), "redacted" | "retention_pruned") { return self.redact_request(request_id, &status); }
    ...
    pub fn capture_workspace_evidence(
        ...
        for _ in 0..3 {
            ...
            if changed != 1 { return Err(ProvenanceError::SourceMissing.into()); }
    ...
    pub fn insert_tool_intent(&self, intent: &ToolIntentRecord) -> Result<()> {
        let request_hash: String = self.connection.query_row("SELECT envelope_hash FROM model_requests WHERE request_id=?1", [&intent.origin_request_id], |row| row.get(0))?;
        if request_hash != intent.origin_request_envelope_hash { return Err(ModelProvenanceError::Contract(ProvenanceError::ToolLinkageMismatch)); }
        if let Some(response_id) = &intent.response_id {
            let owner: String = self.connection.query_row("SELECT request_id FROM model_responses WHERE response_id=?1", [response_id], |row| row.get(0))?;
            if owner != intent.origin_request_id { return Err(ModelProvenanceError::Contract(ProvenanceError::ToolLinkageMismatch)); }
        }
    ...
    pub fn append_shadow_original(&self, record: &ShadowOriginalRecord, bytes: Option<&[u8]>) -> Result<()> {
        if record.operation != "summary" && record.operation != "prune" {
            return Err(ModelProvenanceError::Contract(ProvenanceError::Invalid("shadow operation".into())));
        }
        if bytes.is_some_and(|value| hash_bytes_bytes(value) != record.original_content_hash.clone().unwrap_or_default()) {
            return Err(ModelProvenanceError::Contract(ProvenanceError::HashMismatch));
        }
        let tx = self.connection.unchecked_transaction()?;
    ...
    pub fn list_shadow_originals(&self, request_id: &str, limit: usize) -> Result<Vec<ShadowOriginalRecord>> {
        ...
        let rows = statement.query_map(params![request_id, limit.min(4096) as i64], |row| Ok(ShadowOriginalRecord { shadow_id: row.get(0)?, ledger_id: row.get(1)?, request_id: row.get(2)?, original_kind: row.get(3)?, original_id: row.get(4)?, operation: row.get(5)?, parent_shadow_id: row.get(6)?, content_block_hash: row.get(7)?, source_state: row.get(8)?, original_content_hash: row.get(9)?, byte_len: row.get::<_, i64>(10)? as u64, created_at: row.get(11)? }))?.collect::<std::result::Result<Vec<_>, _>>()?;
        Ok(rows)
    }
    ...
    pub fn compact_shadow_for_task(&self, task_id: &str) -> Result<usize> {
        ...
        if total <= evohime_model_provenance::MAX_SHADOW_BYTES_PER_TASK as i64 { return Ok(0); }
        ...
        let candidates = statement.query_map([task_id], |row| Ok((row.get::<_, String>(0)?, row.get::<_, String>(1)?, row.get::<_, i64>(2)? as usize)))?.collect::<std::result::Result<Vec<_>, _>>()?;
        let mut changed = 0;
        for (shadow_id, hash, size) in candidates {
            if remaining <= evohime_model_provenance::MAX_SHADOW_BYTES_PER_TASK { break; }
            ...
            self.connection.execute("UPDATE context_shadow_blocks SET bytes=NULL WHERE content_hash=?1", [&hash])?;
    ...
    pub fn recover_active(&self) -> Result<usize> {
        let ids: Vec<String> = {
            ...
            let rows = statement.query_map([], |row| row.get(0))?.collect::<std::result::Result<Vec<_>, _>>()?;
            rows
        };
        let mut count = 0;
        for id in ids {
            ...
            let (status, completed) = match response {
                Some((status, _, at)) if status == "complete" => ("completed", at.or_else(now_ms)),
                Some((status, _, at)) if status == "failed" => ("failed", at.or_else(now_ms)),
                Some((status, _, at)) if status == "interrupted" => ("interrupted", at.or_else(now_ms)),
                None => {
                    let dispatch: Option<i64> = self.connection.query_row("SELECT dispatch_at FROM model_requests WHERE request_id=?1", [&id], |row| row.get(0)).optional()?.flatten();
                    (if dispatch.is_some() { "unknown_outcome" } else { "interrupted" }, now_ms())
                }
    ...
    pub fn redact_request(&self, request_id: &str, state: &str) -> Result<()> {
        let tx = self.connection.unchecked_transaction()?;
        let disposition = if state == "redacted" { "hash_removed" } else { "digest_kept" };
        ...
        tx.execute("UPDATE model_responses SET status='interrupted', output=NULL, output_hash=NULL WHERE request_id=?1 AND output IS NOT NULL", [request_id])?;
        tx.execute("UPDATE tool_intents SET state='redacted' WHERE origin_request_id=?1", [request_id])?;
    ...
    pub fn retention_pass(&self, cutoff: i64) -> Result<usize> {
        let ids: Vec<String> = {
            let mut statement = self.connection.prepare("SELECT request_id FROM model_requests WHERE status='active' AND dispatch_at IS NOT NULL AND dispatch_at < ?1")?;
            let rows = statement.query_map([cutoff], |row| row.get(0))?.collect::<std::result::Result<Vec<_>, _>>()?;
            rows
        };
        for id in &ids { self.redact_request(id, "retention_pruned")?; }
        Ok(ids.len())
    }
    ...
    pub fn export_bundle(
        ...
        if destination.exists() { return Err(ModelProvenanceError::CommitFailed("export destination exists".into())); }
        let record = self.get(request_id)?.ok_or(ProvenanceError::SourceMissing)?;
        let parent = destination.parent().unwrap_or_else(|| Path::new("."));
        fs::create_dir_all(parent).map_err(|error| ModelProvenanceError::CommitFailed(error.to_string()))?;
        ...
        for dir in ["receipt_records", "context_ledger", "request_snapshots", "model_responses", "tool_intents", "context_evidence", "context_shadowed_originals", "context_shadow_source_refs", "context_shadow_blocks", "provenance_tombstones"] { fs::create_dir_all(staging.join(dir))?; }
        let request_json = serde_json::to_vec(&record)?;
        write_bytes(&staging.join("model_requests/requests.jsonl"), &line(&request_json))?;
        write_bytes(&staging.join(format!("model_requests/envelopes/{request_id}.json")), &record.envelope_blob)?;
        ...
        for item in stmt.query_map([request_id], |row| Ok((row.get::<_, String>(0)?, row.get::<_, String>(1)?, row.get::<_, String>(2)?)))? {
            ...
            let block: Option<(i64, Option<Vec<u8>>)> = self.connection.query_row("SELECT byte_len,bytes FROM model_request_blocks WHERE content_hash=?1", [&content_hash], |row| Ok((row.get(0)?, row.get(1)?))).optional()?;
            if let Some((byte_len, bytes)) = block { if let Some(bytes) = bytes { write_bytes(&staging.join(format!("model_requests/blocks/{content_hash}.bin")), &bytes)?; blocks.push(serde_json::json!({"content_hash":content_hash,"byte_len":byte_len,"payload_mode":"full","file_path":format!("model_requests/blocks/{content_hash}.bin")})); } else { blocks.push(serde_json::json!({"content_hash":content_hash,"byte_len":byte_len,"payload_mode":"hash_only"})); } }
        }
        write_jsonl(&staging.join("model_requests/block_refs.jsonl"), &block_refs)?;
        write_jsonl(&staging.join("context_evidence/blocks.jsonl"), &blocks)?;
        for path in ["receipt_records/records.jsonl","context_ledger/entries.jsonl","request_snapshots/route_policy.jsonl","model_responses/responses.jsonl","tool_intents/intents.jsonl","context_evidence/sources.jsonl","context_shadowed_originals/records.jsonl","context_shadow_source_refs/refs.jsonl","context_shadow_blocks/blocks.jsonl","provenance_tombstones/tombstones.jsonl"] { write_bytes(&staging.join(path), b"")?; }
        write_bytes(&staging.join("key-history.jsonl"), b"")?;
        write_bytes(&staging.join("checkpoints.jsonl"), b"")?;
        let files = collect_files(&staging)?;
        let bundle_content_sha256 = bundle_digest(&files);
        let manifest = serde_json::json!({"export_id":Uuid::now_v7().to_string(),"created_at":now_ms().unwrap_or_default(),"bundle_schema_version":1,"schema_versions":{"model_request":1,"storage":2},"selection":{"request_id":request_id},"request_count":1,"receipt_count":0,"chain_roots":[],"chain_checkpoints":[],"request_states":[{"request_id":request_id,"payload_mode":record.payload_mode,"status":record.status,"verification_state":if record.payload_mode=="full" {"valid"} else {"legacy_hash_only"},"tombstone_ids":[],"missing_or_pruned_subjects":[]}],"files":files,"bundle_content_sha256":bundle_content_sha256,"signer":{"key_id":signer.key_id(),"algorithm":"Ed25519","signature_path":"bundle.sig"}});
        let manifest_bytes = evohime_receipts::canonicalize_json(&serde_json::to_vec(&manifest)?) .map_err(|error| ModelProvenanceError::CommitFailed(error.to_string()))?;
        write_bytes(&staging.join("manifest.json"), &manifest_bytes)?;
        let digest = Sha256::digest([b"evohime-provenance-manifest-v1\0".as_slice(), manifest_bytes.as_slice()].concat());
        let signature = signer.sign_manifest_digest(&digest)?;
        write_bytes(&staging.join("bundle.sig"), &signature)?;
        fs::rename(&staging, destination).map_err(|error| ModelProvenanceError::CommitFailed(error.to_string()))?;
    ...
    pub fn verify_bundle(bundle: &Path) -> Result<BundleVerification> {
        ...
        let request_id = manifest["selection"]["request_id"].as_str().unwrap_or_default().to_owned();
        let mut errors = Vec::new();
        let files = manifest["files"].as_object().ok_or_else(|| ModelProvenanceError::CommitFailed("manifest files missing".into()))?;
        let mut map = BTreeMap::new();
        for (path, expected) in files { let safe = Path::new(path); if safe.is_absolute() || safe.components().any(|component| matches!(component, std::path::Component::ParentDir)) { errors.push("EXPORT_MANIFEST_MISMATCH".into()); continue; } let bytes = fs::read(bundle.join(safe))?; let actual = hash_bytes_bytes(&bytes); if expected.as_str() != Some(actual.as_str()) { errors.push("EXPORT_MANIFEST_MISMATCH".into()); } map.insert(path.clone(), actual); }
        if manifest["bundle_content_sha256"].as_str() != Some(bundle_digest(&map).as_str()) { errors.push("EXPORT_MANIFEST_MISMATCH".into()); }
        ...
        Ok(BundleVerification { valid: errors.is_empty(), request_id, verification_state: state.into(), errors })
    }
}

fn commit_tx(tx: &Transaction<'_>, envelope: &ModelRequestEnvelopeV1, bytes: &[u8], envelope_hash: Option<&str>, mode: CommitMode) -> Result<ModelRequestRecord> {
    ...
    if let Some(record) = existing { if record.envelope_blob == bytes && record.envelope_hash.as_deref() == envelope_hash { return Ok(record); } return Err(ModelProvenanceError::CommitFailed("request id conflict".into())); }
    if envelope.attempt > 1 {
        let predecessor = envelope.parent_request_id.as_deref().ok_or(ProvenanceError::LineageMismatch)?;
        ...
        let Some((logical, ledger, attempt, hash)) = prior else { return Err(ProvenanceError::LineageMismatch.into()); };
        if logical != envelope.logical_request_id || ledger != envelope.ledger_id || attempt + 1 != i64::from(envelope.attempt) || Some(hash) != envelope.previous_request_hash.clone() { return Err(ProvenanceError::LineageMismatch.into()); }
    }
    let payload_mode = match mode { CommitMode::FullForDispatch => "full", CommitMode::HashOnlyStorage => "hash_only" };
    ...
    for (ordinal, role, content) in std::iter::once((0usize, "system_prompt", envelope.system_prompt.as_str())).chain(envelope.messages.iter().enumerate().map(|(i,m)| (i+1, "message", m.content.as_str()))) {
    ...
    Ok(ModelRequestRecord { request_id: envelope.request_id.clone(), logical_request_id: envelope.logical_request_id.clone(), attempt: envelope.attempt, parent_request_id: envelope.parent_request_id.clone(), previous_request_hash: envelope.previous_request_hash.clone(), request_kind: serde_json::to_string(&envelope.request_kind)?.trim_matches('"').into(), ledger_id: envelope.ledger_id.clone(), provider: envelope.provider.clone(), model: envelope.model.clone(), envelope_version: envelope.version, payload_mode: payload_mode.into(), envelope_hash: envelope_hash.map(str::to_owned), envelope_blob: bytes.to_vec(), context_projection_hash: envelope.context_projection.context_projection_hash.clone(), route_snapshot_hash: envelope.route_snapshot_hash.clone(), policy_snapshot_hash: envelope.policy_snapshot_hash.clone(), route_policy_hash_shared: envelope.route_policy_hash_shared, status: "active".into(), dispatch_at: None, completed_at: None })
}

fn row_record(row: &rusqlite::Row<'_>) -> rusqlite::Result<ModelRequestRecord> {
    Ok(ModelRequestRecord { request_id: row.get(0)?, logical_request_id: row.get(1)?, attempt: row.get(2)?, parent_request_id: row.get(3)?, previous_request_hash: row.get(4)?, request_kind: row.get(5)?, ledger_id: row.get(6)?, provider: row.get(7)?, model: row.get(8)?, envelope_version: row.get(9)?, payload_mode: row.get(10)?, envelope_hash: row.get(11)?, envelope_blob: row.get(12)?, context_projection_hash: row.get(13)?, route_snapshot_hash: row.get(14)?, policy_snapshot_hash: row.get(15)?, route_policy_hash_shared: row.get::<_, i64>(16)? != 0, status: row.get(17)?, dispatch_at: row.get(18)?, completed_at: row.get(19)? })
}

fn hash_bytes(value: &str) -> String { format!("{:x}", Sha256::digest(value.as_bytes())) }
fn hash_bytes_bytes(value: &[u8]) -> String { format!("{:x}", Sha256::digest(value)) }
fn line(value: &[u8]) -> Vec<u8> { let mut bytes = value.to_vec(); bytes.push(b'\n'); bytes }
fn write_bytes(path: &Path, bytes: &[u8]) -> std::io::Result<()> { if let Some(parent) = path.parent() { fs::create_dir_all(parent)?; } let mut file = fs::File::create(path)?; file.write_all(bytes)?; file.sync_all() }
fn write_jsonl(path: &Path, values: &[Value]) -> std::io::Result<()> { let mut bytes = Vec::new(); for value in values { bytes.extend(line(&serde_json::to_vec(value).unwrap_or_default())); } write_bytes(path, &bytes) }
fn collect_files(root: &Path) -> Result<BTreeMap<String, String>> { fn walk(root: &Path, current: &Path, map: &mut BTreeMap<String, String>) -> std::io::Result<()> { for entry in fs::read_dir(current)? { let entry = entry?; let path = entry.path(); if path.is_dir() { walk(root, &path, map)?; } else { let relative = path.strip_prefix(root).unwrap_or(&path).to_string_lossy().replace('\\', "/"); if relative == "manifest.json" || relative == "bundle.sig" { continue; } map.insert(relative, hash_bytes_bytes(&fs::read(path)?)); } } Ok(()) } let mut map = BTreeMap::new(); walk(root, root, &mut map).map_err(|error| ModelProvenanceError::CommitFailed(error.to_string()))?; Ok(map) }
fn bundle_digest(files: &BTreeMap<String, String>) -> String { let mut bytes = b"evohime-provenance-bundle-v1\0".to_vec(); for (path, hash) in files { bytes.extend(path.as_bytes()); bytes.push(0); bytes.extend(hash.as_bytes()); bytes.push(b'\n'); } hash_bytes_bytes(&bytes) }
fn now_ms() -> Option<i64> { SystemTime::now().duration_since(UNIX_EPOCH).ok().map(|duration| duration.as_millis() as i64) }

#[cfg(test)]
mod tests {
    use super::*;
    use evohime_model_provenance::{ContextProjection, ModelMessage, ModelParameters, ProjectionEntry, RequestKind, ToolSchema};

    fn db() -> Connection { let db = Connection::open_in_memory().unwrap(); db.execute_batch("CREATE TABLE context_ledger(id TEXT PRIMARY KEY, context_ledger_hash TEXT NOT NULL)").unwrap(); install_schema(&db).unwrap(); db }
    fn envelope() -> ModelRequestEnvelopeV1 { let mut projection = ContextProjection { ledger_id: "l".into(), context_ledger_hash: "a".repeat(64), entries: vec![ProjectionEntry { projection_entry_id: "m".into(), operation: "include".into(), source_refs: vec![], block_ref_id: Some("b".into()), drop_reason: None }], context_projection_hash: String::new() }; projection.context_projection_hash = projection.compute_hash().unwrap(); ModelRequestEnvelopeV1 { version: 1, request_id: Uuid::now_v7().to_string(), logical_request_id: "logical".into(), attempt: 1, parent_request_id: None, ledger_id: "l".into(), request_kind: RequestKind::Agent, provider: "mock".into(), model: "m".into(), route_snapshot_hash: "b".repeat(64), policy_snapshot_hash: "c".repeat(64), route_policy_hash_shared: false, system_prompt: "system".into(), messages: vec![ModelMessage { role: "user".into(), content: "hello".into() }], tools: vec![ToolSchema { name: "tool".into(), description: "tool".into(), input_schema: serde_json::json!({"type":"object"}) }], model_parameters: ModelParameters { temperature: None, top_p: None, max_output_tokens: Some(10), reasoning_mode: None, provider_options: Default::default() }, context_projection: projection, previous_request_hash: None } }

    #[test] fn commit_is_idempotent_and_deduplicates_blocks() { let db = db(); db.execute("INSERT INTO context_ledger VALUES('l',?1)", ["a".repeat(64)]).unwrap(); let repo = ModelProvenanceRepository::new(&db); let one = envelope(); let first = repo.commit_envelope(&one, CommitMode::FullForDispatch).unwrap(); let second = repo.commit_envelope(&one, CommitMode::FullForDispatch).unwrap(); assert_eq!(first, second); assert_eq!(db.query_row("SELECT COUNT(*) FROM model_request_blocks", [], |r| r.get::<_,i64>(0)).unwrap(), 2); }
    #[test] fn failed_lineage_does_not_leave_rows() { let db = db(); db.execute("INSERT INTO context_ledger VALUES('l',?1)", ["a".repeat(64)]).unwrap(); let repo = ModelProvenanceRepository::new(&db); let mut one = envelope(); one.attempt = 2; one.parent_request_id = Some("missing".into()); one.previous_request_hash = Some("d".repeat(64)); assert!(repo.commit_envelope(&one, CommitMode::FullForDispatch).is_err()); assert_eq!(db.query_row("SELECT COUNT(*) FROM model_requests", [], |r| r.get::<_,i64>(0)).unwrap(), 0); }
}
