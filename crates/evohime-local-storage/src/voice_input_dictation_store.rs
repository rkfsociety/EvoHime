...
pub fn save(
    ...
    let old:Option<(u64,String)>=c.query_row("SELECT revision,content_hash FROM voice_input_dictation WHERE profile_id=?1 AND idempotency_key=?2",params![id,k],|x|Ok((x.get(0)?,x.get(1)?))).optional()?;
    if let Some((or, oh)) = old {
        if or == r && oh == h {
            return Ok(());
        }
        return Err(rusqlite::Error::InvalidParameterName(
            "dictation idempotency conflict".into(),
        ));
    }
    let cur: Option<u64> = c
        .query_row(
            "SELECT MAX(revision) FROM voice_input_dictation WHERE profile_id=?1",
            params![id],
            |x| x.get(0),
        )
        .optional()?
    ...
    if r != cur.unwrap_or(0) + 1 {
        return Err(rusqlite::Error::InvalidParameterName(
            "dictation revision conflict".into(),
        ));
    }
    c.execute(
        "INSERT INTO voice_input_dictation VALUES(?1,?2,?3,?4,?5,?6)",
        params![id, r, h, j, k, n],
    )?;
    Ok(())
}
