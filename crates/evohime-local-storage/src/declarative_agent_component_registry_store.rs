...
/// Creates the declarative component registry and migration-history tables.
///
/// # Errors
///
...
/// Inserts a registry snapshot without replacing an existing registry ID.
///
/// Returns `false` if the ID is already stored.
///
/// # Errors
///
...
/// Replaces a registry snapshot only when its current revision matches `expected`.
///
/// Returns `false` when the registry is absent or its revision has changed.
///
/// # Errors
///
/// Returns the underlying SQLite write error.
...
/// Loads the serialized registry snapshot by ID, if present.
///
/// # Errors
///
