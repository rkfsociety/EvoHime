...
/// Creates the policy, state, and append-only usage tables for team budgets.
...
/// Inserts a versioned policy once; an existing policy ID returns `false`.
pub fn put_policy(
    c: &Connection,
    id: &str,
    version: u64,
...
/// Returns the serialized policy for `id`, if one is stored.
...
/// Creates a session state when `expected` is `None`, or updates it with a version compare-and-set.
pub fn put_state(
    c: &Connection,
    id: &str,
    policy_version: u64,
...
/// Appends a usage event once by event ID; duplicate IDs return `false`.
pub fn append_usage(
    c: &Connection,
    id: &str,
    session: &str,
