...
mod tests {
    ...
    #[test]
    fn migration_12_to_13_is_idempotent_and_preserves_existing_rows() {
        let path = temp_database_path("feedback-migration");
        let _ = std::fs::remove_file(&path);
        let _ = std::fs::remove_file(path.with_extension("db.bak"));
        {
            // Seed a pre-wave (user_version 12) database with an existing
            // memory_entries row, so we can confirm migration 13 does not
        ...
        let database = LocalDatabase::open(&path).expect("database migrates to 13");
        assert_eq!(database.schema_version().expect("version reads"), 13);
        ...
        assert_eq!(reopened.schema_version().expect("version still 13"), 13);
