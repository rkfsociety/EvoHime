...
/// Creates revisioned review-plan and run-to-plan pin tables.
///
/// # Errors
///
...
/// Appends a deterministic review-plan revision with idempotency checks.
///
/// Plan JSON is limited to 64 KiB, revisions must be contiguous, and retrying a
/// key requires the original revision and content hash.
///
/// # Errors
///
/// Returns a SQLite error for size, revision, idempotency, or write failures.
pub fn save(
    c: &Connection,
    id: &str,
    r: u64,
...
/// Loads the serialized plan at its greatest stored revision.
///
/// # Errors
///
...
/// Pins a specific plan revision and content hash to a review run.
///
/// Pin rows are keyed by run ID and are not silently replaced.
///
/// # Errors
///
/// Returns a SQLite error if the run pin cannot be inserted.
