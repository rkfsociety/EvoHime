...
pub fn put(
    ...
    Ok(c.execute(
        "INSERT INTO standing_approval_profiles(id,version,enabled,profile_json,updated_at_ms) VALUES(?1,?2,?3,?4,?5) ON CONFLICT(id) DO UPDATE SET version=excluded.version,enabled=excluded.enabled,profile_json=excluded.profile_json,updated_at_ms=excluded.updated_at_ms WHERE excluded.version > standing_approval_profiles.version",
...
mod tests {
    ...
    #[test]
    fn stale_version_cannot_revoke_newer_profile() {
        let c = Connection::open_in_memory().unwrap();
        install_schema(&c).unwrap();
        assert!(put(&c, "profile", 2, true, br#"{"version":2}"#, 2).unwrap());
        assert!(!put(&c, "profile", 1, false, br#"{"version":1}"#, 3).unwrap());
    ...
    fn list_is_bounded() {
        ...
        install_schema(&c).unwrap();
        for index in 0..300 {
            assert!(put(&c, &format!("profile-{index:03}"), 1, true, b"{}", index).unwrap());
        }
