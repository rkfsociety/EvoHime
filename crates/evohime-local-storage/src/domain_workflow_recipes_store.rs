...
pub fn save(
    ...
    let old:Option<(u64,String)>=c.query_row("SELECT revision,content_hash FROM domain_workflow_recipes WHERE id=?1 AND idempotency_key=?2",params![id,k],|x|Ok((x.get(0)?,x.get(1)?))).optional()?;
    ...
    c.execute(
        "INSERT INTO domain_workflow_recipes VALUES(?1,?2,?3,?4,?5,?6)",
        params![id, r, h, j, k, n],
    )?;
    Ok(())
}
