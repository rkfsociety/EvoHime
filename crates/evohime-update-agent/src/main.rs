...
mod tests {
    use super::{
        ...
        validate_runtime_manifest, write_staged_manifest, CompatibleComponent, CompatibleManifest,
        RuntimeReleaseEntry, RuntimeReleaseManifest, UpdateCandidate, UpdaterBootstrapPaths,
        UpdaterRequirement,
    };
    use sha2::Digest;
    use std::{
    ...
    fn staged_manifest_repairs_legacy_listener_runtime_dependency() {
        ...
        let merged: serde_json::Value = serde_json::from_str(
            &fs::read_to_string(merged_destination).expect("read merged marker"),
        )
        .unwrap();
