fn main() {
    ...
    let manifest: serde_json::Value =
        serde_json::from_slice(&std::fs::read(path).expect("receipt limits manifest"))
    ...
    std::fs::write(
        std::path::Path::new(&std::env::var("OUT_DIR").unwrap()).join("receipt_limits.rs"),
        out,
    )
    .expect("write generated receipt limits");
}
