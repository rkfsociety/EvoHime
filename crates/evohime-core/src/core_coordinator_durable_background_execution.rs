...
pub(super) async fn handle(state: Arc<Mutex<CoordinatorState>>, command: CoreCommand) {
    let CoreCommand::DurableBackgroundExecution {
        operation,
        run_id,
        owner_scope,
        payload,
    ...
    let projection = match &result {
        Ok(value) => String::from_utf8(value.clone()).unwrap_or_else(|_| {
            "{\"status\":\"error\",\"error_code\":\"invalid_projection_encoding\"}".into()
        }),
        Err(error) => {
            let journal = state.lock().await.journal.clone();
            if let Some(journal) = journal {
                String::from_utf8(journal.durable_background_error_projection(error).await)
                    .unwrap_or_else(|_| "{\"status\":\"error\"}".into())
            } else {
                "{\"status\":\"error\",\"error_code\":\"storage_unavailable\"}".into()
            }
        }
    };
    let event = CoreEvent::DurableBackgroundExecution {
        run_id: event_run_id,
