...
pub fn child_output_is_validated_before_it_becomes_an_input() -> Result<(), String> {
    ...
    let wrong_status = NodeSuccess::new(serde_json::json!({"status": "accepted"})).with_evidence(2);
...
pub fn terminal_states_separate_cancel_failure_and_unknown_outcome() -> Result<(), String> {
    ...
    require(
        terminal_state(&workflow, &cancelled).map(|(state, _)| state) == Some(RunState::Cancelled),
    ...
    let (state, reason) = terminal_state(&workflow, &dead).ok_or("dead letter must be terminal")?;
    require(
    ...
    require(
        terminal_state(&workflow, &degraded).map(|(state, _)| state) == Some(RunState::Degraded),
...
pub fn a_started_snapshot_ignores_library_changes() -> Result<(), String> {
    let template =
        crate::workflow_templates::template("repository-research").ok_or("template must exist")?;
