...
mod tests {
    ...
    fn partial_coverage_cannot_be_clean() {
        let target = CodeReviewTarget {
            schema_version: 1,
            id: "target-a".into(),
            kind: TargetKind::CommitRange,
            base_revision: "a".into(),
            head_revision: "b".into(),
            diff_hash: "hash".into(),
        ...
        let record = CodeReviewRecord {
            ...
            coverage: CodeReviewCoverage {
                ...
                eligible_files: 1,
                reviewed_files: 0,
                skipped_generated: vec![],
                unsupported_files: vec![],
                context_failures: vec![],
        ...
        .unwrap();
        assert_eq!(
            conservative_verdict(&record),
            Ok(ReviewVerdict::ReviewIncomplete)
        );
    }
}
