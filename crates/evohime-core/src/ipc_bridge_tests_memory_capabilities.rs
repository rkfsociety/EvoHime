...
async fn memory_create_list_search_archive_forget_round_trip_against_real_storage() {
    let path = std::env::temp_dir().join(format!("evohime-ipc-memory-{}.db", std::process::id()));
    ...
    let listed_after: serde_json::Value =
        serde_json::from_slice(&list_after_archive.payload).expect("list payload is valid json");
...
async fn memory_pending_conflict_confirm_reject_supersede_round_trip() {
    ...
    let conflicts = send(
        ...
        &mut server_writer,
        "memory-conflicts-1",
        generated::command_envelope::Command::GetMemoryConflicts(generated::GetMemoryConflicts {
            scope_kind: "project".into(),
            project_id: "proj-1".into(),
            secondary_id: String::new(),
            limit: 10,
...
async fn capability_selection_get_pin_replace_round_trip_against_real_storage() {
    ...
    .await;
    let after_replace_json: serde_json::Value =
        serde_json::from_slice(&after_replace.payload).expect("selection payload is valid json");
    assert_eq!(
...
async fn child_handoff_request_report_security_boundary_against_real_storage() {
    let path = std::env::temp_dir().join(format!("evohime-ipc-child-{}.db", std::process::id()));
    let _ = std::fs::remove_file(&path);
    let journal = EventJournal::open(&path).expect("journal opens");
    let (coordinator, _events) = TaskCoordinator::new_with_journal(8, None, journal.clone());
    let bridge = IpcBridge::with_coordinator(journal, coordinator);
    ...
    let submitted = send(
        ...
        &mut server_writer,
        "child-request-valid",
        generated::command_envelope::Command::SubmitChildRequest(generated::SubmitChildRequest {
    ...
    let handoff = send(
        ...
        &mut server_writer,
        "child-handoff-valid",
        generated::command_envelope::Command::RequestChildHandoff(generated::RequestChildHandoff {
