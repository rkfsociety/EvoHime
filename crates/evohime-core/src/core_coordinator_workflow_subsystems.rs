    ...
    match command {
        ...
        CoreCommand::AgentProgramOptimizer { operation, program_id, payload, expected_revision, idempotency_key, reply } => {
            ...
            let result = async {
                ...
                let database = journal.database().lock().await;
                match operation.as_str() {
                    "save" => { let program: crate::agent_program_optimizer::AgentProgram = serde_json::from_slice(&payload).map_err(|_| "invalid_agent_program".to_string())?; crate::agent_program_optimizer::validate(&program).map_err(|e| e.to_string())?; if program.id != program_id { return Err("program_id_mismatch".into()); } let json=serde_json::to_vec(&program).map_err(|e|e.to_string())?; evohime_local_storage::agent_program_optimizer_store::save(database.connection(),&program.id,program.revision,&program.content_hash,&json,&idempotency_key,crate::task_memory::now_millis() as i64).map_err(|e|e.to_string())?; serde_json::to_vec(&serde_json::json!({"status":"stored","program_id":program.id,"revision":program.revision,"content_hash_prefix":&program.content_hash[..8]})).map_err(|e|e.to_string()) }
                    "get" => { let json=evohime_local_storage::agent_program_optimizer_store::current(database.connection(),&program_id).map_err(|e|e.to_string())?.ok_or_else(||"agent_program_not_found".to_string())?; let program:crate::agent_program_optimizer::AgentProgram=serde_json::from_slice(&json).map_err(|_|"corrupt_agent_program".to_string())?; crate::agent_program_optimizer::validate(&program).map_err(|e|e.to_string())?; serde_json::to_vec(&serde_json::json!({"status":"ok","program_id":program.id,"revision":program.revision,"lifecycle":program.lifecycle,"step_count":program.steps.len(),"content_hash_prefix":&program.content_hash[..8]})).map_err(|e|e.to_string()) }
                    "optimize" => { let json=evohime_local_storage::agent_program_optimizer_store::current(database.connection(),&program_id).map_err(|e|e.to_string())?.ok_or_else(||"agent_program_not_found".to_string())?; let program:crate::agent_program_optimizer::AgentProgram=serde_json::from_slice(&json).map_err(|_|"corrupt_agent_program".to_string())?; let verdict=crate::agent_program_optimizer::optimize(&program).map_err(|e|e.to_string())?; serde_json::to_vec(&serde_json::json!({"status":"evaluated","program_id":verdict.program_id,"revision":verdict.revision,"score":verdict.score,"accepted":verdict.accepted,"reason":verdict.reason})).map_err(|e|e.to_string()) }
                    "pin" => { let request:serde_json::Value=serde_json::from_slice(&payload).map_err(|_|"invalid_optimizer_pin".to_string())?; let run_id=request.get("run_id").and_then(serde_json::Value::as_str).ok_or_else(||"run_id_required".to_string())?; let json=evohime_local_storage::agent_program_optimizer_store::current(database.connection(),&program_id).map_err(|e|e.to_string())?.ok_or_else(||"agent_program_not_found".to_string())?; let program:crate::agent_program_optimizer::AgentProgram=serde_json::from_slice(&json).map_err(|_|"corrupt_agent_program".to_string())?; crate::agent_program_optimizer::optimize(&program).map_err(|e|e.to_string())?; evohime_local_storage::agent_program_optimizer_store::pin(database.connection(),run_id,&program.id,program.revision,&program.content_hash,crate::task_memory::now_millis() as i64).map_err(|e|e.to_string())?; serde_json::to_vec(&serde_json::json!({"status":"pinned","program_id":program.id,"revision":program.revision,"run_id":run_id,"effect_owner":"existing_core_runtime"})).map_err(|e|e.to_string()) }
                    "activate" | "supersede" => Err("lifecycle_transition_requires_new_immutable_revision".into()),
