        ...
        CoreCommand::StandingApprovalProfiles {
            ...
            let result = async {
                    ...
                    match operation.as_str() {
                        ...
                        "create"|"update" => { let p:profiles::StandingApprovalProfile=serde_json::from_slice(&payload).map_err(|_|"invalid_profile".to_string())?; profiles::validate(&p).map_err(|e|e.to_string())?; if !profile_id.is_empty()&&p.id!=profile_id{return Err("profile_id_mismatch".into())}; let j=serde_json::to_vec(&p).map_err(|_|"serialization_failed".to_string())?; if !store::put(database.connection(),&p.id,p.version,p.enabled,&j,crate::task_memory::now_millis() as i64).map_err(|_|"storage_failed".to_string())? { return Err("stale_profile_version".into()); }; serde_json::to_vec(&serde_json::json!({"schema_version":1,"profile_id":p.id,"version":p.version,"status":"saved","redacted":true})).map_err(|_|"serialization_failed".to_string()) }
