...
impl IpcBridge {
    ...
    pub(super) async fn dispatch_next_waiting_adaptation_inner(&self) -> Result<bool, String> {
        let rows = {
        ...
        for (job_id, _, state, snapshot) in rows {
            if state
                != crate::local_model_adaptation::AdaptationState::WaitingForResources.storage_key()
            {
                continue;
            }
            let job: crate::local_model_adaptation::AdaptationJob =
                serde_json::from_slice(&snapshot)
                    .map_err(|_| "adaptation_queue_corrupt_job".to_string())?;
            job.validate()
                .map_err(|_| "adaptation_queue_corrupt_job".to_string())?;
            ...
            coordinator
                .dispatch(CoreCommand::LocalModelRuntimeManager {
                ...
                .await
                .map_err(|_| "adaptation_queue_dispatch_failed".to_string())?;
            let result = response
                .await
                .map_err(|_| "adaptation_queue_response_lost".to_string())?;
