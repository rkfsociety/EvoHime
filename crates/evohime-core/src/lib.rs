...
- Отвечай компактно: сначала дай итог, затем только необходимые детали. Обычно достаточно 1–3 коротких абзацев или до 5 маркированных пунктов. Не пересказывай запрос, ход рассуждений и очевидные шаги; длинные пояснения давай только по явной просьбе.\n\
- После исследования дай краткий отчёт: что обнаружено, какие файлы проверены и что делать дальше. Указывай только существенные факты; не дублируй одно и то же разными словами.\n\n\
...
mod tests {
    ...
    fn agent_system_prompt_explains_workspace_research_flow() {
        ...
        assert!(prompt.contains("Отвечай компактно"));
        assert!(prompt.contains("1–3 коротких абзацев"));
    }
