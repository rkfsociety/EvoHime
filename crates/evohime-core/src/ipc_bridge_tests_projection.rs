...
async fn push_journal_tail_leaves_generic_rows_unprojected() {
    let path = std::env::temp_dir().join(format!("evohime-ipc-generic-{}.db", std::process::id()));
    let _ = std::fs::remove_file(&path);
    let journal = EventJournal::open(&path).expect("journal opens");
    journal
...
async fn clearing_history_hides_reviews_from_the_next_listing() {
    let path = std::env::temp_dir().join(format!("evohime-ipc-clear-{}.db", std::process::id()));
    let _ = std::fs::remove_file(&path);
    let journal = EventJournal::open(&path).expect("journal opens");
    journal
        .record(&CoreEvent::TaskCompleted {
    ...
    .expect("list decodes");
    let before: serde_json::Value = serde_json::from_slice(&response.payload).expect("list json");
    ...
    let after: serde_json::Value = serde_json::from_slice(&response.payload).expect("list json");
    assert!(
...
async fn an_oversized_snapshot_degrades_instead_of_dropping_the_shell() {
    let path = std::env::temp_dir().join(format!("evohime-ipc-snapshot-{}.db", std::process::id()));
    let _ = std::fs::remove_file(&path);
    let journal = EventJournal::open(&path).expect("journal opens");
...
async fn saves_a_revised_plan_only_to_a_markdown_path() {
    let path = std::env::temp_dir().join(format!("evohime-ipc-revision-{}.db", std::process::id()));
    let _ = std::fs::remove_file(&path);
    let journal = EventJournal::open(&path).expect("journal opens");
    let bridge = IpcBridge::new(journal);
...
async fn stale_generation_produces_a_typed_replay_gap_before_replay() {
    ...
    let gap_envelope = generated::EventEnvelope::decode(gap_frame.as_slice()).expect("gap decodes");
    ...
    let event = generated::EventEnvelope::decode(event_frame.as_slice()).expect("event decodes");
    assert_eq!(event.event_type, "task.completed");
    let _ = std::fs::remove_file(path);
}
