...
use evohime_local_storage::{
    automation_store, durable_background_execution_store as store, StorageError,
};
...
fn bounded_json(bytes: &[u8]) -> bool {
    !bytes.is_empty() && bytes.len() <= contract::MAX_SNAPSHOT_BYTES
}

fn error_code(error: &str) -> &'static str {
    if error.contains("idempotency") {
        "idempotency_conflict"
    } else if error.contains("definition") {
        "unknown_definition"
    } else if error.contains("revision") || error.contains("stale") {
        "version_conflict"
...
fn overflow_policy(value: &str) -> Option<contract::OverflowPolicy> {
    match value {
        ...
        "dropoldestallowed" | "drop_oldest_allowed" => {
            Some(contract::OverflowPolicy::DropOldestAllowed)
        }
        "coalescebykey" | "coalesce_by_key" => Some(contract::OverflowPolicy::CoalesceByKey),
        "backpressurecaller" | "backpressure_caller" => {
            Some(contract::OverflowPolicy::BackpressureCaller)
        }
        _ => None,
    }
}
...
fn queue_record_from_snapshot(snapshot: &contract::BackgroundRunSnapshot) -> store::QueueRecord {
    store::QueueRecord {
        ...
        priority: serde_json::to_string(&snapshot.priority)
            .unwrap_or_else(|_| "\"background_workflow\"".into())
            .trim_matches('"')
            .to_owned(),
...
fn overlap_policy(value: &str) -> contract::OverlapPolicy {
    match value {
        ...
        "cancelpreviousthenrun" | "cancel_previous_then_run" => {
...
fn background_schedule_slots(
    ...
    let latest = match spec {
        ...
        contract::ScheduleSpec::Interval {
            every_ms,
            first_at_ms,
        } if now_ms >= *first_at_ms => (*first_at_ms)
            .saturating_add(((now_ms - *first_at_ms) / *every_ms).saturating_mul(*every_ms)),
        contract::ScheduleSpec::Cron {
            minute,
            hour,
            timezone_minutes,
        } => {
            ...
            let target_local = if target_local > local_minute {
                target_local - 1_440
            } else {
                target_local
            };
    ...
    if last.is_some_and(|value| value >= latest) {
        return None;
    }
    let mut slots = match spec {
        contract::ScheduleSpec::Interval {
            ...
            let count = (latest.saturating_sub(first) / *every_ms)
                .saturating_add(1)
                .max(0) as usize;
            match policy {
                contract::MissedFirePolicy::CatchUpBounded => {
                    ...
                    (start..count)
                        .map(|index| first + index as i64 * *every_ms)
                        .collect()
                }
        ...
        _ => match policy {
            contract::MissedFirePolicy::Skip if now_ms.saturating_sub(latest) > 60_000 => {
                Vec::new()
            }
            _ => vec![latest],
        },
...
fn wait_condition_satisfied(
    ...
    match condition {
        ...
        contract::WaitCondition::WaitForDuration {
            resolved_wake_at_ms,
            ..
        } => now_ms >= *resolved_wake_at_ms,
        contract::WaitCondition::WaitForRunState {
            run_id,
            state,
            timeout_at_ms,
        } => {
            automation_store::get_run(c, run_id)
                .ok()
                .flatten()
                .is_some_and(|run| run.state == *state)
                || timeout_at_ms.is_some_and(|timeout| now_ms >= timeout)
        }
        contract::WaitCondition::WaitForHumanWorkItem {
            work_item_id,
            timeout_at_ms,
        } => {
            c.query_row(
                "SELECT state FROM human_work_items WHERE id=?1",
                [work_item_id],
            ...
            .is_some_and(|state| {
                matches!(
                    state.as_str(),
                    "accepted" | "completed" | "resolved" | "cancelled"
                )
            }) || timeout_at_ms.is_some_and(|timeout| now_ms >= timeout)
        }
    }
...
impl EventJournal {
    ...
    pub async fn poll_durable_background_execution(
        ...
        for wait in waits {
            let condition: contract::WaitCondition =
                match serde_json::from_slice(&wait.condition_json) {
            ...
            if wait_condition_satisfied(database.connection(), &condition, now_ms) {
                ...
                if let Some(run) = automation_store::get_run(database.connection(), &run_id)? {
                    let transitioned = automation_store::transition_run(
                        ...
                        automation_store::RunTransition {
                            run_id: &run_id,
                            from_state: "waiting",
                            to_state: "queued",
                            generation: run.generation,
                            event_type: "background.wakeup",
                            payload_json: "{}",
                            now_ms,
                        },
                    )?;
                    let transitioned = if transitioned {
                        true
                    } else {
    ...
    async fn poll_background_schedules(&self, now_ms: i64) -> Result<u32, StorageError> {
        ...
        for schedule in schedules {
            ...
            let Some((latest, slots)) =
                background_schedule_slots(&spec, schedule.last_slot.as_deref(), now_ms, policy)
            else {
                continue;
            };
            let mut database = self.database.lock().await;
            let Some(definition) = automation_store::get_definition(
                database.connection(),
            ...
            if slots.is_empty() {
                if automation_store::advance_schedule_slot(
                    database.connection(),
                ...
                if matches!(spec, contract::ScheduleSpec::OneShotAt { .. }) {
                    let _ = automation_store::set_schedule_enabled(
                        database.connection(),
                        &schedule.schedule_id,
                        false,
                        now_ms,
            ...
            for logical_fire_ms in slots {
                let fire_key = contract::schedule_fire_key(
                    &schedule.schedule_id,
                    schedule.revision,
                    logical_fire_ms,
                )
                ...
                let active_run = store::find_active_by_concurrency_key(
                    database.connection(),
                    queue_id,
                    &schedule.owner_scope,
                    &schedule.schedule_id,
                ...
                match overlap_policy(&schedule.overlap_policy) {
                    contract::OverlapPolicy::SkipIfRunning | contract::OverlapPolicy::Coalesce
                    ...
                    contract::OverlapPolicy::CancelPreviousThenRun => {
                        if let Some(active_run) = active_run {
                            let _ = automation_store::cancel_run(
                                database.connection_mut(),
                                &active_run,
                                now_ms,
                            )?;
                ...
                let snapshot_json = serde_json::to_vec(&snapshot).map_err(|_| {
                    StorageError::InvalidInput("schedule snapshot serialization".into())
                })?;
                ...
                match automation_store::admit_run(database.connection(), &run, now_ms)? {
                    ...
                    automation_store::AdmitRunResult::Inserted => {
                        store::save_run_metadata(
                            ...
                            &snapshot.queue_ref,
                            "scheduled_workflow",
                            snapshot.concurrency_key.as_deref(),
                        ...
                        automation_store::transition_run(
                            ...
                            automation_store::RunTransition {
                                ...
                                to_state: "queued",
                                generation: 1,
                                event_type: "background.schedule_fired",
                                payload_json: "{}",
                                now_ms,
                            },
                        )?;
    ...
    pub async fn durable_background_command(
        ...
        if operation.is_empty()
            || operation.len() > contract::MAX_ID_BYTES
            || owner_scope.is_empty()
            || owner_scope.len() > contract::MAX_SCOPE_BYTES
            || payload.len() > contract::MAX_SNAPSHOT_BYTES
        {
        ...
        let response = match operation {
            "create_run" => {
                let request: CreateRunRequest = serde_json::from_slice(payload)
                    .map_err(|_| "invalid_run_snapshot".to_string())?;
                ...
                if request.snapshot.owner_scope != owner_scope
                    ...
                    || !idempotency_key.is_empty()
                        && request.snapshot.idempotency_key.as_deref() != Some(idempotency_key)
                {
                    return Err("scope_or_idempotency_mismatch".into());
                }
                if automation_store::get_definition(
                    c,
                    &request.snapshot.source_definition_ref,
                    request.snapshot.source_definition_revision,
                    owner_scope,
                )
                ...
                if let Some(definition_json) = request.definition_json.as_deref() {
                    if definition_json.len() > contract::MAX_SNAPSHOT_BYTES
                        || serde_json::from_str::<Value>(definition_json).is_err()
                    {
                        return Err("invalid_definition_snapshot".into());
                    }
                }
                let snapshot_json = serde_json::to_vec(&request.snapshot)
                    .map_err(|_| "invalid_run_snapshot".to_string())?;
                if !bounded_json(&snapshot_json) {
                    return Err("snapshot_too_large".into());
                }
                let queue = store::get_queue(c, &request.snapshot.queue_ref, owner_scope)
                    .map_err(|e| e.to_string())?
                    .or_else(|| {
                ...
                let (queued, active) = store::queue_load(c, &queue.queue_id, owner_scope)
                    .map_err(|e| e.to_string())?;
                if queued >= queue.max_queued || active >= queue.max_active && queue.max_queued == 0
                {
                    match overflow_policy(&queue.overflow_policy) {
                        Some(contract::OverflowPolicy::CoalesceByKey)
                            if request.snapshot.concurrency_key.is_some() =>
                        {
                            let key = request
                                .snapshot
                                .concurrency_key
                                .as_deref()
                                .unwrap_or_default();
                            let existing = store::find_queued_by_concurrency_key(
                                c,
                                &queue.queue_id,
                                owner_scope,
                                key,
                            ...
                            if let Some(existing) = existing {
                                return Ok(serde_json::to_vec(&serde_json::json!({"status":"ok","accepted":true,"coalesced":true,"run_id":existing,"state":"queued"})).unwrap_or_default());
                            }
                            return Err("queue_overflow_coalescing_key_not_found".into());
                        }
                        Some(contract::OverflowPolicy::DropOldestAllowed) => {
                            if store::drop_oldest_queued(c, &queue.queue_id, owner_scope, now)
                                .map_err(|e| e.to_string())?
                                .is_none()
                            {
                                return Err("queue_overflow_no_droppable_run".into());
                            }
                        }
                        Some(contract::OverflowPolicy::BackpressureCaller) => {
                            return Err("queue_backpressure".into())
                        }
                ...
                let run = automation_store::AutomationRunRecord {
                    ...
                    definition_id: request.snapshot.source_definition_ref.clone(),
                    revision: request.snapshot.source_definition_revision,
                    ...
                    state: "admitted".into(),
                    generation: 1,
                    permission_snapshot: request.snapshot.execution_policy_ref.clone(),
                ...
                match automation_store::admit_run(c, &run, now).map_err(|e| e.to_string())? {
                    automation_store::AdmitRunResult::IdempotencyConflict { .. } => {
                        return Err("idempotency_conflict".into())
                    }
                    automation_store::AdmitRunResult::Existing(existing) => {
                        serde_json::json!({"status":"ok","accepted":true,"deduplicated":true,"run_id":existing.run_id,"state":existing.state})
                    }
                    automation_store::AdmitRunResult::Inserted => {
                        store::save_run_metadata(
                            c,
                            ...
                            "background",
                            &snapshot_json,
                            &request.snapshot.queue_ref,
                            &format!("{:?}", request.snapshot.priority).to_ascii_lowercase(),
                            request.snapshot.concurrency_key.as_deref(),
                            &request.snapshot.content_hash,
                            request.snapshot.next_wakeup_at_ms,
                            now,
                        )
                        .map_err(|e| e.to_string())?;
                        if let Some(wake) = request.snapshot.next_wakeup_at_ms {
                            store::put_wait(
                                c,
                                &store::WaitRecord {
                                    run_id: run.run_id.clone(),
                                    revision: request.snapshot.source_definition_revision,
                                    condition_json: serde_json::to_vec(
                                        &serde_json::json!({"kind":"wait_until","wake_at_ms":wake}),
                                    )
                                    .unwrap_or_default(),
                        ...
                        if request.snapshot.next_wakeup_at_ms.is_none() {
                            automation_store::transition_run(
                                ...
                                automation_store::RunTransition {
                                    ...
                                    to_state: "queued",
                                    generation: run.generation,
                                    event_type: "background.accepted",
                                    payload_json: "{}",
                                    now_ms: now,
                                },
                            )
            ...
            "get_run" => {
                let run = automation_store::get_run(c, run_id)
                    .map_err(|e| e.to_string())?
                    .ok_or_else(|| "unknown_run".to_string())?;
                if run.owner_scope != owner_scope {
                    return Err("scope_mismatch".into());
                }
                let snapshot = store::background_snapshot(c, run_id)
                    .map_err(|e| e.to_string())?
                    .unwrap_or_default();
            ...
            "list_runs" => {
                let runs = automation_store::list_runs(c, owner_scope, "", 256)
                    .map_err(|e| e.to_string())?;
            ...
            "list_schedules" => {
                let schedules = store::list_background_schedules(c, owner_scope, 256)
                    .map_err(|e| e.to_string())?;
            ...
            "create_schedule" | "revise_schedule" => {
                let request: BackgroundScheduleRequest =
                    serde_json::from_slice(payload).map_err(|_| "invalid_schedule".to_string())?;
                if request.owner_scope != owner_scope
                    || request.schedule_id.is_empty()
                    || request.schedule_id.len() > contract::MAX_ID_BYTES
                    || request.revision == 0
                {
                    return Err("invalid_schedule_scope_or_revision".into());
                }
                ...
                let (hour, minute, timezone_minutes) = match &request.spec {
                    contract::ScheduleSpec::Cron {
                        minute,
                        hour,
                        timezone_minutes,
                    } => (*hour, *minute, *timezone_minutes),
                    _ => (0, 0, 0),
                };
                let spec_json = serde_json::to_string(&request.spec)
                    .map_err(|_| "invalid_schedule".to_string())?;
                let saved = store::save_background_schedule(
                    c,
                    &automation_store::AutomationScheduleRecord {
                    ...
                    &spec_json,
                    &format!("{:?}", request.missed_fire_policy).to_ascii_lowercase(),
                    &format!("{:?}", request.overlap_policy).to_ascii_lowercase(),
                    now,
                )
                .map_err(|e| e.to_string())?;
            ...
            "set_schedule_enabled" => {
                ...
                if !schedules
                    .iter()
                    .any(|schedule| schedule.schedule_id == run_id)
                {
                    return Err("unknown_schedule".into());
                }
                let enabled = serde_json::from_slice::<serde_json::Value>(payload)
                    .ok()
                    .and_then(|value| value.get("enabled").and_then(Value::as_bool))
                    .ok_or_else(|| "invalid_schedule_enabled".to_string())?;
                let changed = automation_store::set_schedule_enabled(c, run_id, enabled, now)
                    .map_err(|e| e.to_string())?;
            ...
            "cancel_run" => {
                ...
                let changed =
                    automation_store::cancel_run(c, run_id, now).map_err(|e| e.to_string())?;
            ...
            "wait" => {
                ...
                let condition: contract::WaitCondition =
                    serde_json::from_slice(payload).map_err(|_| "invalid_wait".to_string())?;
                ...
                let wake_at = match condition {
                    ...
                    contract::WaitCondition::WaitForDuration {
                        resolved_wake_at_ms,
                        ..
                    } => Some(resolved_wake_at_ms),
                    contract::WaitCondition::WaitForRunState { timeout_at_ms, .. }
                    | contract::WaitCondition::WaitForHumanWorkItem { timeout_at_ms, .. } => {
                        timeout_at_ms
                    }
                };
                let json =
                    serde_json::to_vec(&condition).map_err(|_| "invalid_wait".to_string())?;
                let saved = store::put_wait(
                    c,
                    &store::WaitRecord {
                        run_id: run_id.into(),
            ...
            "upsert_queue" => {
                let queue: contract::BackgroundQueue =
                    serde_json::from_slice(payload).map_err(|_| "invalid_queue".to_string())?;
                queue.validate().map_err(|e| e.to_string())?;
                if queue.owner_scope != owner_scope {
                    return Err("scope_mismatch".into());
                }
                let saved = store::save_queue(
                    c,
                    &store::QueueRecord {
                        queue_id: queue.queue_id.clone(),
                        owner_scope: queue.owner_scope,
                        revision: queue.revision,
                        max_active: queue.max_active,
                        max_queued: queue.max_queued,
                        priority: format!("{:?}", queue.priority).to_ascii_lowercase(),
                        overflow_policy: format!("{:?}", queue.overflow).to_ascii_lowercase(),
            ...
            "dispatch_once" => {
                ...
                let inserted = store::insert_attempt(
                    c,
                    &store::AttemptRecord {
                        attempt_id,
                        run_id: run_id.into(),
                        generation: run.generation,
                        dispatcher_id: "core-background-plane".into(),
                        state: "blocked".into(),
