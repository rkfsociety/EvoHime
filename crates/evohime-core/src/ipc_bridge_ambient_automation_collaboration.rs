    ...
    pub(super) async fn dispatch_automation_collaboration<W: AsyncWrite + Unpin>(
        ...
        match command {
            ...
            Some(generated::command_envelope::Command::AgenticBrowserSession(request)) => {
                ...
                self.write_agentic_browser_session_response(writer, serde_json::to_vec(&result)?)
                    .await?;
            }
            Some(generated::command_envelope::Command::ArtifactHandoffRegistry(request)) => {
                ...
                self.write_artifact_handoff_registry_response(writer, serde_json::to_vec(&result)?)
                    .await?;
            }
            ...
            Some(generated::command_envelope::Command::SubscribeConversationEvents(request)) => {
                let result = self
