...
pub enum PulseHealth { Healthy, Degraded, Failed }
...
impl PulseDigest {
    pub fn from_events(events: &[&str]) -> Self {
        ...
        let health = if dead_lettered > 0 { PulseHealth::Failed } else if failed > 0 || missed > 0 { PulseHealth::Degraded } else { PulseHealth::Healthy };
        ...
        Self { health, completed, missed, failed, dead_lettered, requeued, summary }
    }
}
...
mod tests {
    ...
    fn pulse_never_masks_failure_as_success() {
        let digest = PulseDigest::from_events(&["runtime.schedule_completed", "runtime.schedule_failed"]);
