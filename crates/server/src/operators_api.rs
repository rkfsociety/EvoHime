...
use crate::{
    app::AppState,
    auth::{require_owner, OperatorIdentity},
    ApiError,
};
use axum::{
...
pub(crate) fn validate_operator_name(name: &str) -> Result<String, ApiError> {
    ...
    if name.is_empty() || name.chars().count() > 128 || name.chars().any(char::is_control) {
        return Err(ApiError::BadRequest(
            "имя оператора должно быть от 1 до 128 символов".into(),
        ));
    }
    Ok(name.to_string())
}
...
pub(crate) async fn list(
    ...
    Ok(Json(
        rows.into_iter()
            .map(|row| OperatorResponse::from_row(row, None))
            .collect(),
    ))
}
...
pub(crate) async fn rotate(
    ...
    let Some((row, token)) = evohime_storage::rotate_operator_token(&state.pool, operator_id)
        ...
        return Err(ApiError::BadRequest(
            "оператор не найден или отключён".into(),
        ));
    };
    Ok(Json(OperatorResponse::from_row(row, Some(token))))
}
...
pub(crate) async fn revoke(
    ...
    if revoked.is_none() {
        return Err(ApiError::BadRequest(
            "оператор не найден, уже отключён или это последний owner".into(),
        ));
    }
