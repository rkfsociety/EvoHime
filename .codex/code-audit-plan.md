...
| Tools/listener/supervisor/updater | в работе | В `TelemetryBuffer::export_jsonl` убрана промежуточная коллекция строк и второй проход `join`; `PulseDigest` считает события одним проходом; проверить bounded I/O, процессы, retry и очистку ресурсов. |
