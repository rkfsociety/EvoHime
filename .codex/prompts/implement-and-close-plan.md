...
**Режим `/goal`:** это уже созданная долгоживущая цель. Продолжай маршрут
автоматически и не считай промежуточный ответ завершением. Не проси повторно
отправить этот prompt. Goal можно считать `complete` только после всех условий
ниже; внешний CI после commit остаётся `pending CI`, а не поводом объявлять успех.
