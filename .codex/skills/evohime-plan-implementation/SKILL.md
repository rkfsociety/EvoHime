...
Если prompt пришёл через `/goal`, считай его уже активной долгоживущей целью:
не завершай goal на промежуточном ревью, частичной реализации или pending CI и
не проси повторно прислать prompt. `complete` допустим только после полного
checklist и task-only commit; внешний блокер фиксируй явно.
