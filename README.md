...
- выбирать API-провайдера и модель или подключать локальный Codex CLI;
- просматривать локальные расписания, Pulse, память и состояние фоновых задач;
...
Для self-repair пользователь сначала выбирает provider и model. Диагностика,
изменение файлов, commit, push и перезапуск выполняются отдельными действиями
с подтверждением; автоматического ремонта нет.
...
только кнопкой после выбора provider/model, а commit, push и перезапуск требуют
