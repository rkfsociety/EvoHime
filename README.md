...
GitHub Actions собирает единственный пользовательский файл `EvoHime-Setup.exe`. После установки создаётся один ярлык `EvoHime`, запускающий `EvoHime.exe`; Core и supervisor являются внутренними служебными компонентами.

Установщик нужен только для первой установки — дальше клиент обновляет себя сам из исходников по коммитам. Поэтому релиз ровно один, с постоянным тегом `installer`, и новые не создаются: ручной запуск workflow перезаписывает файл в нём.

```text
https://github.com/rkfsociety/EvoHime/releases/latest/download/EvoHime-Setup.exe
```

CI стартует на каждый коммит, но тяжёлые job включаются по затронутым путям: правка документации не запускает ни Rust-тесты, ни сборку пакета. Фильтровать сам запуск нельзя — клиент обновляется только на коммит с зелёными проверками, а у коммита без прогона проверок нет вовсе; пропущенный job, в отличие от отсутствующего прогона, засчитывается пройденным.

Установщик собирается в прогоне и прикладывается артефактом; в релиз он попадает только по `workflow_dispatch`.
