...
Для локального запуска из корня репозитория нужен PowerShell 7 или новее
(в Windows PowerShell 5.1 сборка не работает):

```powershell
pwsh -File .\start-dev.ps1
```

Основные команды и критерии проверки собраны в [`AGENTS.md`](AGENTS.md), а
карта проектной документации находится в [`docs/README.md`](docs/README.md).
Electron использует встроенный runtime приложения; отдельный web-сервер и
browser launcher в продукт не входят.
