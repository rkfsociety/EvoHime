...
Установщик нужен для первой установки и обновлений. При запуске клиент сверяет коммит своей сборки с последним зелёным коммитом `main`, показывает экран прогресса, скачивает из этого релиза установщик, проверяет commit, размер и SHA-256, а затем подменяет установку под транзакцией с backup и откатом. Обычный интерфейс открывается только после успешного завершения транзакции; уже работающий клиент может подготовить обновление в фоне и предложить перезапуск.
